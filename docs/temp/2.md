...
*   **Multi-Shape Colliders:** The `EseRect` collision works well. If you introduce more complex shapes (circles, polygons), the `_entity_test_collision` and `entity_component_detect_collision_component` functions would need to be extended with appropriate shape-to-shape intersection tests (e.g., SAT for polygons, circle-circle, circle-rect).
