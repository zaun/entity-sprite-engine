...
- Map resizing copies cells and deep-copies; drag: eje_map_set_cell uses ese_mapcell_copy — copies cell and uses memory_manager; good.
- MapCell userdata implementation is thorough. Watch for memory ownership: ese_mapcell_lua_push when cell->lua_ref != LUA_NOREF you do lua_rawgeti(state, ref) — ok.
