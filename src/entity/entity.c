...
EseEntity *entity_copy(EseEntity *entity) {
  ...
  // Copy components
  copy->components = memory_manager.malloc(
      sizeof(EseEntityComponent *) * entity->component_capacity, MMTAG_ENTITY);
  ...
  if (entity->tag_count > 0) {
    copy->tags = memory_manager.malloc(sizeof(char *) * entity->tag_capacity,
                                       MMTAG_ENTITY);
  ...
  // Copy default props
  copy->default_props =
      entity->default_props
          ? dlist_copy(entity->default_props, (DListCopyFn)lua_value_copy)
          : NULL;
...
void entity_run_function_with_args(EseEntity *entity, const char *func_name,
  ...
  for (size_t i = 0; i < entity->component_count; i++) {
    if (!entity->components[i]->active ||
        entity->components[i]->type != ENTITY_COMPONENT_LUA) {
      continue;
    }
    entity_component_run_function(entity->components[i], entity, func_name,
                                  argc, argv);
  }

  profile_stop(PROFILE_ENTITY_LUA_FUNCTION_CALL,
               "entity_run_function_with_args");
}
...
bool entity_test_collision(EseEntity *a, EseEntity *b, EseArray *out_hits) {
  ...
  log_assert("ENTITY", out_hits,
             "entity_test_collision called with NULL out_hits");
  ...
  for (size_t i = 0; i < a->component_count; i++) {
    ...
    for (size_t j = 0; j < b->component_count; j++) {
      ...
      entity_component_detect_collision_with_component(comp_a, comp_b,
                                                       out_hits);
      if (array_size(out_hits) > 0)
        hit = true;
    }
  }
...
void entity_process_collision_callbacks(EseCollisionHit *hit) {
  log_assert("ENTITY", hit,
             "entity_process_collision_callbacks called with NULL hit");
  ...
  if (ese_collision_hit_get_kind(hit) == COLLISION_KIND_COLLIDER) {
    ...
    case COLLISION_STATE_ENTER:
      // Collision Enter
      {
        ...
        entity_run_function_with_args(ese_collision_hit_get_entity(hit),
                                      "entity_collision_enter", 1, args);
      }
      {
        EseLuaValue *args[] = {ese_collision_hit_get_entity(hit)->lua_val_ref};
        entity_run_function_with_args(ese_collision_hit_get_target(hit),
                                      "entity_collision_enter", 1, args);
      }
      break;
    ...
    log_error("ENTITY",
              "entity_process_collision_callbacks: unknown collision kind");
  }

  profile_stop(PROFILE_ENTITY_COLLISION_CALLBACK,
               "entity_process_collision_callbacks");
}

bool entity_detect_collision_rect(EseEntity *entity, EseRect *rect) {
  log_assert("ENTITY", entity,
             "entity_detect_collision_rect called with NULL entity");
  log_assert("ENTITY", rect,
             "entity_detect_collision_rect called with NULL rect");
  ...
  for (size_t i = 0; i < entity->component_count; i++) {
    ...
    if (entity_component_detect_collision_rect(comp, rect)) {
      profile_stop(PROFILE_ENTITY_COLLISION_RECT_DETECT,
                   "entity_detect_collision_rect");
      return true;
    }
  }
...
void entity_draw(EseEntity *entity, float camera_x, float camera_y,
  ...
  log_assert("ENTITY", callbacks->draw_rect,
             "entity_draw called with NULL draw_rect callback");
  ...
  for (size_t i = 0; i < entity->component_count; i++) {
    ...
    entity_component_draw(entity->components[i], camera_x, camera_y, view_width,
                          view_height, callbacks, callback_user_data);
...
const char *entity_component_add(EseEntity *entity, EseEntityComponent *comp) {
  ...
  if (entity->component_capacity == 0) {
    size_t initial_capacity =
        10; // matches ENTITY_INITIAL_CAPACITY previously used
    entity->components = memory_manager.malloc(
        sizeof(EseEntityComponent *) * initial_capacity, MMTAG_ENTITY);
  ...
  if (entity->component_count == entity->component_capacity) {
    ...
    EseEntityComponent **new_components = memory_manager.realloc(
        entity->components, sizeof(EseEntityComponent *) * new_capacity,
        MMTAG_ENTITY);
  ...
  if (comp->type == ENTITY_COMPONENT_COLLIDER) {
    entity_component_collider_update_bounds(
        (EseEntityComponentCollider *)comp->data);
  }
  ...
  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(
      entity->lua->runtime, ENGINE_KEY);
  if (engine) {
    engine_notify_comp_add(engine, comp);
  }
...
bool entity_component_remove(EseEntity *entity, const char *id) {
  log_assert("ENTITY", entity,
             "entity_component_remove called with NULL entity");
  log_assert("ENTITY", id, "entity_component_remove called with NULL id");

  int idx = _entity_component_find_index(entity, id);
  if (idx < 0) {
    log_error("ENTITY", "entity_component_remove: component not found (id=%s)",
              id);
    return false;
  }
...
size_t entity_component_count(EseEntity *entity) {
  log_assert("ENTITY", entity,
             "entity_component_count called with NULL entity");
...
bool entity_add_prop(EseEntity *entity, EseLuaValue *value) {
  ...
  log_assert("ENTITY", prop_name,
             "entity_add_prop called with NULL value name");
...
bool entity_add_tag(EseEntity *entity, const char *tag) {
  ...
  if (entity->tag_count == entity->tag_capacity) {
    size_t new_capacity =
        entity->tag_capacity == 0 ? 4 : entity->tag_capacity * 2;
    char **new_tags = memory_manager.realloc(
        entity->tags, sizeof(char *) * new_capacity, MMTAG_ENTITY);
    if (!new_tags) {
  ...
  if (!normalized_tag) {
    log_error("ENTITY",
              "entity_add_tag: failed to allocate memory for tag string");
    return false;
  }
...
EseRect *entity_get_collision_bounds(EseEntity *entity, bool to_world_coords) {
  log_assert("ENTITY", entity,
             "entity_get_collision_bounds called with NULL entity");
  ...
  if (to_world_coords) {
    ...
    if (entity->collision_world_bounds) {
      ...
      if (world_bounds) {
        ese_rect_set_x(world_bounds, ese_rect_get_x(world_bounds) +
                                         ese_point_get_x(entity->position));
        ese_rect_set_y(world_bounds, ese_rect_get_y(world_bounds) +
                                         ese_point_get_y(entity->position));
      }
      return world_bounds;
    }
  } else {
