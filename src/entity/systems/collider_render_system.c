...
typedef struct {
  EseEntityComponentCollider *
      *colliders;  /** Array of collider component pointers */
...
static void _collider_rect_callback(float screen_x, float screen_y,
                                    uint64_t z_index, float width, float height,
                                    float rotation, bool filled, uint8_t r,
 ...
 */
static bool collider_render_sys_accepts(EseSystemManager *self,
...
static void collider_render_sys_on_add(EseSystemManager *self, EseEngine *eng,
  ...
  if (d->count == d->capacity) {
    ...
    d->colliders = memory_manager.realloc(
        d->colliders, sizeof(EseEntityComponentCollider *) * d->capacity,
        MMTAG_RS_COLLIDER);
  }
 ...
 */
static void collider_render_sys_on_remove(EseSystemManager *self,
                                          EseEngine *eng,
                                          EseEntityComponent *comp) {
  (void)eng;
  ColliderRenderSystemData *d = (ColliderRenderSystemData *)self->data;
...
static void collider_render_sys_init(EseSystemManager *self, EseEngine *eng) {
  (void)eng;
  ColliderRenderSystemData *d = memory_manager.calloc(
      1, sizeof(ColliderRenderSystemData), MMTAG_RS_COLLIDER);
  self->data = d;
}
 ...
 */
static void collider_render_sys_update(EseSystemManager *self, EseEngine *eng,
                                       float dt) {
  (void)dt;
  ColliderRenderSystemData *d = (ColliderRenderSystemData *)self->data;
  ...
  for (size_t i = 0; i < d->count; i++) {
    ...
    for (size_t j = 0; j < cc->rects_count; j++) {
      ...
      _collider_rect_callback(
          screen_x + ese_point_get_x(cc->offset) + ese_rect_get_x(rect),
          screen_y + ese_point_get_y(cc->offset) + ese_rect_get_y(rect),
          cc->base.entity->draw_order, ese_rect_get_width(rect),
          ese_rect_get_height(rect), ese_rect_get_rotation(rect), false, 0, 0,
          255, 255, engine_get_draw_list(eng));
    }
  }
}
 ...
 * @param eng Engine pointer
 */
static void collider_render_sys_shutdown(EseSystemManager *self,
                                         EseEngine *eng) {
  (void)eng;
  ColliderRenderSystemData *d = (ColliderRenderSystemData *)self->data;
  if (d) {
...
static void _collider_rect_callback(float screen_x, float screen_y,
  ...
  draw_list_object_set_bounds(rect_obj, screen_x, screen_y, (int)width,
                              (int)height);
...
EseSystemManager *collider_render_system_create(void) {
  return system_manager_create(&ColliderRenderSystemVTable, SYS_PHASE_LATE,
                               NULL);
}
