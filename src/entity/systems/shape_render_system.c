 ...
 */
static bool shape_render_sys_accepts(EseSystemManager *self,
...
static void shape_render_sys_on_add(EseSystemManager *self, EseEngine *eng,
  ...
  if (d->count == d->capacity) {
    ...
    d->shapes = memory_manager.realloc(
        d->shapes, sizeof(EseEntityComponentShape *) * d->capacity,
        MMTAG_RS_SHAPE);
  }
 ...
 */
static void shape_render_sys_update(EseSystemManager *self, EseEngine *eng,
                                    float dt) {
  (void)dt;
  ShapeRenderSystemData *d = (ShapeRenderSystemData *)self->data;
  ...
  for (size_t i = 0; i < d->count; i++) {
    ...
    if (!shape->base.entity || !shape->base.entity->active ||
        !shape->base.entity->visible) {
      continue;
    }
    ...
    for (size_t idx = 0; idx < shape->polylines_count; ++idx) {
      ...
      if ((polyline_type == POLY_LINE_CLOSED ||
           polyline_type == POLY_LINE_FILLED) &&
          point_count >= 3) {
        points_to_use = memory_manager.malloc(
            sizeof(float) * (point_count + 1) * 2, MMTAG_RS_SHAPE);
        if (points_to_use) {
          ...
          points_to_use = memory_manager.malloc(sizeof(float) * point_count * 2,
                                                MMTAG_RS_SHAPE);
          if (points_to_use) {
            const float *original_points = ese_poly_line_get_points(polyline);
            for (size_t i = 0; i < point_count; i++) {
      ...
      unsigned char stroke_r =
          (unsigned char)(stroke_color ? ese_color_get_r(stroke_color) * 255
                                       : 0);
      unsigned char stroke_g =
          (unsigned char)(stroke_color ? ese_color_get_g(stroke_color) * 255
                                       : 0);
      unsigned char stroke_b =
          (unsigned char)(stroke_color ? ese_color_get_b(stroke_color) * 255
                                       : 0);
      unsigned char stroke_a =
          (unsigned char)(stroke_color ? ese_color_get_a(stroke_color) * 255
                                       : 255);
      ...
      _engine_add_polyline_to_draw_list(
          screen_x, screen_y, 0, points_to_use, point_count_to_use,
          stroke_width, fill_r, fill_g, fill_b, fill_a, stroke_r, stroke_g,
          stroke_b, stroke_a, draw_list);
