...
static void _text_vtable_update(EseEntityComponent *component,
                                EseEntity *entity, float delta_time) {
  _entity_component_text_update((EseEntityComponentText *)component->data,
                                entity, delta_time);
}

static void _text_vtable_draw(EseEntityComponent *component, int screen_x,
...
static bool _text_vtable_run_function(EseEntityComponent *component,
...
static void _text_vtable_collides_component(EseEntityComponent *a,
...
static void _text_vtable_ref(EseEntityComponent *component) {
  ...
  if (text->base.lua_ref == LUA_NOREF) {
    ...
    *ud = text;
    luaL_getmetatable(text->base.lua->runtime,
                      ENTITY_COMPONENT_TEXT_PROXY_META);
...
static void _text_vtable_unref(EseEntityComponent *component) {
  ...
  if (text->base.lua_ref != LUA_NOREF && text->base.lua_ref_count > 0) {
    ...
    if (text->base.lua_ref_count == 0) {
      luaL_unref(text->base.lua->runtime, LUA_REGISTRYINDEX,
                 text->base.lua_ref);
...
static EseEntityComponent *_entity_component_text_make(EseLuaEngine *engine,
                                                       const char *text) {
...
_entity_component_text_copy(const EseEntityComponentText *src) {
  log_assert("ENTITY_COMP", src,
             "_entity_component_text_copy called with NULL src");

  EseEntityComponent *copy =
      _entity_component_text_make(src->base.lua, src->text);
...
void _entity_component_text_destroy(EseEntityComponentText *component) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_text_destroy called with NULL src");
...
void _entity_component_text_update(EseEntityComponentText *component,
  ...
  log_assert("ENTITY_COMP", entity,
             "_entity_component_text_update called with NULL entity");
...
static int _entity_component_text_gc(lua_State *L) {
  // Get from userdata
  EseEntityComponentText **ud = (EseEntityComponentText **)luaL_testudata(
...
static int _entity_component_text_tostring(lua_State *L) {
  ...
  snprintf(
      buf, sizeof(buf),
      "EntityComponentText: %p (id=%s active=%s text='%s' justify=%d align=%d)",
      (void *)component, ese_uuid_get_value(component->base.id),
      component->base.active ? "true" : "false",
      component->text ? component->text : "", (int)component->justify,
      (int)component->align);
...
static int _entity_component_text_new(lua_State *L) {
  ...
  if (n_args == 1 && lua_isstring(L, 1)) {
    ...
    log_debug(
        "ENTITY_COMP",
        "EntityComponentText.new() or EseEntityComponentText.new(String)");
  }
  ...
  // ref
  EseEntityComponentText **ud = (EseEntityComponentText **)lua_newuserdata(
      L, sizeof(EseEntityComponentText *));
...
EseEntityComponentText *_entity_component_text_get(lua_State *L, int idx) {
  ...
  EseEntityComponentText **ud = (EseEntityComponentText **)luaL_testudata(
      L, idx, ENTITY_COMPONENT_TEXT_PROXY_META);
  if (!ud) {
...
void _entity_component_text_init(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "_entity_component_text_init called with NULL engine");

  // Create metatable
  lua_engine_new_object_meta(
      engine, ENTITY_COMPONENT_TEXT_PROXY_META, _entity_component_text_index,
      _entity_component_text_newindex, _entity_component_text_gc,
...
EseEntityComponent *entity_component_text_create(EseLuaEngine *engine,
                                                 const char *text) {
  log_assert("ENTITY_COMP", engine,
             "entity_component_text_create called with NULL engine");
