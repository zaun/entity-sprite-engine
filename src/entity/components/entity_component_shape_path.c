...
static void _shape_path_free_lines_and_destroy(EsePolyLine **lines,
                                               size_t count) {
  if (!lines)
    return;
...
static bool _shape_path_ensure_lines_capacity(EsePolyLine ***lines_ptr,
                                              size_t *count, size_t *cap) {
  if (*count < *cap)
    return true;
  size_t new_cap = *cap ? (*cap * 2) : 4;
  EsePolyLine **tmp =
      (EsePolyLine **)realloc(*lines_ptr, sizeof(EsePolyLine *) * new_cap);
  if (!tmp)
    return false;
  *lines_ptr = tmp;
  *cap = new_cap;
...
static bool _shape_path_handle_moveto(const EseLuaEngine *engine, float scale,
                                      const char **pp, EsePolyLine ***lines_ptr,
                                      size_t *lines_count, size_t *lines_cap,
                                      EsePolyLine **pcurrent, float *cx,
...
static bool _shape_path_handle_lineto(const EseLuaEngine *engine, float scale,
  ...
  while (1) {
    _shape_path_skip_separators(pp);
    if (!**pp || !(isdigit((unsigned char)**pp) || **pp == '+' || **pp == '-' ||
                   **pp == '.'))
      break;
    int ok;
    double x = _shape_path_parse_number(pp, &ok);
...
static bool _shape_path_handle_hlineto(const EseLuaEngine *engine, float scale,
                                       const char **pp, EsePolyLine **pcurrent,
                                       float *cx, float *cy, bool relative,
                                       EsePolyLine ***lines_ptr,
                                       size_t *lines_count, size_t *lines_cap) {
...
static bool _shape_path_handle_vlineto(const EseLuaEngine *engine, float scale,
                                       const char **pp, EsePolyLine **pcurrent,
                                       float *cx, float *cy, bool relative,
                                       EsePolyLine ***lines_ptr,
                                       size_t *lines_count, size_t *lines_cap) {
...
// cubic bezier:
static bool _shape_path_handle_cubic_bezier(
    const EseLuaEngine *engine, float scale, const char **pp,
  ...
  while (1) {
    ...
    for (int i = 1; i <= segments; ++i) {
      ...
      float mt = 1.0f - t;
      float bx = mt * mt * mt * x0 + 3 * mt * mt * t * cx1 +
                 3 * mt * t * t * cx2 + t * t * t * x3;
      float by = mt * mt * mt * y0 + 3 * mt * mt * t * cy1 +
                 3 * mt * t * t * cy2 + t * t * t * y3;
      EsePoint *pt = ese_point_create((EseLuaEngine *)engine);
      if (!pt)
        return false;
...
// smooth cubic bezier:
static bool _shape_path_handle_smooth_cubic_bezier(
    const EseLuaEngine *engine, float scale, const char **pp,
    EsePolyLine **pcurrent, float *cx, float *cy, bool relative,
    EsePolyLine ***lines_ptr, size_t *lines_count, size_t *lines_cap,
    float *prev_ctrl_x, float *prev_ctrl_y, bool *prev_ctrl_valid) {
...
// quadratic bezier:
static bool _shape_path_handle_quadratic_bezier(
    const EseLuaEngine *engine, float scale, const char **pp,
    EsePolyLine **pcurrent, float *cx, float *cy, bool relative,
    EsePolyLine ***lines_ptr, size_t *lines_count, size_t *lines_cap) {
  (void)engine;
...
static bool _shape_path_handle_smooth_quadratic_bezier(
    const EseLuaEngine *engine, float scale, const char **pp,
    EsePolyLine **pcurrent, float *cx, float *cy, bool relative,
    EsePolyLine ***lines_ptr, size_t *lines_count, size_t *lines_cap) {
  (void)engine;
...
// arc curve:
static bool _shape_path_handle_arc(EseLuaEngine *engine, float scale,
                                   const char **pp, EsePolyLine **pcurrent,
                                   float *cx, float *cy, bool relative,
                                   EsePolyLine ***lines_ptr,
                                   size_t *lines_count, size_t *lines_cap) {
  ...
  while (1) {
    ...
    *cy = y1;
    _shape_path_skip_separators(pp);
    while (*pp &&
           (isdigit((unsigned char)**pp) || **pp == '+' || **pp == '-' ||
            **pp == '.' || **pp == ',' || isspace((unsigned char)**pp))) {
      ++(*pp);
    }
  }
  return true;
...
static bool _shape_path_handle_closepath(const EseLuaEngine *engine,
                                         EsePolyLine **pcurrent, float *cx,
                                         float *cy, float spx, float spy) {
  if (!*pcurrent)
    return true;
...
EsePolyLine **shape_path_to_polylines(EseLuaEngine *engine, float scale,
  ...
  while (*p) {
    ...
    if (isalpha((unsigned char)*p)) {
      ...
      log_debug("SVG",
                "shape_path_to_polylines: path did not start with a command");
    ...
    case 'M': // Move To
      ok =
          _shape_path_handle_moveto(engine, scale, &p, &lines, &lines_count,
                                    &lines_cap, &current, &cx, &cy, &spx, &spy);
    ...
    case 'm': { // Relative Move To
      ...
      if (!_shape_path_ensure_lines_capacity(&lines, &lines_count,
                                             &lines_cap)) {
        ok = false;
        break;
      }
    ...
    case 'L': // Line To
      ok = _shape_path_handle_lineto(engine, scale, &p, &current, &cx, &cy,
                                     false, &lines, &lines_count, &lines_cap);
      prev_ctrl_valid = false;
      break;
    ...
    case 'H': // Horizontal Line
      ok = _shape_path_handle_hlineto(engine, scale, &p, &current, &cx, &cy,
                                      false, &lines, &lines_count, &lines_cap);
      prev_ctrl_valid = false;
      break;
    case 'h':
    ...
    case 'V': // Vertical Line
      ok = _shape_path_handle_vlineto(engine, scale, &p, &current, &cx, &cy,
                                      false, &lines, &lines_count, &lines_cap);
      prev_ctrl_valid = false;
      break;
    ...
    case 'C': // Cubic Bezier Curve
      ok = _shape_path_handle_cubic_bezier(
          engine, scale, &p, &current, &cx, &cy, false, &lines, &lines_count,
          &lines_cap, &prev_ctrl_x, &prev_ctrl_y, &prev_ctrl_valid);
      break;
    case 'c': // Relative Cubic Bezier Curve
      ok = _shape_path_handle_cubic_bezier(
          engine, scale, &p, &current, &cx, &cy, true, &lines, &lines_count,
          &lines_cap, &prev_ctrl_x, &prev_ctrl_y, &prev_ctrl_valid);
      break;
    ...
    case 'Q': // Quadratic Bezier Curve
      ok = _shape_path_handle_quadratic_bezier(engine, scale, &p, &current, &cx,
                                               &cy, false, &lines, &lines_count,
                                               &lines_cap);
      prev_ctrl_valid = false;
      break;
    case 'q': // Relative Quadratic Bezier Curve
      ok = _shape_path_handle_quadratic_bezier(engine, scale, &p, &current, &cx,
                                               &cy, true, &lines, &lines_count,
                                               &lines_cap);
      prev_ctrl_valid = false;
      break;
    ...
    case 't': // Relative Smooth Quadratic Bezier Curve
      ...
      break;
    case 'A': // Arc Curve
      ok = _shape_path_handle_arc(engine, scale, &p, &current, &cx, &cy, false,
                                  &lines, &lines_count, &lines_cap);
      prev_ctrl_valid = false;
      break;
    case 'a': // Relative Arc Curve
      ok = _shape_path_handle_arc(engine, scale, &p, &current, &cx, &cy, true,
                                  &lines, &lines_count, &lines_cap);
      prev_ctrl_valid = false;
      break;
    case 'Z': // Close Path
    ...
    default:
      log_debug(
          "SVG",
          "Unsupported SVG command '%c' encountered; skipping its parameters",
          cmd);
