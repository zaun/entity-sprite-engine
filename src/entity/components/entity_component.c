...
EseEntityComponent *entity_component_copy(EseEntityComponent *component) {
  log_assert("ENTITY_COMP", component,
             "entity_component_copy called with NULL component");
...
void entity_component_destroy(EseEntityComponent *component) {
  log_assert("ENTITY_COMP", component,
             "entity_component_destroy called with NULL component");
...
void entity_component_push(EseEntityComponent *component) {
  log_assert("ENTITY_COMP", component,
             "entity_component_push called with NULL component");
...
void entity_component_update(EseEntityComponent *component, EseEntity *entity,
                             float delta_time) {
  log_assert("ENTITY_COMP", component,
             "entity_component_update called with NULL component");
...
void entity_component_detect_collision_with_component(EseEntityComponent *a,
  ...
  if (a->type == ENTITY_COMPONENT_COLLIDER &&
...
bool entity_component_detect_collision_rect(EseEntityComponent *component,
                                            EseRect *rect) {
  log_assert(
      "ENTITY_COMP", component,
      "entity_component_detect_collision_rect called with NULL component");
  log_assert("ENTITY_COMP", rect,
             "entity_component_detect_collision_rect called with NULL rect");
  ...
  for (size_t i = 0; i < collider->rects_count; i++) {
    ...
    ese_rect_set_x(worldRect, ese_rect_get_x(worldRect) +
                                  ese_point_get_x(component->entity->position));
    ese_rect_set_y(worldRect, ese_rect_get_y(worldRect) +
                                  ese_point_get_y(component->entity->position));
    if (ese_rect_intersects(worldRect, rect)) {
      ese_rect_destroy(worldRect);
      profile_stop(PROFILE_ENTITY_COLLISION_RECT_DETECT,
                   "entity_component_detect_coll_rect");
      return true;
    }
    ese_rect_destroy(worldRect);
  }
...
void entity_component_draw(EseEntityComponent *component, float camera_x,
  ...
  profile_stop(PROFILE_ENTITY_DRAW_SCREEN_POS,
               "entity_component_draw_screen_pos");

  component->vtable->draw(component, screen_x, screen_y, callbacks,
                          callback_user_data);
...
bool entity_component_run_function(EseEntityComponent *component,
  ...
  log_assert("ENTITY_COMP", entity,
             "entity_component_run_function called with NULL entity");

  profile_start(PROFILE_ENTITY_LUA_FUNCTION_CALL);

  bool result = component->vtable->run_function(component, entity, func_name,
                                                argc, (void **)argv);

  profile_stop(PROFILE_ENTITY_LUA_FUNCTION_CALL,
               "entity_component_run_function");
  return result;
}

void *entity_component_get_data(EseEntityComponent *component) {
  log_assert("ENTITY_COMP", component,
             "entity_component_get_type called with NULL component");
...
EseEntityComponent *entity_component_get(lua_State *L) {
  ...
  EseEntityComponentLua **ud_lua = (EseEntityComponentLua **)luaL_testudata(
  ...
  EseEntityComponentMap **ud_map = (EseEntityComponentMap **)luaL_testudata(
  ...
  EseEntityComponentShape **ud_shape =
      (EseEntityComponentShape **)luaL_testudata(
  ...
  EseEntityComponentSprite **ud_sprite =
      (EseEntityComponentSprite **)luaL_testudata(
  ...
  EseEntityComponentText **ud_text = (EseEntityComponentText **)luaL_testudata(
      L, 1, ENTITY_COMPONENT_TEXT_PROXY_META);
  if (ud_text) {
    return &(*ud_text)->base;
  }

  luaL_argerror(L, 1,
                "expected a component userdata, got unknown userdata type");
  return NULL;
}
