...
static void _sprite_vtable_collides_component(EseEntityComponent *a, EseEntityComponent *b,
                                              EseArray *out_hits) {
    (void)a;
    (void)b;
    (void)out_hits;
...
static EseEntityComponent *_entity_component_sprite_make(EseLuaEngine *engine,
    ...
    if (sprite_name != NULL) {
        ...
        log_debug("ENTITY_COMP", "Sprite component created with name '%s'", sprite_name);
    } else {
...
static int _entity_component_sprite_new(lua_State *L) {
    ...
    EseEntityComponentSprite **ud =
        (EseEntityComponentSprite **)lua_newuserdata(L, sizeof(EseEntityComponentSprite *));
...
EseEntityComponentSprite *_entity_component_sprite_get(lua_State *L, int idx) {
    ...
    EseEntityComponentSprite **ud =
        (EseEntityComponentSprite **)luaL_testudata(L, idx, ENTITY_COMPONENT_SPRITE_PROXY_META);
    if (!ud) {
...
static int _entity_component_sprite_tostring(lua_State *L) {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "EntityComponentSprite: id=%s active=%s sprite_name=%s",
             ese_uuid_get_value(component->base.id), component->base.active ? "true" : "false",
