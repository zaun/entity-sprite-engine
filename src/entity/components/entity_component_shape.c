...
static EseEntityComponentShape *
_entity_component_shape_polylines_get_component(lua_State *L, int idx) {
  if (!lua_isuserdata(L, idx)) {
    return NULL;
  }
  EseEntityComponentShape **ud = (EseEntityComponentShape **)luaL_testudata(
      L, idx, "ShapePolylinesProxyMeta");
  if (!ud) {
    return NULL;
  }
  return *ud;
...
static int _entity_component_shape_polylines_add(lua_State *L) {
  ...
  if (pl == NULL) {
    return luaL_argerror(L, (n_args == 2 ? 2 : 1),
                         "Expected a PolyLine argument.");
  }
...
static int _entity_component_shape_polylines_remove(lua_State *L) {
  EseEntityComponentShape *shape =
      _entity_component_shape_polylines_get_component(L, 1);
  if (!shape) {
...
static EseEntityComponent *_shape_vtable_copy(EseEntityComponent *component) {
  return _entity_component_shape_copy(
...
static void _shape_vtable_update(EseEntityComponent *component,
                                 EseEntity *entity, float delta_time) {
  // Shape components don't have update functionality
}

static void _shape_vtable_draw(EseEntityComponent *component, int screen_x,
...
static bool _shape_vtable_run_function(EseEntityComponent *component,
...
static void _shape_vtable_collides_component(EseEntityComponent *a,
...
static void _shape_vtable_ref(EseEntityComponent *component) {
  ...
  if (shape->base.lua_ref == LUA_NOREF) {
    ...
    *ud = shape;
    luaL_getmetatable(shape->base.lua->runtime,
                      ENTITY_COMPONENT_SHAPE_PROXY_META);
...
static void _shape_vtable_unref(EseEntityComponent *component) {
  ...
  if (shape->base.lua_ref != LUA_NOREF && shape->base.lua_ref_count > 0) {
    ...
    if (shape->base.lua_ref_count == 0) {
      luaL_unref(shape->base.lua->runtime, LUA_REGISTRYINDEX,
                 shape->base.lua_ref);
...
static void _entity_component_shape_register(EseEntityComponentShape *component,
  ...
  log_assert(
      "ENTITY_COMP", component->base.lua_ref == LUA_NOREF,
...
static EseEntityComponent *_entity_component_shape_make(EseLuaEngine *engine) {
  ...
  component->polylines = memory_manager.malloc(
      sizeof(EsePolyLine *) * SHAPE_POLYLINE_CAPACITY, MMTAG_ENTITY);
...
_entity_component_shape_copy(const EseEntityComponentShape *src) {
  log_assert("ENTITY_COMP", src,
             "_entity_component_shape_copy called with NULL src");
  ...
  shape_copy->polylines = memory_manager.malloc(
      sizeof(EsePolyLine *) * src->polylines_capacity, MMTAG_ENTITY);
...
static int _entity_component_shape_new(lua_State *L) {
  ...
  if (n_args != 0) {
    log_debug("ENTITY_COMP",
              "EntityComponentShape.new() - no arguments expected");
  }
  ...
  // ref
  EseEntityComponentShape **ud = (EseEntityComponentShape **)lua_newuserdata(
      L, sizeof(EseEntityComponentShape *));
...
EseEntityComponentShape *_entity_component_shape_get(lua_State *L, int idx) {
  ...
  EseEntityComponentShape **ud = (EseEntityComponentShape **)luaL_testudata(
      L, idx, ENTITY_COMPONENT_SHAPE_PROXY_META);
  if (!ud) {
...
static int _entity_component_shape_set_path(lua_State *L) {
  ...
  if (argc != 2 && argc != 3) {
    return luaL_error(
        L, "component:set_path(string[, table]) takes 1 or 2 arguments");
  }
  ...
  if (argc == 3) {
    if (lua_istable(L, 3)) {
      ...
      return luaL_error(L,
                        "component:set_path expects options table as 3rd arg");
    }
  }

  size_t new_count = 0;
  EsePolyLine **lines =
      shape_path_to_polylines(component->base.lua, scale, path, &new_count);
  if (!lines && new_count == 0) {
    lua_pushboolean(L, 0);
    return 1;
  }

  // Pre-fetch color templates from options (do not attach directly; we'll copy
  // per polyline)
  ...
  for (size_t i = 0; i < new_count; ++i) {
    if (component->polylines_count == component->polylines_capacity) {
      ...
      EsePolyLine **new_arr = memory_manager.realloc(
          component->polylines, sizeof(EsePolyLine *) * new_capacity,
          MMTAG_ENTITY);
...
static int _entity_component_shape_gc(lua_State *L) {
  // Get from userdata
  EseEntityComponentShape **ud = (EseEntityComponentShape **)luaL_testudata(
      L, 1, ENTITY_COMPONENT_SHAPE_PROXY_META);
  if (!ud) {
...
static int _entity_component_shape_tostring(lua_State *L) {
  ...
  snprintf(buf, sizeof(buf),
           "EntityComponentShape: %p (id=%s active=%s polylines=%zu)",
...
void _entity_component_shape_init(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "_entity_component_shape_init called with NULL engine");

  // Create main metatable
  lua_engine_new_object_meta(
      engine, ENTITY_COMPONENT_SHAPE_PROXY_META, _entity_component_shape_index,
      _entity_component_shape_newindex, _entity_component_shape_gc,
  ...
  lua_engine_new_object_meta(engine, "ShapePolylinesProxyMeta",
                             _entity_component_shape_polylines_index, NULL,
                             NULL, NULL);
}

EseEntityComponent *entity_component_shape_create(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "entity_component_shape_create called with NULL engine");
