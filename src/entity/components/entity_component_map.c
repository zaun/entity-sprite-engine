...
static const char *STANDARD_FUNCTIONS[] = {
    "map_init", "map_update", "cell_update", "cell_enter", "cell_exit"};
static const size_t STANDARD_FUNCTIONS_COUNT =
    sizeof(STANDARD_FUNCTIONS) / sizeof(STANDARD_FUNCTIONS[0]);

// Forward declarations
bool _entity_component_map_collides_component(
    EseEntityComponentMap *component, EseEntityComponentCollider *collider,
    EseArray *out_hits);
...
bool _entity_component_map_run(EseEntityComponentMap *component,
...
static void
_entity_component_map_update_world_bounds(EseEntityComponentMap *component);
...
static void _map_vtable_update(EseEntityComponent *component, EseEntity *entity,
                               float delta_time) {
  _entity_component_map_update((EseEntityComponentMap *)component->data, entity,
                               delta_time);
}

static void _map_vtable_draw(EseEntityComponent *component, int screen_x,
                             int screen_y, void *callbacks, void *user_data) {
  EntityDrawCallbacks *draw_callbacks = (EntityDrawCallbacks *)callbacks;
  _entity_component_map_draw((EseEntityComponentMap *)component->data, screen_x,
                             screen_y, draw_callbacks->draw_texture, user_data);
}

static bool _map_vtable_run_function(EseEntityComponent *component,
                                     EseEntity *entity, const char *func_name,
                                     int argc, void *argv[]) {
...
static void _map_vtable_collides_component(EseEntityComponent *a,
                                           EseEntityComponent *b,
                                           EseArray *out_hits) {
  _entity_component_map_collides_component(
      (EseEntityComponentMap *)a->data, (EseEntityComponentCollider *)b->data,
      out_hits);
}
...
static EseEntityComponent *_entity_component_map_make(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "_entity_component_map_make called with NULL engine");
...
EseEntityComponent *
_entity_component_map_copy(const EseEntityComponentMap *src) {
  log_assert("ENTITY_COMP", src,
             "_entity_component_map_copy called with NULL src");
  ...
  if (src->show_layer) {
    copy->show_layer = memory_manager.malloc(
        sizeof(bool) * copy->show_layer_count, MMTAG_COMP_MAP);
    memcpy(copy->show_layer, src->show_layer,
           sizeof(bool) * copy->show_layer_count);
  }
  ...
  if (copy->map) {
    ...
    copy->sprite_frames =
        memory_manager.malloc(sizeof(int) * cells, MMTAG_COMP_MAP);
...
void _entity_component_map_cleanup(EseEntityComponentMap *component) {
  ...
  if (component->map) {
    ...
    ese_map_remove_watcher(component->map, _entity_component_map_changed,
                           component);
    component->map = NULL;
  }
...
void _entity_component_map_destroy(EseEntityComponentMap *component) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_map_destroy called with NULL src");
...
void _entity_component_map_update(EseEntityComponentMap *component,
                                  EseEntity *entity, float delta_time) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_map_update called with NULL component");
  log_assert("ENTITY_COMP", entity,
             "_entity_component_map_update called with NULL src");
...
void _entity_component_map_cache_functions(EseEntityComponentMap *component) {
  ...
  profile_stop(PROFILE_ENTITY_COMP_MAP_FUNCTION_CACHE,
               "entity_comp_map_cache_functions");
...
void _entity_component_map_clear_cache(EseEntityComponentMap *component) {
  ...
  if (iter) {
    ...
    while (hashmap_iter_next(iter, &key, &value)) {
      ...
      if (cached->exists && cached->function_ref != LUA_NOREF &&
          component->engine) {
        luaL_unref(component->engine->runtime, LUA_REGISTRYINDEX,
                   cached->function_ref);
      }
...
bool _entity_component_map_run(EseEntityComponentMap *component,
                               ...
                               int argc, EseLuaValue *argv[]) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_map_run called with NULL component");
  log_assert("ENTITY_COMP", entity,
             "_entity_component_map_run called with NULL entity");
  log_assert("ENTITY_COMP", func_name,
             "_entity_component_map_run called with NULL func_name");
  ...
  if (!cached) {
    ...
    if (component->instance_ref == LUA_NOREF) {
      ...
      component->instance_ref =
          lua_engine_instance_script(component->engine, component->script);
      profile_stop(PROFILE_ENTITY_COMP_MAP_INSTANCE_CREATE,
                   "entity_comp_map_instance_create");
  ...
  bool result =
      lua_engine_run_function_ref(component->engine, cached->function_ref,
                                  entity_get_lua_ref(entity), argc, argv, NULL);

  if (result) {
    profile_stop(PROFILE_ENTITY_COMP_MAP_FUNCTION_RUN,
                 "entity_comp_map_function_run");
...
static int _entity_component_map_new(lua_State *L) {
  ...
  // ref
  EseEntityComponentMap **ud = (EseEntityComponentMap **)lua_newuserdata(
      L, sizeof(EseEntityComponentMap *));
...
EseEntityComponentMap *_entity_component_map_get(lua_State *L, int idx) {
  ...
  EseEntityComponentMap **ud = (EseEntityComponentMap **)luaL_testudata(
      L, idx, ENTITY_COMPONENT_MAP_PROXY_META);
  if (!ud) {
...
static int _entity_component_map_newindex(lua_State *L) {
  ...
  if (strcmp(key, "active") == 0) {
    ...
    component->show_layer = memory_manager.malloc(
        sizeof(bool) * component->show_layer_count, MMTAG_COMP_MAP);
    memset(component->show_layer, true,
           sizeof(bool) * component->show_layer_count);
    ese_map_add_watcher(component->map, _entity_component_map_changed,
                        component);
    ...
    size_t cells =
        ese_map_get_width(component->map) * ese_map_get_height(component->map);
    component->sprite_frames =
        memory_manager.malloc(sizeof(int) * cells, MMTAG_COMP_MAP);
...
static int _entity_component_map_show_layer_len(lua_State *L) {
  EseEntityComponentMap *component =
      (EseEntityComponentMap *)lua_touserdata(L, lua_upvalueindex(1));
  int len = (component && component->show_layer)
                ? (int)component->show_layer_count
                : 0;
...
static int _entity_component_map_gc(lua_State *L) {
  // Get from userdata
  EseEntityComponentMap **ud = (EseEntityComponentMap **)luaL_testudata(
      L, 1, ENTITY_COMPONENT_MAP_PROXY_META);
  if (!ud) {
...
static int _entity_component_map_tostring(lua_State *L) {
  ...
  snprintf(buf, sizeof(buf),
           "EseEntityComponentMap: %p (id=%s active=%s ma[]=%p)",
...
void _entity_component_map_init(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "_entity_component_map_init called with NULL engine");

  // Create metatable
  lua_engine_new_object_meta(
      engine, ENTITY_COMPONENT_MAP_PROXY_META, _entity_component_map_index,
...
bool _entity_component_map_collides_component(
  ...
  EseRect **world_rects = memory_manager.malloc(
      sizeof(EseRect *) * collider->rects_count, MMTAG_COMP_MAP);
  for (size_t i = 0; i < collider->rects_count; i++) {
    world_rects[i] = ese_rect_create(component->base.lua);
    ese_rect_set_x(world_rects[i],
  ...
  for (size_t y = 0; y < mh; y++) {
    for (size_t x = 0; x < mw; x++) {
      ...
      if (intersect) {
        ...
        EseCollisionHit *hit =
            ese_collision_hit_create(collider->base.entity->lua);
  ...
  profile_stop(PROFILE_ENTITY_COMP_MAP_COLLIDES,
               "entity_comp_map_collides_comp");
  return did_hit;
}

void _entity_component_map_draw_grid(EseEntityComponentMap *component,
                                     float screen_x, float screen_y,
                                     EntityDrawTextureCallback texCallback,
                                     void *callback_user_data) {
  log_assert("ENTITY_COMP_MAP",
             (component->base.entity->draw_order & (DRAW_ORDER_SCALE - 1ULL)) ==
                 0,
             "draw_order must be a multiple of %llu",
             (unsigned long long)DRAW_ORDER_SCALE);

  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(
      component->base.lua->runtime, ENGINE_KEY);
  ...
  for (uint32_t y = 0; y < mh; y++) {
    for (uint32_t x = 0; x < mw; x++) {
      ...
      for (size_t i = 0; i < ese_map_cell_get_layer_count(cell); i++) {
        ...
        float x1, y1, x2, y2;
        int w, h;
        sprite_get_frame(sprite, component->sprite_frames[y * mw + x],
...
void _entity_component_map_draw_hex_point_up(
    EseEntityComponentMap *component, float screen_x, float screen_y,
  ...
  for (uint32_t y = 0; y < mh; y++) {
    for (uint32_t x = 0; x < mw; x++) {
      ...
      float dx = screen_x + (x - cx) * tw;
      float dy = screen_y +
                 (y - cy) * (th * 0.75f); // 3/4 of height for vertical spacing
...
void _entity_component_map_draw_hex_flat_up(
  ...
  for (uint32_t y = 0; y < mh; y++) {
    for (uint32_t x = 0; x < mw; x++) {
      ...
      float dx = screen_x +
                 (x - cx) * (tw * 0.75f); // 3/4 of width for horizontal spacing
      float dy = screen_y + (y - cy) * th;
...
void _entity_component_map_draw_iso(EseEntityComponentMap *component,
                                    float screen_x, float screen_y,
                                    EntityDrawTextureCallback texCallback,
                                    void *callback_user_data) {
  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(
...
void _entity_component_map_draw(EseEntityComponentMap *component,
                                float screen_x, float screen_y,
                                EntityDrawTextureCallback texCallback,
                                void *callback_user_data) {
  if (!component->map) {
  ...
  case MAP_TYPE_GRID:
    _entity_component_map_draw_grid(component, screen_x, screen_y, texCallback,
                                    callback_user_data);
    break;
  case MAP_TYPE_HEX_POINT_UP:
    _entity_component_map_draw_hex_point_up(component, screen_x, screen_y,
                                            texCallback, callback_user_data);
    break;
  case MAP_TYPE_HEX_FLAT_UP:
    _entity_component_map_draw_hex_flat_up(component, screen_x, screen_y,
                                           texCallback, callback_user_data);
    break;
  case MAP_TYPE_ISO:
    _entity_component_map_draw_iso(component, screen_x, screen_y, texCallback,
                                   callback_user_data);
    break;
  default:
...
EseEntityComponent *entity_component_map_create(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "entity_component_map_create called with NULL engine");
...
static void _entity_component_map_changed(EseMap *map, void *userdata) {
  ...
  if (component->show_layer_count != new_count) {
    ...
    component->show_layer = memory_manager.realloc(
        component->show_layer, sizeof(bool) * new_count, MMTAG_COMP_MAP);
...
EseRect *entity_component_map_get_cell_rect(EseEntityComponentMap *component,
                                            int x, int y) {
  log_assert("ENTITY_COMP_MAP", component,
...
_entity_component_map_update_world_bounds(EseEntityComponentMap *component) {
  log_assert(
      "ENTITY_COMP_MAP", component,
      "_entity_component_map_update_world_bounds called with NULL component");

  // If not attached yet, or no map set, clear bounds
  if (!component->base.entity || !component->map) {
    if (component->base.entity &&
        component->base.entity->collision_world_bounds) {
  ...
  if (!component->base.entity->collision_world_bounds) {
    component->base.entity->collision_world_bounds =
        ese_rect_create(component->base.lua);
  }
