 ...
 *
 * @details This structure contains function pointers for all component
 * operations, allowing polymorphic behavior without large switch statements.
 */
typedef struct ComponentVTable {
  ...
  void (*update)(EseEntityComponent *component, EseEntity *entity,
                 float delta_time);
  void (*draw)(EseEntityComponent *component, int screen_x, int screen_y,
               void *callbacks, void *user_data);
  bool (*run_function)(EseEntityComponent *component, EseEntity *entity,
                       const char *func_name, int argc, void *argv[]);
  void (*collides)(EseEntityComponent *a, EseEntityComponent *b,
                   EseArray *out_hits);
...
typedef struct EseEntityComponent {
  ...
  const ComponentVTable
      *vtable; /** Virtual function table for polymorphic operations */
