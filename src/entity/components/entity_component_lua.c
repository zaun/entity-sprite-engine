...
static const char *STANDARD_FUNCTIONS[] = {
    "entity_init", "entity_update", "entity_collision_enter",
...
static void _lua_vtable_update(EseEntityComponent *component, EseEntity *entity,
                               float delta_time) {
  _entity_component_lua_update((EseEntityComponentLua *)component->data, entity,
                               delta_time);
}

static void _lua_vtable_draw(EseEntityComponent *component, int screen_x,
                             int screen_y, void *callbacks, void *user_data) {
  // Lua components don't have draw functionality
}

static bool _lua_vtable_run_function(EseEntityComponent *component,
                                     EseEntity *entity, const char *func_name,
                                     int argc, void *argv[]) {
  return entity_component_lua_run((EseEntityComponentLua *)component->data,
                                  entity, func_name, argc,
                                  (EseLuaValue **)argv);
}

static void _lua_vtable_collides_component(EseEntityComponent *a,
...
static void _entity_component_lua_register(EseEntityComponentLua *component,
                                           bool is_lua_owned) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_lua_register called with NULL component");
  log_assert("ENTITY_COMP", component->base.lua_ref == LUA_NOREF,
             "_entity_component_lua_register component is already registered");
...
void entity_component_lua_ref(EseEntityComponentLua *component) {
  log_assert("ENTITY_COMP", component,
             "entity_component_lua_ref called with NULL component");

  if (component->base.lua_ref == LUA_NOREF) {
    // First time referencing - create userdata and store reference
    EseEntityComponentLua **ud = (EseEntityComponentLua **)lua_newuserdata(
        component->base.lua->runtime, sizeof(EseEntityComponentLua *));
    *ud = component;

    // Attach metatable
    luaL_getmetatable(component->base.lua->runtime,
                      ENTITY_COMPONENT_LUA_PROXY_META);
...
void entity_component_lua_unref(EseEntityComponentLua *component) {
  log_assert("ENTITY_COMP", component,
             "entity_component_lua_unref called with NULL component");
...
static EseEntityComponent *_entity_component_lua_make(EseLuaEngine *engine,
                                                      const char *script) {
  log_assert("ENTITY_COMP", engine,
             "_entity_component_lua_make called with NULL engine");

  EseEntityComponentLua *component =
      memory_manager.malloc(sizeof(EseEntityComponentLua), MMTAG_COMP_LUA);
...
_entity_component_lua_copy(const EseEntityComponentLua *src) {
  log_assert("ENTITY_COMP", src,
             "entity_component_lua_copy called with NULL src");

  EseEntityComponent *copy =
      _entity_component_lua_make(src->base.lua, src->script);
...
void _entity_component_lua_destroy(EseEntityComponentLua *component) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_lua_destroy called with NULL src");
...
void _entity_component_lua_update(EseEntityComponentLua *component,
  ...
  log_assert("ENTITY_COMP", component,
             "_entity_component_lua_update called with NULL component");
  log_assert("ENTITY_COMP", entity,
             "_entity_component_lua_update called with NULL entity");
  ...
  if (component->instance_ref == LUA_NOREF) {
    ...
    profile_stop(PROFILE_ENTITY_COMP_LUA_INSTANCE_CREATE,
                 "entity_comp_lua_instance_create");
    ...
    profile_stop(PROFILE_ENTITY_COMP_LUA_FUNCTION_CACHE,
                 "entity_comp_lua_function_cache");
    ...
    profile_stop(PROFILE_ENTITY_COMP_LUA_FUNCTION_RUN,
                 "entity_comp_lua_init_function");
  ...
  profile_stop(PROFILE_ENTITY_COMP_LUA_FUNCTION_RUN,
               "entity_comp_lua_update_function");
 ...
 *
 * @details Callable from Lua as EseEntityComponentLua.new(). Creates a new
 * EsePoint.
 *
...
static int _entity_component_lua_new(lua_State *L) {
  ...
  // reference
  EseEntityComponentLua **ud = (EseEntityComponentLua **)lua_newuserdata(
      L, sizeof(EseEntityComponentLua *));
...
void _entity_component_lua_cache_functions(EseEntityComponentLua *component) {
  log_assert(
      ...
      "_entity_component_lua_cache_functions called with NULL component");
  ...
  for (size_t i = 0; i < STANDARD_FUNCTIONS_COUNT; ++i) {
    ...
    if (lua_isfunction(L, -1)) {
      ...
      CachedLuaFunction *cached =
          memory_manager.malloc(sizeof(CachedLuaFunction), MMTAG_COMP_LUA);
  ...
  profile_stop(PROFILE_ENTITY_COMP_LUA_FUNCTION_CACHE,
               "entity_comp_lua_cache_functions");
...
void _entity_component_lua_clear_cache(EseEntityComponentLua *component) {
  log_assert("ENTITY_COMP", component,
             "_entity_component_lua_clear_cache called with NULL component");
...
bool entity_component_lua_run(EseEntityComponentLua *component,
  ...
  log_assert("ENTITY_COMP", component,
             "entity_component_lua_run called with NULL component");
  log_assert("ENTITY_COMP", entity,
             "entity_component_lua_run called with NULL entity");
  log_assert("ENTITY_COMP", func_name,
             "entity_component_lua_run called with NULL func_name");
...
EseEntityComponentLua *_entity_component_lua_get(lua_State *L, int idx) {
  ...
  EseEntityComponentLua **ud = (EseEntityComponentLua **)luaL_testudata(
      L, idx, ENTITY_COMPONENT_LUA_PROXY_META);
  if (!ud) {
 ...
 *
 * @details Handles property assignment for EseEntityComponentLua objects from
 * Lua.
 *
 * @param L Lua state pointer
...
static int _entity_component_lua_gc(lua_State *L) {
  // Get from userdata
  EseEntityComponentLua **ud = (EseEntityComponentLua **)luaL_testudata(
      L, 1, ENTITY_COMPONENT_LUA_PROXY_META);
  if (!ud) {
...
static int _entity_component_lua_tostring(lua_State *L) {
  ...
  snprintf(buf, sizeof(buf),
           "EntityComponentLua: %p (id=%s active=%s script=%s)",
...
void _entity_component_lua_init(EseLuaEngine *engine) {
  log_assert("ENTITY_COMP", engine,
             "_entity_component_lua_init called with NULL engine");

  // Create metatable
  lua_engine_new_object_meta(
      engine, ENTITY_COMPONENT_LUA_PROXY_META, _entity_component_lua_index,
      _entity_component_lua_newindex, _entity_component_lua_gc,
...
EseEntityComponent *entity_component_lua_create(EseLuaEngine *engine,
                                                const char *script) {
  log_assert("ENTITY_COMP", engine,
             "entity_component_lua_create called with NULL engine");
