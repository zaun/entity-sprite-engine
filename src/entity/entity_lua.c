...
static void _entity_remove_subscription(EseEntity *entity,
...
void entity_ref(EseEntity *entity) {
  ...
  if (entity->lua_ref == LUA_NOREF) {
    ...
    EseEntity **ud = (EseEntity **)lua_newuserdata(entity->lua->runtime,
                                                   sizeof(EseEntity *));
    *ud = entity;
    // Attach metatable
...
static int _entity_lua_components_find(lua_State *L) {
  ...
  snprintf(full_metatable_name, sizeof(full_metatable_name), "%sProxyMeta",
           comp_type_name);
  ...
  for (size_t i = 0; i < entity->component_count; ++i) {
    ...
    if (lua_getmetatable(L, -1)) {
      ...
      if (lua_isstring(L, -1) &&
          strcmp(lua_tostring(L, -1), full_metatable_name) == 0) {
...
static int _entity_lua_index(lua_State *L) {
  ...
  if (strcmp(key, "id") == 0) {
    ...
    if (entity->position != NULL &&
        ese_point_get_lua_ref(entity->position) != LUA_NOREF) {
      lua_rawgeti(L, LUA_REGISTRYINDEX,
                  ese_point_get_lua_ref(entity->position));
      return 1;
    } else {
      lua_pushnil(L);
      return 1;
...
static int _entity_lua_newindex(lua_State *L) {
  ...
  if (strcmp(key, "id") == 0) {
    ...
    if (public_z < 0 || (uint64_t)public_z > DRAW_ORDER_MAX_USERZ) {
      return luaL_error(
          L, "Entity draw_order must be an integer between 0 and %llu",
...
static int _entity_lua_tostring(lua_State *L) {
  ...
  char buf[128];
  snprintf(buf, sizeof(buf), "Entity: %p (id=%s active=%s components=%zu)",
           (void *)entity, ese_uuid_get_value(entity->id),
           entity->active ? "true" : "false", entity->component_count);
...
static void _lua_value_to_stack(lua_State *L, EseLuaValue *value) {
  ...
  default:
    log_warn("ENTITY", "_lua_value_to_stack: unknown EseLuaValue type %d",
             value->type);
    lua_pushnil(L);
    break;
  }
...
static int _entity_lua_subscribe(lua_State *L) {
  ...
  EseEntitySubscription *sub =
      memory_manager.malloc(sizeof(EseEntitySubscription), MMTAG_ENTITY);
...
void entity_lua_init(EseLuaEngine *engine) {
  ...
  lua_engine_new_object_meta(engine, "EntityProxyMeta", _entity_lua_index,
                             _entity_lua_newindex, _entity_lua_gc,
                             _entity_lua_tostring);

  // Create ComponentsProxyMeta metatable
  lua_engine_new_object_meta(engine, "ComponentsProxyMeta",
                             _entity_lua_components_index, NULL, NULL, NULL);

  // Create global Entity table with functions
  const char *keys[] = {"new",        "find_by_tag", "find_first_by_tag",
                        "find_by_id", "count",       "publish"};
  lua_CFunction functions[] = {_entity_lua_new,
                               _entity_lua_find_by_tag,
                               _entity_lua_find_first_by_tag,
                               _entity_lua_find_by_id,
                               _entity_lua_get_count,
                               _entity_lua_publish};
...
bool _entity_lua_to_data(EseEntity *entity, EseLuaValue *value) {
  ...
  if (!lua_isuserdata(L, -1)) {
    ...
    log_error(
        "ENTITY",
        "entity_add_prop: invalid entity Lua reference (expected userdata)");
    return false;
  }
