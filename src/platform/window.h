 ...
 *          across different platforms (macOS, Linux). It stores
 * platform-specific window handles, dimensions, and references to renderer and
 * input state for consistent window behavior.
 */
typedef struct EseWindow {
  void *platform_window; /** Platform-specific window handle (GLFWwindow* or
                            NSWindow*) */
  int width;             /** Window width in pixels */
  int height;            /** Window height in pixels */
  EseRenderer *renderer; /** Reference to the renderer for this window */
  EseInputState
      *input_state;  /** Reference to the input state for this window */
