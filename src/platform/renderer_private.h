...
typedef struct {
  EseVector1i
      useTexture; /** Flag indicating whether to use texture (1) or color (0) */
...
typedef struct EseRenderer {
  ...
  EseGroupedHashMap
      *shaders; /** Hash map of compiled shaders by group and ID */
  EseGroupedHashMap
      *shadersSources; /** Hash map of shader source code by group and ID */
