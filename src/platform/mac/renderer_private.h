 ...
 *          It provides the Metal implementation of the platform-agnostic
 * renderer with support for efficient GPU rendering and texture management.
 */
typedef struct EseMetalRenderer {
  MTKView *view; /** MetalKit view for rendering */
  RendererViewDelegate
      *viewDelegate; /** Delegate for view events and rendering */
  ...
  MTLPixelFormat
      colorPixelFormat; /** Color pixel format for the render target */

  id<MTLCommandQueue>
      commandQueue; /** Command queue for GPU command submission */
  id<MTLRenderPipelineState>
      pipelineState; /** Render pipeline state for shaders */
