 ...
 *          the bridge between macOS window events and the engine's input
 * system.
 */
...
EseWindow *window_create(int width, int height, const char *title) {
  ...
  EseMetalWindow *metalWindow = (EseMetalWindow *)memory_manager.malloc(
      sizeof(EseMetalWindow), MMTAG_WINDOW);
  metalWindow->inputState = ese_input_state_create(nil);

  // Create window
  NSRect frame = NSMakeRect(100, 100, width, height);
  metalWindow->window = [[NSWindow alloc]
...
void window_process(EseWindow *window, EseInputState *out_input_state) {
  ...
  while ((event = [NSApp nextEventMatchingMask:NSEventMaskAny
    ...
    metalWindow->inputState->keys_down[InputKey_LSHIFT] =
        (mods & NSEventModifierFlagShift) != 0;
    metalWindow->inputState->keys_down[InputKey_RSHIFT] =
        (mods & NSEventModifierFlagShift) != 0;

    metalWindow->inputState->keys_down[InputKey_LCTRL] =
        (mods & NSEventModifierFlagControl) != 0;
    metalWindow->inputState->keys_down[InputKey_RCTRL] =
        (mods & NSEventModifierFlagControl) != 0;

    metalWindow->inputState->keys_down[InputKey_LALT] =
        (mods & NSEventModifierFlagOption) != 0;
    metalWindow->inputState->keys_down[InputKey_RALT] =
        (mods & NSEventModifierFlagOption) != 0;

    metalWindow->inputState->keys_down[InputKey_LCMD] =
        (mods & NSEventModifierFlagCommand) != 0;
    metalWindow->inputState->keys_down[InputKey_RCMD] =
        (mods & NSEventModifierFlagCommand) != 0;

    metalWindow->inputState->keys_down[InputKey_CAPSLOCK] =
        (mods & NSEventModifierFlagCapsLock) != 0;
    ...
    case NSEventTypeRightMouseDragged: {
      NSPoint loc = [metalWindow->view convertPoint:[event locationInWindow]
                                           fromView:nil];
      NSRect viewBounds = [metalWindow->view bounds];
      // Convert from Mac coordinate system (bottom-left origin) to GUI
  ...
  memset(metalWindow->inputState->keys_pressed, 0,
         sizeof(metalWindow->inputState->keys_pressed));
  memset(metalWindow->inputState->keys_released, 0,
         sizeof(metalWindow->inputState->keys_released));
  metalWindow->inputState->mouse_scroll_dx = 0;
  metalWindow->inputState->mouse_scroll_dy = 0;
  memset(metalWindow->inputState->mouse_clicked, 0,
         sizeof(metalWindow->inputState->mouse_clicked));
  memset(metalWindow->inputState->mouse_released, 0,
         sizeof(metalWindow->inputState->mouse_released));
