...
static void _ensure_vertex_ring_buffer(EseMetalRenderer *internal, size_t perFrameNeeded) {
    ...
    if (currentPerFrame >= perFrameNeeded && internal->vertexBuffer) return;

    size_t newPerFrame = currentPerFrame ? currentPerFrame * 2 : (1 << 20);
    while (newPerFrame < perFrameNeeded) newPerFrame *= 2;
    if (newPerFrame < perFrameNeeded) newPerFrame = perFrameNeeded;

    size_t total = newPerFrame * inflight;
    id<MTLBuffer> newBuf = [internal->device newBufferWithLength:total options:MTLResourceStorageModeShared];
    if (!newBuf) return;

    if (internal->vertexBuffer) [internal->vertexBuffer release];
...
static void _ensure_vertex_buffer_capacity(EseMetalRenderer *internal, size_t needed) {
    ...
    id<MTLBuffer> newBuffer = [internal->device newBufferWithLength:newCap options:MTLResourceStorageModeShared];
    if (!newBuffer) {
...
void renderer_destroy(EseRenderer* renderer) {
    ...
    if (internal->vertexBuffer) [internal->vertexBuffer release];
    if (internal->uboBuffer) [internal->uboBuffer release];
...
void renderer_draw(EseRenderer *renderer) {
    ...
    if (renderer->render_list && render_list_get_batch_count(renderer->render_list) > 0) {
        ...
        for (size_t i = 0; i < numBatches; ++i) {
            ...
            [encoder setVertexBuffer:internal->vertexBuffer offset:(frameBase + writeOffset) atIndex:0];
