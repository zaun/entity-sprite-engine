...
bool _renderer_shader_compile_source(EseRenderer *renderer,
  ...
  id<MTLLibrary> vertexLibrary =
      [internal->device newLibraryWithSource:shaderSourceString
                                     options:nil
                                       error:&error];
  if (!vertexLibrary) {
    free_shader_blob(vs);
    ...
    log_debug("RENDERER", "Failed to compile shader library %s:\n%s",
              library_name, error.localizedDescription.UTF8String);
    return false;
  }
  grouped_hashmap_set(renderer->shaders, library_name, "vertexShader",
                      [vertexLibrary retain]);
  grouped_hashmap_set(renderer->shadersSources, library_name, "vertexShader",
                      [shaderSourceString retain]);

  log_debug("RENDERER", "Vertex shader source: %s.",
            shaderSourceString.UTF8String);
  ...
  log_debug("RENDERER", "Fragment shader source: %s.",
            shaderSourceString.UTF8String);
...
static inline size_t _align_up(size_t v, size_t a) {
  return (v + (a - 1)) & ~(a - 1);
}

static void _ensure_vertex_ring_buffer(EseMetalRenderer *internal,
                                       size_t perFrameNeeded) {
  if (!internal)
    return;
  ...
  id<MTLBuffer> newBuf =
      [internal->device newBufferWithLength:total
...
static void _ensure_vertex_buffer_capacity(EseMetalRenderer *internal,
                                           size_t needed) {
  if (!internal)
    return;
  ...
  size_t newCap = internal->vertexBufferCapacity
                      ? internal->vertexBufferCapacity * 2
                      : (1 << 20);
  while (newCap < needed)
    newCap *= 2;
  if (newCap < needed)
  ...
  id<MTLBuffer> newBuffer =
      [internal->device newBufferWithLength:newCap
                                    options:MTLResourceStorageModeShared];
  if (!newBuffer) {
...
EseRenderer *renderer_create(bool hiDPI) {
  ...
  EseRenderer *renderer =
      memory_manager.malloc(sizeof(EseRenderer), MMTAG_RENDERER);
  memset(renderer, 0, sizeof(EseRenderer));

  // Allocate internal Metal renderer
  EseMetalRenderer *internal =
      memory_manager.malloc(sizeof(EseMetalRenderer), MMTAG_RENDERER);
  ...
  internal->view = [[MTKView alloc] initWithFrame:CGRectZero
  ...
  internal->viewDelegate =
      [[RendererViewDelegate alloc] initWithRenderer:renderer];
  ...
  renderer->shaders =
      grouped_hashmap_create((EseGroupedHashMapFreeFn)_free_hash_item);
  ...
  internal->uboBuffer =
      [internal->device newBufferWithLength:sizeof(UniformBufferObject)
                                    options:MTLResourceStorageModeShared];
  ...
  renderer_create_pipeline_state(renderer, "default:vertexShader",
...
void renderer_destroy(EseRenderer *renderer) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_destroy called with NULL renderer");
...
bool renderer_shader_compile(EseRenderer *renderer, const char *library_name,
                             const char *filename) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_shader_compile called with NULL renderer");
  log_assert("METAL_RENDERER", library_name,
             "renderer_shader_compile called with NULL library_name");
  log_assert("METAL_RENDERER", filename,
             "renderer_shader_compile called with NULL filename");
  ...
  bool status =
      _renderer_shader_compile_source(renderer, library_name, sourceString);
  return status;
}

bool renderer_create_pipeline_state(EseRenderer *renderer,
  ...
  id<MTLLibrary> vertexLibrary =
      grouped_hashmap_get(renderer->shaders, vLib, vFunc);
  if (!vertexLibrary) {
  ...
  id<MTLLibrary> fragmentLibrary =
      grouped_hashmap_get(renderer->shaders, fLib, fFunc);
  if (!fragmentLibrary) {
  ...
  id<MTLFunction> fragment = [fragmentLibrary
      newFunctionWithName:[NSString stringWithUTF8String:fFunc]];
  if (!vertex || !fragment) {
    NSLog(@"Failed to get vertex or fragment function: %s, %s", vertexFunc,
          fragmentFunc);
    return false;
  }
  ...
  vertexDescriptor.attributes[1].offset =
      sizeof(float) * 3; // Offset after the float3 position
  vertexDescriptor.attributes[1].bufferIndex = 0;

  // Layout for buffer 0
  vertexDescriptor.layouts[0].stride =
      sizeof(float) *
      5; // Size of a single vertex: 3 floats (pos) + 2 floats (tex coords)
...
void renderer_draw(EseRenderer *renderer) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_draw called with NULL renderer");
  ...
  MTLRenderPassDescriptor *descriptor =
      internal->view.currentRenderPassDescriptor;
  if (!descriptor)
    return;
  ...
  if (renderer->render_list &&
      render_list_get_batch_count(renderer->render_list) > 0) {
    size_t numBatches = render_list_get_batch_count(renderer->render_list);

    for (size_t i = 0; i < numBatches; ++i) {
      const EseRenderBatch *batch =
          render_list_get_batch(renderer->render_list, i);
      if (!batch)
        continue;
      ...
      size_t align = 256;
      size_t writeOffset =
          (internal->frameVertexCursor + (align - 1)) & ~(align - 1);
      ...
      void *dest =
          (char *)[internal->vertexBuffer contents] + frameBase + writeOffset;
      ...
      // prepare and set small UBO via setFragmentBytes (avoids MTLBuffer reuse
      // races)
      ...
      [encoder setFragmentTexture:(batch->type == RL_TEXTURE ? tex : nil)
                          atIndex:0];
      [encoder setFragmentBytes:&ubo
                         length:sizeof(UniformBufferObject)
                        atIndex:1];
  ...
  internal->inflightIndex =
      (internal->inflightIndex + 1) %
      (internal->inflightCount ? internal->inflightCount : 3);
}
...
bool renderer_load_texture(EseRenderer *renderer, const char *texture_id,
                           ...
                           int height) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_load_texture called with NULL renderer");
  log_assert("METAL_RENDERER", texture_id,
             "renderer_load_texture called with NULL texture_id");
  log_assert("METAL_RENDERER", rgba_data,
             "renderer_load_texture called with NULL rgba_data");
  log_assert("METAL_RENDERER", width > 0,
             "renderer_load_texture called with invalid width");
  log_assert("METAL_RENDERER", height > 0,
             "renderer_load_texture called with invalid height");
...
bool renderer_set_render_list(EseRenderer *renderer,
                              EseRenderList *render_list) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_set_render_list called with NULL renderer");
...
EseRenderList *renderer_get_render_list(EseRenderer *renderer) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_get_render_list called with NULL renderer");
...
bool renderer_clear_render_list(EseRenderer *renderer) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_clear_render_list called with NULL renderer");
...
bool renderer_get_size(EseRenderer *renderer, int *width, int *height) {
  log_assert("METAL_RENDERER", renderer,
             "renderer_get_size called with NULL renderer");
