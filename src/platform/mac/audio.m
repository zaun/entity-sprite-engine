...
bool audio_startup(void) {
    ...
    @try {
        ...
        log_error("AUDIO", "Exception during audio startup: %s", exception.reason.UTF8String);
        return false;
    }
}
...
void audio_shutdown(void) {
    ...
    @try {
        ...
        log_error("AUDIO", "Exception during audio shutdown: %s", exception.reason.UTF8String);
    }
}

bool audio_is_ready(void) { return audio_initialized && audio_engine != nil; }
...
EseSound *audio_sound_create(const uint8_t *raw) {
    ...
    @try {
        ...
        // and create an AVAudioPCMBuffer from it. For now, we'll create an
        // empty buffer.
        ...
        log_error("AUDIO", "Exception creating sound: %s", exception.reason.UTF8String);
        return NULL;
    }
}
...
void audio_sound_destroy(EseSound *sound) {
    ...
    @try {
        ...
        log_error("AUDIO", "Exception destroying sound: %s", exception.reason.UTF8String);
    }
}
...
void audio_sound_get_position(EseSound *sound, float *out_x, float *out_y) {
    ...
    EseMacSound *mac_sound = (EseMacSound *)sound;
    if (out_x)
        *out_x = mac_sound->position_x;
    if (out_y)
        *out_y = mac_sound->position_y;
}
...
void audio_sound_play(EseSound *sound) {
    ...
    @try {
        ...
        log_error("AUDIO", "Exception playing sound: %s", exception.reason.UTF8String);
    }
}
...
void audio_sound_pause(EseSound *sound) {
    ...
    @try {
        ...
        log_error("AUDIO", "Exception pausing sound: %s", exception.reason.UTF8String);
    }
}
...
void audio_sound_stop(EseSound *sound) {
    ...
    @try {
        ...
        log_error("AUDIO", "Exception stopping sound: %s", exception.reason.UTF8String);
    }
}
...
void audio_sound_fade(EseSound *sound, float target_volume, float duration) {
    ...
    log_debug("AUDIO", "Fade started: %.2f -> %.2f over %.2fs", mac_sound->fade_start_volume,
