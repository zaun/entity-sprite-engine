...
EseSound *audio_sound_create(const uint8_t *raw) {
  ...
  @try {
    ...
    EseMacSound *sound =
        (EseMacSound *)memory_manager.malloc(sizeof(EseMacSound), MMTAG_AUDIO);
    if (!sound) {
    ...
    [audio_engine connect:sound->playerNode
                       to:master_mixer
                   format:sound->format];
    ...
    // and create an AVAudioPCMBuffer from it. For now, we'll create an empty
    // buffer.
    AVAudioFrameCount frameCount = 44100; // 1 second of audio
    sound->audioBuffer =
        [[AVAudioPCMBuffer alloc] initWithPCMFormat:sound->format
