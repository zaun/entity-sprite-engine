...
EseRenderer *renderer_create(bool hiDPI) {
  ...
  EseRenderer *renderer =
      (EseRenderer *)memory_manager.malloc(sizeof(EseRenderer), MMTAG_RENDERER);
  EseGLRenderer *internal = (EseGLRenderer *)memory_manager.malloc(
      sizeof(EseGLRenderer), MMTAG_RENDERER);

  renderer->internal = (void *)internal;
  renderer->textures = hashmap_create((EseHashMapFreeFn)_gl_free_texture);
  renderer->shaders =
      grouped_hashmap_create((EseGroupedHashMapFreeFn)_gl_free_shader);
  renderer->shadersSources =
      grouped_hashmap_create((EseGroupedHashMapFreeFn)memory_manager.free);
...
void renderer_destroy(EseRenderer *renderer) {
  log_assert("GL_RENDERER", renderer,
...
bool _renderer_shader_compile_source(EseRenderer *renderer,
  ...
  log_assert("GL_RENDERER", library_name,
             "_renderer_shader_compile_source called with NULL library_name");
  log_assert("GL_RENDERER", sourceString,
  ...
  GLuint *vertexShaderId =
      memory_manager.malloc(sizeof(GLuint), MMTAG_RENDERER);
...
bool renderer_shader_compile(EseRenderer *renderer, const char *library,
  ...
  log_assert("GL_RENDERER", library,
             "renderer_shader_compile called with NULL library");
  log_assert("GL_RENDERER", filename,
...
bool renderer_create_pipeline_state(EseRenderer *renderer,
  ...
  log_assert("GL_RENDERER", vertexFunc,
             "renderer_create_pipeline_state called with NULL vertexFunc");
  log_assert("GL_RENDERER", fragmentFunc,
...
bool renderer_load_texture(EseRenderer *renderer, const char *id,
  ...
  GLTexture *tex_data =
      memory_manager.malloc(sizeof(GLTexture), MMTAG_RENDERER);
  if (tex_data) {
    tex_data->id = texture_id;
    tex_data->width = width;
    tex_data->height = height;
    hashmap_set(renderer->textures, id, tex_data);
    log_debug("RENDERER_GL", "Loaded raw texture (%s) %dx%d", id, width,
              height);
  } else {
...
bool renderer_set_render_list(EseRenderer *renderer,
  ...
  log_assert("GL_RENDERER", render_list,
...
void renderer_draw(EseRenderer *renderer) {
  ...
  if (renderer->render_list &&
    ...
    GLint useTextureLocation =
        glGetUniformLocation(internal->shaderProgram, "ubo.useTexture");
    GLint rectColorLocation =
        glGetUniformLocation(internal->shaderProgram, "ubo.rectColor");
    GLint tintLocation =
        glGetUniformLocation(internal->shaderProgram, "ubo.tint");
    GLint opacityLocation =
        glGetUniformLocation(internal->shaderProgram, "ubo.opacity");
    GLint textureLocation =
        glGetUniformLocation(internal->shaderProgram, "ourTexture");
    ...
    for (size_t i = 0; i < numBatches; ++i) {
      ...
      if (batch->scissor_active) {
        ...
        int scissor_y =
            (int)(renderer->view_h - batch->scissor_y - batch->scissor_h);
      ...
      if (batch->type == RL_TEXTURE) {
        ...
        GLTexture *tex_data = (GLTexture *)hashmap_get(
            renderer->textures, batch->shared_state.texture_id);
        if (!tex_data) {
          log_debug("GL_RENDERER", "Unable to find texture %s",
                    batch->shared_state.texture_id);
          continue;
        }
        ...
        if (data_size > internal->vbo_capacity) {
          internal->vbo_capacity =
              max(internal->vbo_capacity * 2, data_size * 2);
          glBufferData(GL_ARRAY_BUFFER, internal->vbo_capacity,
...
bool renderer_get_size(EseRenderer *renderer, int *width, int *height) {
  ...
  if (width)
    *width = renderer->view_w;
  if (height)
    *height = renderer->view_h;
