...
static bool check_alc_error(ALCdevice* device, const char* operation) {
    ...
    if (error != ALC_NO_ERROR) {
        const char* error_str = "Unknown error";
        switch (error) {
            case ALC_INVALID_DEVICE: error_str = "ALC_INVALID_DEVICE"; break;
            case ALC_INVALID_CONTEXT: error_str = "ALC_INVALID_CONTEXT"; break;
            case ALC_INVALID_ENUM: error_str = "ALC_INVALID_ENUM"; break;
            case ALC_INVALID_VALUE: error_str = "ALC_INVALID_VALUE"; break;
            case ALC_OUT_OF_MEMORY: error_str = "ALC_OUT_OF_MEMORY"; break;
        }
...
EseSound* audio_sound_create(const uint8_t *raw) {
    ...
    EseOpenALSound *sound = (EseOpenALSound *)memory_manager.malloc(sizeof(EseOpenALSound), MMTAG_AUDIO);
    if (!sound) {
    ...
    if (sound->channels == 1) {
        ...
        format = (sound->bits_per_sample == 8) ? AL_FORMAT_STEREO8 : AL_FORMAT_STEREO16;
    }
...
void audio_sound_get_position(EseSound *sound, float *out_x, float *out_y) {
    ...
    EseOpenALSound *openal_sound = (EseOpenALSound *)sound;
    if (out_x) *out_x = openal_sound->position_x;
    if (out_y) *out_y = openal_sound->position_y;
}
...
size_t audio_sound_get_length(EseSound *sound) {
    ...
    EseOpenALSound *openal_sound = (EseOpenALSound *)sound;
    return openal_sound->data_size / (openal_sound->channels * (openal_sound->bits_per_sample / 8));
}
