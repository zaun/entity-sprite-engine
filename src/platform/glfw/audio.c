...
EseSound *audio_sound_create(const uint8_t *raw) {
    ...
    EseOpenALSound *sound =
        (EseOpenALSound *)memory_manager.malloc(sizeof(EseOpenALSound), MMTAG_AUDIO);
    if (!sound) {
    ...
    alBufferData(sound->buffer_id, format, sound->audio_data, (ALsizei)sound->data_size,
                 sound->sample_rate);
...
void audio_sound_set_max_distance(EseSound *sound, float max_dist,
    ...
    ALenum distance_model =
        (attenuation == AUDIO_ATTENUATION_LINEAR) ? AL_LINEAR_DISTANCE : AL_EXPONENT_DISTANCE;
...
void audio_sound_fade(EseSound *sound, float target_volume, float duration) {
    ...
    log_debug("AUDIO", "Fade started: %.2f -> %.2f over %.2fs", openal_sound->fade_start_volume,
