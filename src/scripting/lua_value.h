 ...
 *          and all data; for strings, creates new duplicate strings; for tables,
 *          allocates new items array and recursively copies all contained items;
 ...
 * 
 * @details Allocates memory for a EseLuaValue using calloc, sets type to LUA_VAL_NIL,
 *          and optionally duplicates the provided name string.
 * 
 * @param name Optional name for the value. Can be NULL. Will be duplicated if provided.
 * 
...
/**
 * @brief Creates a new EseLuaValue structure initialized to BOOL type with given value.
 * 
 * @details Allocates memory, sets type to LUA_VAL_BOOL, stores the boolean value,
 *          and optionally duplicates the name string.
 * 
 * @param name Optional name for the value. Can be NULL. Will be duplicated if provided.
...
/**
 * @brief Creates a new EseLuaValue structure initialized to NUMBER type with given value.
 * 
 * @details Allocates memory, sets type to LUA_VAL_NUMBER, stores the numeric value
 *          as a double, and optionally duplicates the name string.
 * 
 * @param name Optional name for the value. Can be NULL. Will be duplicated if provided.
...
/**
 * @brief Creates a new EseLuaValue structure initialized to STRING type with duplicated string.
 * 
 * @details Allocates memory, sets type to LUA_VAL_STRING, creates a duplicate copy
 *          of the provided string value, and optionally duplicates the name string.
 * 
 * @param name Optional name for the value. Can be NULL. Will be duplicated if provided.
 ...
 * 
 * @details Allocates memory, sets type to LUA_VAL_TABLE, initializes the items array
 *          to NULL with zero count and capacity, and optionally duplicates the name.
 * 
 * @param name Optional name for the table. Can be NULL. Will be duplicated if provided.
 * 
 ...
 * 
 * @details Allocates memory, sets type to LUA_VAL_REF, stores the reference value,
 *          and optionally duplicates the name string.
 * 
 * @param name Optional name for the value. Can be NULL. Will be duplicated if provided.
 ...
 *          (doubling capacity), and either stores a pointer to the item directly
 *          or creates a deep copy before storing. The array automatically expands
 *          starting from capacity 4 and doubling each time.
 * 
 ...
 * @param copy If true, creates a deep copy of item; if false, takes ownership of the pointer.
 * 
 * @warning If copy is false, the caller must not free or modify the item after this call.
 * @warning The function silently fails if val is NULL, item is NULL, or val is not a table.
 * @warning Memory allocation failures during array growth will be silently ignored.
 */
 ...
 *          table items), preserves the existing name field, and sets type to NIL.
 * 
 * @param val Pointer to the EseLuaValue to reset. Safe to pass NULL.
 * 
 * @warning Any previously stored data (strings, table contents) will be freed and lost.
 */
void lua_value_set_nil(EseLuaValue *val);

/**
 * @brief Resets an existing EseLuaValue to BOOL type with new value, preserving the name.
 * 
...
/**
 * @brief Resets an existing EseLuaValue to NUMBER type with new value, preserving the name.
 * 
 * @details Frees any existing allocated memory, preserves the name field,
...
/**
 * @brief Resets an existing EseLuaValue to STRING type with new duplicated string, preserving the name.
 * 
 * @details Frees any existing allocated memory, preserves the name field,
 *          sets type to LUA_VAL_STRING, and stores a duplicate of the provided string.
 * 
 * @param val Pointer to the EseLuaValue to modify. Safe to pass NULL.
...
/**
 * @brief Resets an existing EseLuaValue to empty TABLE type, preserving the name.
 * 
 * @details Frees any existing allocated memory including all table items,
 *          preserves the name field, sets type to LUA_VAL_TABLE, and initializes
 *          an empty items array with zero count and capacity.
 * 
 * @param val Pointer to the EseLuaValue to modify. Safe to pass NULL.
 * 
 * @warning Any previously stored data including all table contents will be freed and lost.
 */
void lua_value_set_table(EseLuaValue *val);

/**
 * @brief Resets an existing EseLuaValue to REF type with new value, preserving the name.
 * 
 * @details Frees any existing allocated memory, preserves the name field,
 ...
 *          items array, compares each item's name field using strcmp, and returns
 *          a direct pointer to the first matching item.
 * 
 * @param val Pointer to a EseLuaValue of type TABLE to search.
 * @param prop_name Name of the property to find. Must not be NULL.
 * 
 * @return Pointer to the EseLuaValue property within the table if found, NULL if not found,
 *         if val is NULL, if val is not a table, or if prop_name is NULL.
 * 
 * @warning The returned pointer is to the actual item in the table's items array.
 *          Do not free it directly; it will be freed when the parent table is freed.
 * @warning Modifying the returned EseLuaValue will directly affect the table's contents.
 */
 ...
 *          all table items, frees the name field, and finally frees the EseLuaValue
 *          structure itself.
 * 
 ...
 * 
 * @details Recursively traverses the EseLuaValue structure, formats all values with
 *          proper indentation for nested tables, and outputs the result using
 *          the log_debug function with "LUA" category.
 * 
 ...
 * @warning Uses a fixed-size internal buffer; extremely large structures may be truncated.
 */
