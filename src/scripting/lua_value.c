...
/**
 * @brief Resets a EseLuaValue structure by memory_manager.freeing allocated
 * memory and clearing data.
 *
 * @details memory_manager.frees string memory for STRING types, recursively
 * memory_manager.frees all table items for TABLE types including the items
 * array itself, optionally preserves or memory_manager.frees the name field,
 * resets all counters and pointers, and sets type back to NIL for reuse.
 *
 * @param val Pointer to EseLuaValue to reset. Safe to pass NULL.
 * @param keep_name If true, preserves the name field; if false,
 * memory_manager.frees it.
 *
 * @warning Recursively memory_manager.frees table contents; all references to
 * contained items become invalid.
 * @warning After reset, only the name field (if kept) and structure itself
 * remain valid.
 */
...
EseLuaValue *lua_value_copy(const EseLuaValue *src) {
    ...
    case LUA_VAL_STRING:
        copy->value.string =
            src->value.string ? memory_manager.strdup(src->value.string, MMTAG_LUA_VALUE) : NULL;
        break;
    case LUA_VAL_TABLE:
        if (src->value.table.count > 0) {
            copy->value.table.items = memory_manager.malloc(
                src->value.table.count * sizeof(EseLuaValue *), MMTAG_LUA_VALUE);
    ...
    case LUA_VAL_COLLISION_HIT:
        ...
        break;
    case LUA_VAL_CFUNC:
        copy->value.cfunc_data.cfunc =
            src->value.cfunc_data.cfunc; // Function pointers are just copied
        copy->value.cfunc_data.upvalue =
            src->value.cfunc_data.upvalue; // Upvalue is just copied (shallow)
        break;
    default:
...
EseLuaValue *lua_value_from_stack(lua_State *L, int index) {
    ...
    case LUA_TTABLE: {
        // Initialize result as a table and fill it directly to avoid
        // shallow-copy bugs
...
EseLuaValue *lua_value_create_nil(const char *name) {
    ...
    v->type = LUA_VAL_NIL;
    if (name)
        v->name = memory_manager.strdup(name, MMTAG_LUA_VALUE);
    return v;
}
...
void lua_value_push(EseLuaValue *val, EseLuaValue *item, bool copy) {
    ...
    if (val->value.table.count >= val->value.table.capacity) {
        ...
        EseLuaValue **new_items =
            memory_manager.realloc(val->value.table.items, new_capacity * sizeof(EseLuaValue *),
                                   MMTAG_LUA_VALUE); // ← Array of pointers
        if (!new_items)
            return;
...
EseLuaValue *lua_value_get_table_prop(EseLuaValue *val, const char *prop_name) {
    ...
    if (val->type != LUA_VAL_TABLE || !prop_name)
        return NULL;
...
void lua_value_set_table_prop(EseLuaValue *val, EseLuaValue *prop_value) {
    ...
    log_assert("LUA_VALUE", val->type == LUA_VAL_TABLE,
               "lua_value_set_table_prop called on non-table value");
    log_assert("LUA_VALUE", prop_value->name,
               "lua_value_set_table_prop called with prop_value that has no name");
    ...
    lua_value_push(val, new_item,
                   false); // Don't copy since we already copied it
}
...
bool lua_value_is_nil(EseLuaValue *val) { return val && val->type == LUA_VAL_NIL; }

bool lua_value_is_bool(EseLuaValue *val) { return val && val->type == LUA_VAL_BOOL; }

bool lua_value_is_number(EseLuaValue *val) { return val && val->type == LUA_VAL_NUMBER; }

bool lua_value_is_string(EseLuaValue *val) { return val && val->type == LUA_VAL_STRING; }

bool lua_value_is_table(EseLuaValue *val) { return val && val->type == LUA_VAL_TABLE; }

bool lua_value_is_ref(EseLuaValue *val) { return val && val->type == LUA_VAL_REF; }

bool lua_value_is_userdata(EseLuaValue *val) { return val && val->type == LUA_VAL_USERDATA; }

bool lua_value_is_rect(EseLuaValue *val) { return val && val->type == LUA_VAL_RECT; }

bool lua_value_is_point(EseLuaValue *val) { return val && val->type == LUA_VAL_POINT; }

bool lua_value_is_map(EseLuaValue *val) { return val && val->type == LUA_VAL_MAP; }

bool lua_value_is_arc(EseLuaValue *val) { return val && val->type == LUA_VAL_ARC; }

bool lua_value_is_color(EseLuaValue *val) { return val && val->type == LUA_VAL_COLOR; }

bool lua_value_is_display(EseLuaValue *val) { return val && val->type == LUA_VAL_DISPLAY; }

bool lua_value_is_input_state(EseLuaValue *val) { return val && val->type == LUA_VAL_INPUT_STATE; }

bool lua_value_is_map_cell(EseLuaValue *val) { return val && val->type == LUA_VAL_MAP_CELL; }

bool lua_value_is_poly_line(EseLuaValue *val) { return val && val->type == LUA_VAL_POLY_LINE; }

bool lua_value_is_ray(EseLuaValue *val) { return val && val->type == LUA_VAL_RAY; }

bool lua_value_is_tileset(EseLuaValue *val) { return val && val->type == LUA_VAL_TILESET; }

bool lua_value_is_uuid(EseLuaValue *val) { return val && val->type == LUA_VAL_UUID; }

bool lua_value_is_vector(EseLuaValue *val) { return val && val->type == LUA_VAL_VECTOR; }

bool lua_value_is_collision_hit(EseLuaValue *val) {
    return val && val->type == LUA_VAL_COLLISION_HIT;
}

bool lua_value_is_cfunc(EseLuaValue *val) { return val && val->type == LUA_VAL_CFUNC; }

bool lua_value_is_error(EseLuaValue *val) { return val && val->type == LUA_VAL_ERROR; }
...
static void _log_luavalue_rec(EseLuaValue *val, char *buf, size_t buflen, size_t *offset,
                              int indent) {
    if (!val || !buf || !offset)
        return;
    ...
    case LUA_VAL_INPUT_STATE:
        *offset +=
            snprintf(buf + *offset, buflen - *offset, "InputState: %p\n", val->value.input_state);
        break;
    case LUA_VAL_MAP_CELL:
    ...
    case LUA_VAL_POLY_LINE:
        *offset +=
            snprintf(buf + *offset, buflen - *offset, "PolyLine: %p\n", val->value.poly_line);
        break;
    case LUA_VAL_RAY:
