...
static void _lua_value_reset(EseLuaValue *val, bool keep_name) {
    ...
    if (val->type == LUA_VAL_CAMERA) {
        val->value.camera = NULL;
    }
...
EseLuaValue* lua_value_copy(const EseLuaValue *src) {
    ...
    switch (src->type) {
        ...
        case LUA_VAL_ARC:
            ...
            break;
        case LUA_VAL_CAMERA:
            copy->value.camera = ese_camera_copy(src->value.camera);
            break;
        case LUA_VAL_COLOR:
...
EseLuaValue *lua_value_create_camera(const char *name, struct EseCamera* camera) {
    log_assert("LUA_VALUE", name, "lua_value_create_camera called with NULL name");

    EseLuaValue *v = memory_manager.calloc(1, sizeof(EseLuaValue), MMTAG_LUA_VALUE);
    v->type = LUA_VAL_CAMERA;
    v->value.camera = camera;
    if (name) v->name = memory_manager.strdup(name, MMTAG_LUA_VALUE);
    return v;
}
...
EseCamera* lua_value_get_camera(EseLuaValue *val) {
    log_assert("LUA_VALUE", val, "lua_value_get_camera called with NULL val");

    return val->value.camera;
}
...
bool lua_value_is_camera(EseLuaValue *val) {
    return val && val->type == LUA_VAL_CAMERA;
}
    ...
    switch (val->type) {
        ...
        case LUA_VAL_ARC:
            *offset += snprintf(buf + *offset, buflen - *offset, "Arc: %p\n", val->value.arc);
            break;
        case LUA_VAL_CAMERA:
            *offset += snprintf(buf + *offset, buflen - *offset, "Camera: %p\n", val->value.camera);
            break;
        case LUA_VAL_COLOR:
