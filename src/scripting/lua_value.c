...
static void _lua_value_reset(EseLuaValue *val, bool keep_name) {
  ...
  if (val->type == LUA_VAL_STRING && val->value.string) {
    ...
    profile_stop(PROFILE_LUA_VALUE_RESET_SECTION,
                 "lua_value_reset_string_free");
    val->value.string = NULL;
  }
...
EseLuaValue *lua_value_copy(const EseLuaValue *src) {
  ...
  EseLuaValue *copy =
      memory_manager.calloc(1, sizeof(EseLuaValue), MMTAG_LUA_VALUE);
  copy->type = src->type;
  copy->name =
      src->name ? memory_manager.strdup(src->name, MMTAG_LUA_VALUE) : NULL;
  ...
  case LUA_VAL_STRING:
    copy->value.string =
        src->value.string
            ? memory_manager.strdup(src->value.string, MMTAG_LUA_VALUE)
            : NULL;
    break;
  case LUA_VAL_TABLE:
    if (src->value.table.count > 0) {
      ...
      copy->value.table.capacity =
          src->value.table.count; // Initialize capacity

      for (size_t i = 0; i < src->value.table.count; i++) {
        if (src->value.table.items[i]) {
          copy->value.table.items[i] =
              lua_value_copy(src->value.table.items[i]);
  ...
  case LUA_VAL_COLLISION_HIT:
    copy->value.collision_hit =
        ese_collision_hit_copy(src->value.collision_hit);
    break;
  case LUA_VAL_CFUNC:
...
EseLuaValue *lua_value_from_stack(lua_State *L, int index) {
  ...
  case LUA_TTABLE: {
    // Initialize result as a table and fill it directly to avoid shallow-copy
    // bugs
...
EseLuaValue *lua_value_create_number(const char *name, double value) {
  log_assert("LUA_VALUE", name,
             "lua_value_create_number called with NULL name");
...
EseLuaValue *lua_value_create_string(const char *name, const char *value) {
  log_assert("LUA_VALUE", name,
             "lua_value_create_string called with NULL name");
...
EseLuaValue *lua_value_create_display(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_display called with NULL name");
...
EseLuaValue *lua_value_create_input_state(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_input_state called with NULL name");
...
EseLuaValue *lua_value_create_map_cell(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_map_cell called with NULL name");
...
EseLuaValue *lua_value_create_poly_line(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_poly_line called with NULL name");
...
EseLuaValue *lua_value_create_tileset(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_tileset called with NULL name");
...
EseLuaValue *lua_value_create_vector(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_vector called with NULL name");
...
EseLuaValue *lua_value_create_collision_hit(const char *name,
  ...
  log_assert("LUA_VALUE", name,
             "lua_value_create_collision_hit called with NULL name");
...
EseLuaValue *lua_value_create_cfunc(const char *name, EseLuaCFunction cfunc,
  ...
  log_assert("LUA_VALUE", cfunc,
             "lua_value_create_cfunc called with NULL cfunc");
...
void lua_value_push(EseLuaValue *val, EseLuaValue *item, bool copy) {
  ...
  if (val->value.table.count >= val->value.table.capacity) {
    size_t new_capacity =
        val->value.table.capacity == 0 ? 4 : val->value.table.capacity * 2;
    EseLuaValue **new_items = memory_manager.realloc(
        val->value.table.items, new_capacity * sizeof(EseLuaValue *),
  ...
  if (copy) {
    ...
    val->value.table.items[val->value.table.count] =
        item; // ← Store the POINTER
  }
...
void lua_value_set_collision_hit(EseLuaValue *val,
                                 struct EseCollisionHit *hit) {
  log_assert("LUA_VALUE", val,
             "lua_value_set_collision_hit called with NULL val");
...
void lua_value_set_table_prop(EseLuaValue *val, EseLuaValue *prop_value) {
  ...
  log_assert("LUA_VALUE", prop_value,
             "lua_value_set_table_prop called with NULL prop_value");
  log_assert("LUA_VALUE", val->type == LUA_VAL_TABLE,
             "lua_value_set_table_prop called on non-table value");
  log_assert(
      "LUA_VALUE", prop_value->name,
...
EseInputState *lua_value_get_input_state(EseLuaValue *val) {
  log_assert("LUA_VALUE", val,
             "lua_value_get_input_state called with NULL val");
...
EseCollisionHit *lua_value_get_collision_hit(EseLuaValue *val) {
  log_assert("LUA_VALUE", val,
             "lua_value_get_collision_hit called with NULL val");
...
EseLuaValue *lua_value_get_cfunc_upvalue(EseLuaValue *val) {
  log_assert("LUA_VALUE", val,
             "lua_value_get_cfunc_upvalue called with NULL val");
...
static void _log_luavalue_rec(EseLuaValue *val, char *buf, size_t buflen,
  ...
  case LUA_VAL_MAP:
    *offset +=
        snprintf(buf + *offset, buflen - *offset, "Map: %p\n", val->value.map);
    break;
  case LUA_VAL_ARC:
    *offset +=
        snprintf(buf + *offset, buflen - *offset, "Arc: %p\n", val->value.arc);
    break;
  case LUA_VAL_COLOR:
  ...
  case LUA_VAL_RAY:
    *offset +=
        snprintf(buf + *offset, buflen - *offset, "Ray: %p\n", val->value.ray);
    break;
  case LUA_VAL_TILESET:
  ...
  case LUA_VAL_CFUNC:
    *offset +=
        snprintf(buf + *offset, buflen - *offset, "CFunc: %p (upvalue: %p)\n",
  ...
  case LUA_VAL_TABLE: {
    *offset += snprintf(buf + *offset, buflen - *offset, "Table:\n");
    for (size_t i = 0; i < val->value.table.count; ++i) {
      _log_luavalue_rec(val->value.table.items[i], buf, buflen, offset,
                        indent + 2);
    }
    break;
  }
