...
_Static_assert(sizeof(LuaAllocHdr) == 16,
               "LuaAllocHdr must be 16 bytes for alignment");
...
int _lua_global_write_error(lua_State *L) {
  ...
  log_debug("LUA_ENGINE",
            "Global variable writes are not allowed (attempted to set '%s')",
            key ? key : "?");
  return 0;
}
...
void *_lua_engine_limited_alloc(void *ud, void *ptr, size_t osize,
                                size_t nsize) {
  ...
  if (ptr == NULL) {
    ...
    LuaAllocHdr *hdr =
        (LuaAllocHdr *)memory_manager.malloc(total_size, MMTAG_LUA);
    if (!hdr) {
      profile_cancel(PROFILE_LUA_ENGINE_ALLOC);
  ...
  if (internal->memory_used + grow > internal->memory_limit) {
    log_error("LUA_ALLOC", "realloc limit exceeded: %zu + %zu > %zu",
              internal->memory_used, grow, internal->memory_limit);
    profile_cancel(PROFILE_LUA_ENGINE_ALLOC);
...
bool _lua_engine_instance_get_function(lua_State *L, int instance_ref,
  ...
  if (!lua_istable(L, -1)) {
    log_debug("LUA", "Instance is not a table (type: %s)",
  ...
  if (lua_isfunction(L, -1)) {
    ...
    log_debug("LUA", "Function value type: %s",
              lua_typename(L, lua_type(L, -1)));
  ...
  if (lua_isstring(L, -1)) {
    ...
    log_debug("LUA", "Found boolean value: %s",
              lua_toboolean(L, -1) ? "true" : "false");
  } else if (lua_isnil(L, -1)) {
    log_debug("LUA", "Found nil value");
  } else {
    log_debug("LUA", "Found other value type: %s",
              lua_typename(L, lua_type(L, -1)));
  }
  ...
  log_debug("LUA", "Function '%s' not found in instance or metatable",
            func_name);
  profile_cancel(PROFILE_LUA_ENGINE_FUNCTION_LOOKUP);
...
void _lua_engine_push_luavalue(lua_State *L, EseLuaValue *arg) {
  ...
  if (arg->type == LUA_VAL_TABLE || arg->type == LUA_VAL_REF) {
    profile_stop(PROFILE_LUA_ENGINE_ARG_CONVERSION,
                 "lua_eng_push_luavalue_complex_arg");
    profile_count_add("lua_eng_push_luavalue_complex_arg");
  } else {
    profile_stop(PROFILE_LUA_ENGINE_ARG_CONVERSION,
                 "lua_eng_push_luavalue_simple_arg");
  }
}
