...
/**
 * @brief Creates and initializes a new EseLuaEngine instance with security
 * restrictions.
 *
 * @details Allocates a new EseLuaEngine, creates a Lua state with custom memory
 * allocator, loads standard libraries (base, table, string, math), removes
 * dangerous functions (dofile, loadfile, require), replaces print with custom
 * logging version, and sets memory/execution limits. The engine enforces a 10MB
 *          memory limit, 10-second execution timeout, and 100,000 instruction
 * limit.
 *
 * @return Pointer to newly created EseLuaEngine instance on success, NULL on
 * failure.
 *
 * @warning The returned engine must be freed with lua_engine_destroy() to
 ...
 *
 * @details Iterates through all loaded script references, unreferences them
 * from the Lua registry, frees the EseHashMap containing function references,
 *          closes the Lua state, and frees the engine structure.
 *
 * @param engine Pointer to the EseLuaEngine instance to free. Safe to pass
 * NULL.
 */
 ...
 *          reads the entire file into memory, compiles it using
 * luaL_loadstring, executes it expecting a table return value, and stores a
 * registry reference in the engine's EseHashMap. The script is expected to
 * return a class table that can be instantiated later.
 *
 * @param engine Pointer to the EseLuaEngine instance.
 * @param filename Name of the Lua script file to load (must have .lua
 * extension).
 * @param module_name Name of the module to load).
 *
 * @return true if the script was successfully loaded and compiled, false on any
 * error.
 *
 * @warning Script files are read entirely into memory. Large files may cause
 * issues.
 ...
 * function registry.
 *
 * @details Validates the script string is not empty, compiles it using
 * luaL_loadstring, executes it expecting a table return value, and stores a
 * registry reference in the engine's EseHashMap. The script is expected to
 * return a class table that can be instantiated later.
 *
 ...
 *
 * @details Retrieves the script's class table from registry, creates a real
 * instance table with metatable pointing to the class (__index = class),
 * creates a readonly_keys tracking table, wraps the real instance in a proxy
 * table with custom __index/__newindex metamethods for read-only property
 * support, and returns a registry reference to the proxy.
 *
 * @param engine Pointer to the EseLuaEngine instance.
 * @param filename Name of the previously loaded Lua script to instantiate.
 *
 * @return Registry reference ID (positive integer) to the new proxy instance on
 * success, -1 on failure.
 *
 * @warning The returned reference must be removed with
 * lua_engine_instance_remove() to prevent registry leaks.
 * @warning The script must have been loaded first with
 * lua_engine_load_script().
 */
 ...
 *
 * @details Uses luaL_unref to remove the instance reference from the Lua
 * registry, allowing the proxy table and all associated data to be garbage
 * collected.
 *
 ...
 *
 * @warning Using the instance_ref after calling this function will result in
 * undefined behavior.
 */
 ...
 *
 * @details Executes a Lua function using its registry reference. This is faster
 * than the old instance_run_function functions as it avoids repeated function
 * lookups. Hook setup is still performed for security.
 *
 * @param engine Pointer to the EseLuaEngine instance.
 ...
 * @param out_result Pointer to EseLuaValue to store the function result, or
 * NULL if no result needed.
 *
 * @return true if the function executed successfully, false on error or
 * timeout.
 *
 ...
 *
 * @details Executes a Lua function by looking it up by name in the script
 * instance. This is slower than lua_engine_run_function_ref as it requires
 * function lookup, but more convenient when you don't have a cached function
 * reference.
 *
 * @param engine Pointer to the EseLuaEngine instance.
...
/**
 * @brief Creates a new Lua metatable for an object type with standard
 * metamethods.
 *
 * @details Checks if the metatable already exists and only creates it if it
 * doesn't. Sets up standard metamethods: __name, __index, __newindex, __gc, and
 * __tostring. Also sets __metatable to "locked" to prevent tampering.
 *
 * @param engine Pointer to the EseLuaEngine instance.
 ...
 *
 * @details Checks if the global table already exists and only creates it if it
 * doesn't. Creates a new table and populates it with the provided key-function
 * pairs.
 *
 * @param engine Pointer to the EseLuaEngine instance.
