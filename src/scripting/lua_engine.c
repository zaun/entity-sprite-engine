...
void lua_engine_add_function(EseLuaEngine *engine, const char *function_name, lua_CFunction func) {
    ...
    log_assert("LUA_ENGINE", engine->internal->sandbox_master_ref != LUA_NOREF,
               "lua_eng_add_function engine->internal->sandbox_master_ref is "
               "LUA_NOREF");
...
bool lua_engine_load_script(EseLuaEngine *engine, const char *filename, const char *module_name) {
    ...
    log_assert("LUA_ENGINE", engine->internal->sandbox_master_ref != LUA_NOREF,
               "lua_eng_load_script engine->internal->sandbox_master_ref is "
               "LUA_NOREF");
...
bool lua_engine_load_script_from_string(EseLuaEngine *engine, const char *script, const char *name,
    ...
    size_t new_len = strlen(prologue_fmt) + strlen(module_name) + strlen(processed_script) +
...
bool lua_engine_run_function_ref(EseLuaEngine *engine, int function_ref, int self_ref, int argc,
    ...
    if (lua_pcall(L, n_args, n_results, 0) != LUA_OK) {
        ...
        // Log the error first
        log_error("LUA_ENGINE", "(lua_engine_run_function_ref) Error running function: %s",
                  error_message);
...
bool lua_engine_run_function(EseLuaEngine *engine, int instance_ref, int self_ref,
    ...
    if (initial_stack_size != 0) {
        log_warn("LUA_ENGINE", "Stack not clean! Found %d items on stack before function call",
                 initial_stack_size);
    ...
    if (cached_func_ref == -1) {
        ...
        if (!_lua_engine_instance_get_function(L, instance_ref, func_name)) {
            log_verbose("LUA_ENGINE", "Function '%s' not found in instance %d", func_name,
                        instance_ref);
        ...
        log_verbose("LUA_ENGINE", "Cached function '%s' for future calls (ref: %d)", func_name,
                    cached_func_ref);
        ...
        if (lua_isfunction(L, -1)) {
            ...
            // The function is now at the top of the stack and will be used for
            // the call
        } else {
        ...
        if (lua_isfunction(L, -1)) {
            ...
            log_error("LUA_ENGINE", "Failed to retrieve cached function! Got type: %s",
                      lua_typename(L, lua_type(L, -1)));
    ...
    log_verbose("LUA_ENGINE", "About to call lua_pcall with %d args, expecting %d results", n_args,
                n_results);
    ...
    if (cached_func_ref != -1 && argc == 0 && !out_result) {
        ...
        // CRITICAL FIX: Use the n_args variable calculated during argument
        // setup n_args already includes both self and actual arguments, which
        // is what lua_pcall expects
        ...
        if (pcall_result != LUA_OK) {
            ...
            // The stack should be back to initial_stack_size after popping
            // return values
    ...
    if (pcall_result != LUA_OK) {
        ...
        // Log the error first
        log_error("LUA_ENGINE",
                  "(lua_engine_run_function) Fast path Error running function "
                  "'%s': %s",
                  func_name, error_message);
        ...
        if (return_values > 0) {
            ...
            log_verbose("LUA_ENGINE", "Stack corruption detected! Expected %d, got %d",
                        expected_stack_size, current_stack_size);
        }
    }
    ...
    log_verbose("LUA_ENGINE", "Stats: call count = %d  instruction count = %zu", timeout.call_count,
                timeout.call_count);
