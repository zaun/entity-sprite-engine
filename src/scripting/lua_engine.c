...
EseLuaEngine *lua_engine_create() {
  ...
  engine->internal =
      memory_manager.malloc(sizeof(EseLuaEngineInternal), MMTAG_LUA);

  engine->internal->memory_limit = 1024 * 1024 * 10; // 10MB
  engine->internal->memory_used = 0;

  // Set default limits
  engine->internal->max_execution_time =
      (clock_t)((10) * CLOCKS_PER_SEC); // 10 second timeout protection
  engine->internal->max_instruction_count =
      4000000; // 4M instruction limit for security
  ...
  engine->internal->functions =
      hashmap_create((EseHashMapFreeFn)memory_manager.free);
  ...
  if (lua_istable(engine->runtime, -1)) {
    ...
    if (lua_isfunction(engine->runtime, -1)) {
      ...
      if (call_result == LUA_OK) {
        ...
        log_error("LUA_ENGINE", "Failed to enable JIT: %s",
  ...
  if (lua_istable(engine->runtime, -1)) {
    ...
    if (lua_isstring(engine->runtime, -1)) {
      log_debug("LUA_ENGINE", "JIT Version: %s",
                lua_tostring(engine->runtime, -1));
    }
    lua_pop(engine->runtime, 1);
    ...
    if (lua_isstring(engine->runtime, -1)) {
      log_debug("LUA_ENGINE", "JIT Arch: %s",
                lua_tostring(engine->runtime, -1));
    }
    lua_pop(engine->runtime, 1);
  ...
  lua_settable(engine->runtime,
               LUA_REGISTRYINDEX); // registry[ENTITY_LIST_KEY] = table
  ...
  engine->internal->sandbox_master_ref =
      luaL_ref(engine->runtime, LUA_REGISTRYINDEX);
...
void lua_engine_destroy(EseLuaEngine *engine) {
  ...
  if (engine->internal->sandbox_master_ref != LUA_NOREF) {
    luaL_unref(engine->runtime, LUA_REGISTRYINDEX,
               engine->internal->sandbox_master_ref);
  }
...
void lua_engine_global_lock(EseLuaEngine *engine) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_global_lock called with NULL engine");
...
void lua_engine_add_function(EseLuaEngine *engine, const char *function_name,
                             lua_CFunction func) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_add_function called with NULL engine");
  log_assert("LUA_ENGINE", function_name,
             "lua_eng_add_function called with NULL function_name");
  log_assert("LUA_ENGINE", func, "lua_eng_add_function called with NULL func");
  log_assert(
      "LUA_ENGINE", engine->internal->sandbox_master_ref != LUA_NOREF,
      "lua_eng_add_function engine->internal->sandbox_master_ref is LUA_NOREF");

  // Add to script_sandbox_master
  lua_rawgeti(engine->runtime, LUA_REGISTRYINDEX,
              engine->internal->sandbox_master_ref); // master
...
void lua_engine_add_global(EseLuaEngine *engine, const char *global_name,
                           int lua_ref) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_add_global called with NULL engine");
  log_assert("LUA_ENGINE", global_name,
             "lua_eng_add_global called with NULL global_name");
  log_assert("LUA_ENGINE", lua_ref != LUA_NOREF,
             "lua_eng_add_global called with LUA_NOREF lua_ref");
  log_assert(
      "LUA_ENGINE", engine->internal->sandbox_master_ref != LUA_NOREF,
      "lua_eng_add_global engine->internal->sandbox_master_ref is LUA_NOREF");
...
bool lua_engine_load_script(EseLuaEngine *engine, const char *filename,
  ...
  log_assert("LUA_ENGINE", engine,
             "lua_eng_load_script called with NULL engine");
  log_assert("LUA_ENGINE", filename,
             "lua_eng_load_script called with NULL filename");
  log_assert("LUA_ENGINE", module_name,
             "lua_eng_load_script called with NULL module_name");
  log_assert(
      "LUA_ENGINE", engine->internal->sandbox_master_ref != LUA_NOREF,
      "lua_eng_load_script engine->internal->sandbox_master_ref is LUA_NOREF");
  ...
  if (!full_path) {
    log_error("LUA_ENGINE", "Error: filesystem_get_resource failed for %s",
              filename);
    profile_cancel(PROFILE_LUA_ENGINE_LOAD_SCRIPT);
    profile_count_add("lua_eng_load_script_failed");
    return false;
  ...
  if (!file) {
    log_error("LUA_ENGINE", "Error: Failed to open Lua script file '%s'",
              full_path);
  ...
  if (file_size < 0) {
    log_error("LUA_ENGINE", "Error: Failed to get file size for '%s'",
              full_path);
    memory_manager.free(full_path);
    fclose(file);
    profile_cancel(PROFILE_LUA_ENGINE_LOAD_SCRIPT);
  ...
  if (bytes_read != (size_t)file_size) {
    log_error("LUA_ENGINE",
              "Error: Failed to read complete Lua script from '%s'", full_path);
  ...
  bool status =
      lua_engine_load_script_from_string(engine, script, filename, module_name);
  memory_manager.free(script);
  memory_manager.free(full_path);
...
bool lua_engine_load_script_from_string(EseLuaEngine *engine,
                                        ...
                                        const char *module_name) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_load_script_frm_str called with NULL engine");
  log_assert("LUA_ENGINE", script,
             "lua_eng_load_script_frm_str called with NULL script");
  log_assert("LUA_ENGINE", name,
             "lua_eng_load_script_frm_str called with NULL name");
  log_assert("LUA_ENGINE", module_name,
             "lua_eng_load_script_frm_str called with NULL module_name");

  profile_start(PROFILE_LUA_ENGINE_LOAD_SCRIPT_STRING);

  // Build environment
  _lua_engine_build_env_from_master(engine->runtime,
                                    engine->internal->sandbox_master_ref);
  ...
  size_t new_len = strlen(prologue_fmt) + strlen(module_name) +
                   strlen(processed_script) + strlen(epilogue_fmt) +
  ...
  if (luaL_loadbuffer(engine->runtime, wrapped, strlen(wrapped), chunkname) ==
      LUA_OK) {
    ...
    if (lua_pcall(engine->runtime, 1, 1, 0) == LUA_OK) {
      if (lua_istable(engine->runtime, -1)) {
        ...
        *ref = script_ref;
        ...
        profile_stop(PROFILE_LUA_ENGINE_LOAD_SCRIPT_STRING,
                     "lua_eng_load_script_string");
      ...
      log_error("LUA_ENGINE", "Error executing script '%s': %s", name,
                lua_tostring(engine->runtime, -1));
      lua_pop(engine->runtime, 1);
    }
  } else {
    log_error("LUA_ENGINE", "Error loading script '%s': %s", name,
              lua_tostring(engine->runtime, -1));
    lua_pop(engine->runtime, 1);
  }
...
int lua_engine_instance_script(EseLuaEngine *engine, const char *name) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_inst_script called with NULL engine");
...
void lua_engine_instance_remove(EseLuaEngine *engine, int instance_ref) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_inst_remove called with NULL engine");
 ...
 */
static void _lua_engine_convert_stack_to_luavalue(lua_State *L, int idx,
...
bool lua_engine_run_function_ref(EseLuaEngine *engine, int function_ref,
                                 ...
                                 EseLuaValue *out_result) {
  log_assert("LUA_ENGINE", engine,
             "lua_eng_run_func_ref called with NULL engine");
  ...
  profile_stop(PROFILE_LUA_ENGINE_FUNCTION_LOOKUP,
               "lua_eng_run_func_ref_entity_proxy");
  ...
  profile_stop(PROFILE_LUA_ENGINE_ARG_CONVERSION,
               "lua_eng_run_func_ref_arg_conversion");
  ...
  profile_stop(PROFILE_LUA_ENGINE_HOOK_SETUP,
               "lua_eng_run_func_ref_hook_setup");

  // Lua execution timing
  profile_start(PROFILE_LUA_ENGINE_LUA_EXECUTION);
  bool ok = true;
  int n_results =
      out_result ? 1 : 0; // Expect 1 result if out_result is provided
  if (lua_pcall(L, n_args, n_results, 0) != LUA_OK) {
    ...
    // Log the error first
    log_error("LUA_ENGINE",
              "(lua_engine_run_function_ref) Error running function: %s",
              error_message);
    ...
    profile_stop(PROFILE_LUA_ENGINE_RESULT_CONVERSION,
                 "lua_eng_run_func_ref_result_conversion");
    lua_pop(L, 1); // pop the result
  }
  profile_stop(PROFILE_LUA_ENGINE_LUA_EXECUTION,
               "lua_eng_run_func_ref_execution");

  // Hook cleanup timing - DISABLED FOR TESTING
  profile_start(PROFILE_LUA_ENGINE_HOOK_CLEANUP);
  lua_sethook(L, NULL, 0, 0);
  profile_stop(PROFILE_LUA_ENGINE_HOOK_CLEANUP,
               "lua_eng_run_func_ref_hook_cleanup");
...
bool lua_engine_run_function(EseLuaEngine *engine, int instance_ref,
  ...
  log_assert("LUA_ENGINE", engine,
             "lua_eng_run_function called with NULL engine");
  log_assert("LUA_ENGINE", func_name,
             "lua_eng_run_function called with NULL func_name");

  profile_start(PROFILE_LUA_ENGINE_RUN_FUNCTION);

  lua_State *L = engine->runtime;

  // OPTIMIZATION: Function caching - check if we have a cached function
  // reference
  int cached_func_ref = -1;
  char cache_key[256];
  snprintf(cache_key, sizeof(cache_key), "func_cache_%d_%s", instance_ref,
           func_name);
  ...
  log_verbose("LUA_ENGINE", "Stack size before function call: %d",
              initial_stack_size);

  // CRITICAL FIX: Ensure stack is clean before starting
  if (initial_stack_size != 0) {
    log_warn("LUA_ENGINE",
             "Stack not clean! Found %d items on stack before function call",
             initial_stack_size);
    log_verbose("LUA_ENGINE", "Cleaning up stack items:");
    for (int i = 1; i <= initial_stack_size; i++) {
      log_verbose("LUA_ENGINE", "  Removing Stack[%d]: %s", i,
                  lua_typename(L, lua_type(L, i)));
    }
    lua_settop(L, 0); // Clean the stack
    initial_stack_size = 0;
    log_verbose("LUA_ENGINE", "Stack cleaned. New stack size: %d",
                lua_gettop(L));
  }
  ...
  if (cached_func_ref == -1) {
    ...
    if (!_lua_engine_instance_get_function(L, instance_ref, func_name)) {
      log_verbose("LUA_ENGINE", "Function '%s' not found in instance %d",
    ...
    log_verbose("LUA_ENGINE", "Cached function '%s' for future calls (ref: %d)",
                func_name, cached_func_ref);
    ...
    if (lua_isfunction(L, -1)) {
      ...
      // The function is now at the top of the stack and will be used for the
      // call
    } else {
      log_error("LUA_ENGINE", "Cached value is not a function! Type: %s",
                lua_typename(L, lua_type(L, -1)));
      lua_pop(L, 1); // Only pop if verification failed
    }
  } else {
    // Use cached function
    log_verbose("LUA_ENGINE", "Retrieving cached function (ref: %d)",
                cached_func_ref);
    lua_rawgeti(L, LUA_REGISTRYINDEX, cached_func_ref);
    ...
    if (lua_isfunction(L, -1)) {
      ...
      log_error("LUA_ENGINE",
                "Failed to retrieve cached function! Got type: %s",
                lua_typename(L, lua_type(L, -1)));
  ...
  for (int i = 1; i <= lua_gettop(L); i++) {
    log_verbose("LUA_ENGINE", "  Stack[%d]: %s", i,
                lua_typename(L, lua_type(L, i)));
  }
  ...
  log_verbose("LUA_ENGINE",
              "Stack after reordering (function should be at Stack[1]):");
  for (int i = 1; i <= lua_gettop(L); i++) {
    log_verbose("LUA_ENGINE", "  Stack[%d]: %s", i,
                lua_typename(L, lua_type(L, i)));
  }
  ...
  log_verbose("LUA_ENGINE", "Stack before pcall: %d (function + %d args)",
              lua_gettop(L), n_args);
  ...
  log_verbose("LUA_ENGINE",
              "About to call lua_pcall with %d args, expecting %d results",
              n_args, n_results);
  ...
  if (cached_func_ref != -1 && argc == 0 && !out_result) {
    ...
    log_verbose("LUA_ENGINE",
                "Fast path stack (function already at Stack[1]):");
    for (int i = 1; i <= lua_gettop(L); i++) {
      log_verbose("LUA_ENGINE", "  Stack[%d]: %s", i,
                  lua_typename(L, lua_type(L, i)));
    }
    ...
    // We have: Stack[1] = function, Stack[2] = self, Stack[3+] = additional
    // args
    int fast_path_args = lua_gettop(L) - 1;
    log_verbose("LUA_ENGINE", "Fast path calling with %d arguments",
                fast_path_args);
    ...
    if (pcall_result != LUA_OK) {
      ...
      if (lua_islightuserdata(L, -1)) {
        ...
        if (engine) {
          engine_add_to_console(engine, ESE_CONSOLE_ERROR, "LUA",
                                error_message);
          engine_show_console(engine, true);
        }
      } else {
    ...
    // n_args already includes both self and actual arguments, which is what
    // lua_pcall expects
    ...
    if (pcall_result != LUA_OK) {
      ...
      // Log the error first
      log_error("LUA_ENGINE",
                "(lua_engine_run_function) Standard path, Error running "
                "function '%s': %s",
      ...
      // The stack should be back to initial_stack_size after popping return
      // values
      ...
      log_verbose("LUA_ENGINE",
                  "Stack cleanup: current=%d, expected=%d, return_values=%d",
                  current_stack_size, expected_stack_size, return_values);

      if (return_values > 0) {
        log_verbose("LUA_ENGINE", "Function returned %d values, popping them",
                    return_values);
        lua_pop(L, return_values);

        // Verify stack is now clean
        int final_stack_size = lua_gettop(L);
        log_verbose("LUA_ENGINE", "After popping return values, stack size: %d",
                    final_stack_size);
      } else if (return_values < 0) {
        log_warn("LUA_ENGINE", "Stack corruption detected! Expected %d, got %d",
                 expected_stack_size, current_stack_size);
      }
    }
  }
  ...
  if (pcall_result != LUA_OK) {
    ...
    log_error(
        "LUA_ENGINE",
        "(lua_engine_run_function) Fast path Error running function '%s': %s",
        func_name, error_message);
    ...
    if (return_values > 0) {
      ...
      log_verbose("LUA_ENGINE",
                  "Stack corruption detected! Expected %d, got %d",
                  expected_stack_size, current_stack_size);
    }
  }
  ...
  profile_stop(PROFILE_LUA_ENGINE_HOOK_CLEANUP,
               "lua_eng_run_func_hook_cleanup");

  log_verbose("LUA_ENGINE", "Stats: call count = %d  instruction count = %zu",
              timeout.call_count, timeout.call_count);

  // Ensure stack is clean after function call
  int final_stack_size = lua_gettop(L);
  log_verbose("LUA_ENGINE", "Stack size after function call: %d",
              final_stack_size);

  if (final_stack_size != 0) {
    log_warn("LUA_ENGINE",
             "Stack not clean after function call! Cleaning up...");
    lua_settop(L, 0);
  }
  ...
  if (cleaned_stack_size != 0) {
    log_error("LUA_ENGINE", "Failed to clean stack! Final size: %d",
              cleaned_stack_size);
  }
...
void *lua_getextraspace_lj(lua_State *L) {
  log_assert("LUA_ENGINE", L,
             "lua_getextraspace_lj called with NULL Lua state");
...
bool lua_engine_new_object_meta(EseLuaEngine *engine, const char *name,
  ...
  log_assert("LUA_ENGINE", engine,
             "lua_engine_new_object_meta called with NULL engine");
  log_assert("LUA_ENGINE", engine->runtime,
             "lua_engine_new_object_meta called with NULL runtime");
  log_assert("LUA_ENGINE", name,
             "lua_engine_new_object_meta called with NULL name");
...
bool lua_engine_new_object(EseLuaEngine *engine, const char *name, int count,
  ...
  log_assert("LUA_ENGINE", engine,
             "lua_engine_new_object called with NULL engine");
  log_assert("LUA_ENGINE", engine->runtime,
             "lua_engine_new_object called with NULL runtime");
  log_assert("LUA_ENGINE", name, "lua_engine_new_object called with NULL name");
  log_assert("LUA_ENGINE", count >= 0,
             "lua_engine_new_object called with negative count");
  log_assert("LUA_ENGINE", keys,
             "lua_engine_new_object called with NULL keys array");
  log_assert("LUA_ENGINE", functions,
             "lua_engine_new_object called with NULL functions array");
  ...
  if (lua_isnil(engine->runtime, -1)) {
    ...
    for (int i = 0; i < count; i++) {
      log_assert("LUA_ENGINE", keys[i],
                 "lua_engine_new_object called with NULL key at index %d", i);
      log_assert("LUA_ENGINE", functions[i],
                 "lua_engine_new_object called with NULL function at index %d",
                 i);
