...
int _lua_asset_load_script(lua_State *L) {
  ...
  if (n_args != 1) {
    log_warn("ENGINE",
             "asset_load_script(String script) takes 1 string argument");
  ...
  log_debug("ENGINE", "Loading script %s has %s.", script,
...
int _lua_asset_load_atlas(lua_State *L) {
  ...
  // Get engine reference
  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(L, ENGINE_KEY);
  bool status = asset_manager_load_sprite_atlas(engine->asset_manager, atlas,
                                                group, indexed);

  log_debug("ENGINE", "Loading atlas %s has %s.", atlas,
            status ? "completed" : "failed");
...
int _lua_asset_load_shader(lua_State *L) {
  ...
  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(L, ENGINE_KEY);
  bool status =
      renderer_shader_compile(engine->renderer, group_name, file_name);
...
int _lua_asset_load_map(lua_State *L) {
  ...
  if (n_args != 2) {
    log_warn(
        "ENGINE",
        "asset_load_map(String group, String map) takes 2 string arguments");
    lua_pushboolean(L, false);
    return 1;
  }
  ...
  // Get engine reference
  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(L, ENGINE_KEY);
  bool status = asset_manager_load_map(engine->asset_manager,
                                       engine->lua_engine, map, group);

  log_debug("ENGINE", "Loading map %s has %s.", map,
            status ? "completed" : "failed");
...
int _lua_asset_get_map(lua_State *L) {
  ...
  if (n_args != 1) {
    log_warn("ENGINE",
             "asset_load_map(String ese_map_id) takes 2 string arguments");
    lua_pushboolean(L, false);
    return 1;
  }
  ...
  if (!lua_isstring(L, 1)) {
    log_warn("ENGINE",
             "asset_load_map(String ese_map_id) takes 1 string argument");
    lua_pushboolean(L, false);
    return 1;
  }
...
int _lua_set_pipeline(lua_State *L) {
  ...
  EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(L, ENGINE_KEY);
  bool status = renderer_create_pipeline_state(engine->renderer, vertexShader,
                                               fragmentShader);
...
int _lua_detect_collision(lua_State *L) {
  ...
  if (n_args != 2) {
    log_warn("ENGINE",
             "detect_collision(rect, number max_results) takes 2 argument");
    lua_newtable(L);
    return 1;
  }
  ...
  if (strcmp(tname, "RectProxyMeta") == 0) {
    ...
    EseEntity **entities =
        engine_detect_collision_rect(engine, rect, max_results);
