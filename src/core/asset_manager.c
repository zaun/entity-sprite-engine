...
void _asset_free(void *data) {
  ...
  if (asset->type == ASSET_SPRITE) {
    ...
    log_error("ASSET_MANAGER",
              "Unable to memory_manager.free unknown asset type");
  }
  memory_manager.free(asset);
}

static void _asset_manager_add_group(EseAssetManager *manager,
                                     const char *group) {
  ...
  if (manager->group_count == manager->group_capacity) {
    size_t new_capacity =
        manager->group_capacity == 0 ? 4 : manager->group_capacity * 2;
    char **new_groups = memory_manager.realloc(
        manager->groups, new_capacity * sizeof(char *), MMTAG_ASSET);
    if (!new_groups) {
  ...
  manager->groups[manager->group_count++] =
      memory_manager.strdup(group, MMTAG_ASSET);
}

static void _asset_manager_remove_group(EseAssetManager *manager,
                                        const char *group) {
  for (size_t i = 0; i < manager->group_count; i++) {
    if (strcmp(manager->groups[i], group) == 0) {
...
cJSON *_asset_manager_load_json(const char *filename) {
  ...
  if (!full_path) {
    log_error("ENGINE", "Error: filesystem_get_resource failed for %s",
              filename);
    return NULL;
  }
...
EseAssetManager *asset_manager_create(EseRenderer *renderer) {
  if (!renderer) {
    log_error("ASSET_MANAGER",
              "Error: asset_manager_create called with NULL renderer");
    return NULL;
  }

  EseAssetManager *manager =
      memory_manager.malloc(sizeof(EseAssetManager), MMTAG_ASSET);
  manager->renderer = renderer;
  manager->sprites =
      grouped_hashmap_create((EseGroupedHashMapFreeFn)_asset_free);
  manager->textures =
      grouped_hashmap_create((EseGroupedHashMapFreeFn)_asset_free);
...
void asset_manager_destroy(EseAssetManager *manager) {
  if (!manager) {
    log_error("ASSET_MANAGER",
              "Error: asset_manager_destroy called with NULL manager");
    return;
  }
...
bool asset_manager_load_sprite_atlas(EseAssetManager *manager,
  ...
  log_assert(
      "ASSET_MANAGER", filename,
      "asset_manager_load_sprite_atlas_grouped called with NULL filename");
  log_assert("ASSET_MANAGER", group,
             "asset_manager_load_sprite_atlas_grouped called with NULL group");
  ...
  if (!json) {
    log_error("ASSET_MANAGER", "Error: Failed to load atlas JSON: %s",
              filename);
    return false;
  }
  ...
  if (!cJSON_IsString(image_item)) {
    log_error("ASSET_MANAGER",
              "Error: 'image' property missing or not a string in atlas");
    cJSON_Delete(json);
    return false;
  }
  ...
  if (dot) {
    ...
    for (int i = 0; i < sizeof(extensions) / sizeof(extensions[0]); i++) {
      if (snprintf(full_filename, sizeof(full_filename), "%s.%s", image,
                   extensions[i]) >= sizeof(full_filename)) {
        continue;
      }
  ...
  if (!image_path) {
    if (dot) {
      ...
      log_error("ASSET_MANAGER",
                "Error: Image file not found: %s (tried png, jpg, jpeg, bmp)",
                image);
    }
    cJSON_Delete(json);
  ...
  if (indexed) {
    ...
    if (!processed_data) {
      log_error(
          "ASSET_MANAGER",
          "Error: Failed to allocate memory for indexed texture processing");
    ...
    for (int y = 0; y < img_height; y++) {
      ...
      for (int x = 0; x < img_width; x++) {
        ...
        dst_pixel[3] =
            (src_pixel[0] == tr && src_pixel[1] == tg && src_pixel[2] == tb)
                ? 0
                : 255; // A
      }
    }
  ...
  if (!renderer_load_texture(manager->renderer, texture_id, processed_data,
                             img_width, img_height)) {
    log_error("ASSET_MANAGER", "Error: Failed to load texture for image: %s",
              image);
  ...
  asset_texture->data =
      memory_manager.malloc(sizeof(EseAssetTexture), MMTAG_ASSET);
  ...
  if (!cJSON_IsArray(frameData)) {
    log_error("ASSET_MANAGER",
              "Error: 'frames' property missing or not an array in atlas");
    cJSON_Delete(json);
    memory_manager.free(texture_id);
    return false;
  }
  ...
  if (!cJSON_IsArray(sprites)) {
    log_error("ASSET_MANAGER",
              "Error: 'sprites' property missing or not an array in atlas");
    cJSON_Delete(json);
    memory_manager.free(texture_id);
    return false;
  }
  ...
  for (int i = 0; i < sprite_count; i++) {
    ...
    if (!cJSON_IsString(name_item) || !cJSON_IsNumber(speed_item) ||
    ...
    if (!sprite) {
      log_error("ASSET_MANAGER", "Error: Failed to create sprite for %s",
                spriteName);
      continue;
    }
    ...
    for (int j = 0; j < frame_count; j++) {
      ...
      for (int k = 0; k < region_count; k++) {
        ...
        if (cJSON_IsString(reg_name) &&
            strcmp(reg_name->valuestring, frame_name) == 0) {
          frame = reg;
          break;
        }
      }
      if (!frame) {
        log_error("ASSET_MANAGER",
                  "Error: Frame region '%s' not found for sprite '%s'",
      ...
      if (!cJSON_IsNumber(x_item) || !cJSON_IsNumber(y_item) ||
        ...
        log_error("ASSET_MANAGER", "Error: Malformed region for frame '%s'",
                  frame_name);
        continue;
      }
      ...
      sprite_add_frame(
          sprite, texture_id, (float)x_item->valueint / (float)img_width,
          (float)y_item->valueint / (float)img_height,
          (float)(x_item->valueint + w_item->valueint) / (float)img_width,
          (float)(y_item->valueint + h_item->valueint) / (float)img_height,
          w_item->valueint, h_item->valueint);
    }

    if (sprite_get_frame_count(sprite) == 0) {
      log_error("ASSET_MANAGER", "Sprite '%s' has no valid frames; skipping",
                spriteName);
      sprite_free(sprite);
      continue;
    }
    ...
    log_debug("ASSET_MANAGER", "Adding sprite '%s' with %d frames from atlas",
              spriteName, sprite_get_frame_count(sprite));
  }
...
EseSprite *asset_manager_get_sprite(EseAssetManager *manager,
                                    const char *asset_id) {
  log_assert("ASSET_MANAGER", manager,
             "asset_manager_get_sprite called with NULL manager");
  log_assert("ASSET_MANAGER", asset_id,
             "asset_manager_get_sprite called with NULL asset_id");

  char out_group[64];
  char out_name[64];
  ese_helper_split(asset_id, out_group, sizeof(out_group), out_name,
                   sizeof(out_name));

  EseAsset *asset =
      (EseAsset *)grouped_hashmap_get(manager->sprites, out_group, out_name);
...
bool asset_manager_load_map(EseAssetManager *manager, EseLuaEngine *lua,
                            const char *filename, const char *group) {
  log_assert("ASSET_MANAGER", manager,
             "asset_manager_load_map called with NULL manager");
  log_assert("ASSET_MANAGER", lua,
             "asset_manager_load_map called with NULL lua");
  log_assert("ASSET_MANAGER", filename,
             "asset_manager_load_map called with NULL filename");
  log_assert("ASSET_MANAGER", group,
             "asset_manager_load_map called with NULL group");
  ...
  if (!cJSON_IsNumber(width_item) || !cJSON_IsNumber(height_item)) {
    log_error("ASSET_MANAGER",
              "Map JSON missing width/height or not numbers: %s", filename);
    cJSON_Delete(json);
    return false;
  }
  ...
  if (!tileset_obj || tileset_obj->type != cJSON_Object) {
    log_error("ASSET_MANAGER",
              "Map JSON missing or invalid 'tileset' object: %s", filename);
    cJSON_Delete(json);
    return false;
  }
  ...
  for (cJSON *tile_entry = tileset_obj->child; tile_entry;
       tile_entry = tile_entry->next) {
    if (!tile_entry->string) {
      log_error("ASSET_MANAGER", "Invalid tile key in tileset for %s",
                filename);
    ...
    if (!cJSON_IsArray(tile_entry)) {
      log_error("ASSET_MANAGER",
                "Tileset entry for id %d is not an array in %s", tile_id,
                filename);
      ese_tileset_destroy(tileset);
      cJSON_Delete(json);
      return false;
    ...
    for (int mi = 0; mi < mapping_count; mi++) {
      ...
      if (!sprite_item || !cJSON_IsString(sprite_item) ||
        ...
        log_error("ASSET_MANAGER",
                  "Missing or invalid 'sprite' for tile %d in %s", tile_id,
                  filename);
        ese_tileset_destroy(tileset);
        cJSON_Delete(json);
        return false;
      ...
      size_t sprite_len =
          strlen(group) + 1 + strlen(sprite_item->valuestring) + 1;
      char *sprite_str = (char *)memory_manager.malloc(sprite_len, MMTAG_ASSET);
      snprintf(sprite_str, sprite_len, "%s:%s", group,
               sprite_item->valuestring);
      ...
      if (!ese_tileset_add_sprite(tileset, tile_id, sprite_str,
                                  (uint16_t)weight)) {
        log_error("ASSET_MANAGER",
                  "Failed to add sprite '%s' for tile %d in %s", sprite_str,
                  tile_id, filename);
  ...
  if (!cells || !cJSON_IsArray(cells)) {
    log_error("ASSET_MANAGER", "Map JSON missing or invalid 'cells' array: %s",
              filename);
    ese_tileset_destroy(tileset);
    cJSON_Delete(json);
    return false;
  ...
  EseMap *map = ese_map_create(lua, (uint32_t)width, (uint32_t)height,
                               ese_map_type, false);
  ...
  for (int ci = 0; ci < cell_count; ci++) {
    ...
    if (!cell_obj || !cJSON_IsObject(cell_obj)) {
      log_error("ASSET_MANAGER", "Invalid cell object at index %d in %s", ci,
                filename);
      ese_map_destroy(map);
      ese_tileset_destroy(tileset);
      cJSON_Delete(json);
    ...
    if (!layers || !cJSON_IsArray(layers)) {
      log_error("ASSET_MANAGER", "Cell %d missing 'layers' array in %s", ci,
                filename);
      ese_map_destroy(map);
      ese_tileset_destroy(tileset);
      cJSON_Delete(json);
    ...
    if (!dst) {
      log_error("ASSET_MANAGER", "Invalid cell coords (%u,%u) in %s", x, y,
                filename);
      ese_map_destroy(map);
      ese_tileset_destroy(tileset);
      cJSON_Delete(json);
    ...
    for (int li = 0; li < layer_count; li++) {
      ...
      if (lid == -1) {
        if (!ese_map_cell_add_layer(dst, -1)) {
          log_error("ASSET_MANAGER",
                    "Failed to add layer for blank tile at cell %d in %s", ci,
                    filename);
          ese_map_destroy(map);
          ese_tileset_destroy(tileset);
          cJSON_Delete(json);
      ...
      if (!ese_map_cell_add_layer(dst, tile_id)) {
        log_error("ASSET_MANAGER",
                  "Failed to add layer for tile %d at cell %d in %s", tile_id,
                  ci, filename);
        ese_map_destroy(map);
        ese_tileset_destroy(tileset);
        cJSON_Delete(json);
    ...
    if (dyn_item) {
      if (dyn_item->type == cJSON_True)
        ese_map_cell_set_is_dynamic(dst, true);
      else if (dyn_item->type == cJSON_False)
        ese_map_cell_set_is_dynamic(dst, false);
      else if (cJSON_IsNumber(dyn_item))
        ese_map_cell_set_is_dynamic(dst, (dyn_item->valueint != 0));
    }
  }
...
EseMap *asset_manager_get_map(EseAssetManager *manager, const char *asset_id) {
  log_assert("ASSET_MANAGER", manager,
             "asset_manager_get_map called with NULL manager");
  log_assert("ASSET_MANAGER", asset_id,
             "asset_manager_get_map called with NULL asset_id");
  ...
  EseAsset *asset =
      (EseAsset *)grouped_hashmap_get(manager->maps, out_group, out_name);
...
bool asset_manager_create_font_atlas(EseAssetManager *manager, const char *name,
                                     ...
                                     int total_chars, int char_width,
                                     int char_height) {
  log_assert("ASSET_MANAGER", manager,
             "asset_manager_create_font_atlas called with NULL manager");
  log_assert("ASSET_MANAGER", name,
             "asset_manager_create_font_atlas called with NULL name");
  log_assert("ASSET_MANAGER", font_data,
             "asset_manager_create_font_atlas called with NULL font_data");
  ...
  unsigned char *rgba_data =
      memory_manager.malloc(atlas_width * atlas_height * 4, MMTAG_ASSET);
  if (!rgba_data) {
    ...
    for (int char_x = 0; char_x < chars_per_row; char_x++) {
      ...
      for (int y = 0; y < char_height; y++) {
        for (int x = 0; x < char_width; x++) {
          ...
          int atlas_pixel_index =
              (atlas_pixel_y * atlas_width + atlas_pixel_x) * 4;
  ...
  if (!renderer_load_texture(manager->renderer, name, rgba_data, atlas_width,
                             atlas_height)) {
  ...
  for (int char_y = 0; char_y < (total_chars / chars_per_row); char_y++) {
    for (int char_x = 0; char_x < chars_per_row; char_x++) {
      ...
      if (sprite) {
        ...
        sprite_add_frame(sprite, texture_id, u1, v1, u2, v2, char_width,
                         char_height);

        EseAsset *sprite_asset = _asset_create();
        sprite_asset->type = ASSET_SPRITE;
        sprite_asset->data = sprite;
        grouped_hashmap_set(manager->sprites, "fonts", sprite_name,
                            sprite_asset);
      }
    }
  }
  ...
  log_debug("ASSET_MANAGER", "Created font atlas '%s' with %d glyphs", name,
            total_chars);
  return true;
}
...
void asset_manager_remove_group(EseAssetManager *manager, const char *group) {
  if (!manager) {
    log_error("ASSET_MANAGER",
              "Error: asset_manager_remove_group called with NULL manager");
    return;
  }

  if (!group) {
    log_error("ASSET_MANAGER",
              "Error: asset_manager_remove_group called with NULL group");
    return;
  }
