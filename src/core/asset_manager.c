...
static void _asset_manager_add_group(EseAssetManager *manager, const char *group) {
    ...
    if (manager->group_count == manager->group_capacity) {
        ...
        char **new_groups = memory_manager.realloc(manager->groups, new_capacity * sizeof(char *), MMTAG_ASSET);
        if (!new_groups) {
...
bool asset_manager_load_sprite_atlas(EseAssetManager* manager, const char* filename, const char* group) {
    ...
    for (int i = 0; i < sprite_count; i++) {
        ...
        if (!cJSON_IsObject(sprite_obj)) continue;
        ...
        if (!cJSON_IsString(name_item) || !cJSON_IsNumber(speed_item) || !cJSON_IsArray(frames)) continue;
        ...
        for (int j = 0; j < frame_count; j++) {
            ...
            if (!cJSON_IsString(frame_name_item)) continue;
            ...
            if (!frame) {
                log_error("ASSET_MANAGER", "Error: Frame region '%s' not found for sprite '%s'", frame_name, spriteName);
                continue;
            }
            ...
            sprite_add_frame(
                sprite, texture_id,
                (float)x_item->valueint / (float)img_width,
