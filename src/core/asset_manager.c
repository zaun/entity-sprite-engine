...
bool asset_manager_load_sprite_atlas(EseAssetManager *manager, const char *filename,
    ...
    if (dot) {
        ...
        for (int i = 0; i < sizeof(extensions) / sizeof(extensions[0]); i++) {
            if (snprintf(full_filename, sizeof(full_filename), "%s.%s", image, extensions[i]) >=
    ...
    if (!image_path) {
        if (dot) {
            ...
            log_error("ASSET_MANAGER",
                      "Error: Image file not found: %s (tried png, jpg, jpeg, bmp)", image);
        }
        cJSON_Delete(json);
    ...
    if (indexed) {
        ...
        if (!processed_data) {
            log_error("ASSET_MANAGER", "Error: Failed to allocate memory for "
                                       "indexed texture processing");
        ...
        for (int y = 0; y < img_height; y++) {
            ...
            for (int x = 0; x < img_width; x++) {
                ...
                dst_pixel[3] =
                    (src_pixel[0] == tr && src_pixel[1] == tg && src_pixel[2] == tb) ? 0 : 255; // A
            }
        }
    }

    // Load texture using renderer_load_texture
    if (!renderer_load_texture(manager->renderer, texture_id, processed_data, img_width,
                               img_height)) {
    ...
    for (cJSON *tile_entry = tileset_obj->child; tile_entry; tile_entry = tile_entry->next) {
        ...
        for (int mi = 0; mi < mapping_count; mi++) {
            ...
            if (!ese_map_item || !cJSON_IsObject(ese_map_item)) {
                log_error("ASSET_MANAGER", "Malformed mapping for tile %d in %s", tile_id,
                          filename);
                ese_tileset_destroy(tileset);
                cJSON_Delete(json);
                return false;
        ...
        for (int li = 0; li < layer_count; li++) {
            ...
            if (lid == -1) {
                if (!ese_map_cell_add_layer(dst, -1)) {
                    log_error("ASSET_MANAGER",
                              "Failed to add layer for blank tile at cell %d in %s", ci, filename);
                    ese_map_destroy(map);
                    ese_tileset_destroy(tileset);
                    cJSON_Delete(json);
                    return false;
            ...
            if (ese_tileset_get_sprite_count(tileset, tile_id) == 0) {
                log_error("ASSET_MANAGER",
                          "Tile id %d used in cells but not defined in tileset "
                          "for %s",
                          tile_id, filename);
                ese_map_destroy(map);
                ese_tileset_destroy(tileset);
