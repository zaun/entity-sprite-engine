 ...
 * 
 * @brief This file contains the collision index system for efficient spatial collision detection.
 * 
 * @details The collision index uses a spatial hash grid to partition entities for fast collision
 * detection. It automatically manages collision pair collection and provides efficient neighbor
 * lookups for collision checking.
 */
...
/**
 * @brief Hashed key for grid cells (combines x and y coordinates into a uint64_t)
 */
...
// Entity management
/**
 * @brief Inserts an entity into the collision index based on its position and collider rects.
 * 
 ...
 * 
 * @note The returned EseArray is owned by the collision index and should NOT be freed by the caller.
 * The array is automatically cleared internally before collecting new pairs.
 */
