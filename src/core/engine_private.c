...
void _engine_render_list_clear(EseEngine *engine) {
  log_assert("ENGINE", engine,
             "_engine_render_list_clear called with NULL engine");
...
void _engine_add_texture_to_draw_list(float screen_x, float screen_y,
  ...
  log_assert("ENGINE", user_data,
             "_engine_add_texture_to_draw_list called with NULL user_data");

  EseDrawList *draw_list = (EseDrawList *)user_data;
  EseDrawListObject *obj = draw_list_request_object(draw_list);
  draw_list_object_set_texture(obj, texture_id, texture_x1, texture_y1,
...
void _engine_add_rect_to_draw_list(float screen_x, float screen_y,
  ...
  log_assert("ENGINE", user_data,
             "_engine_add_rect_to_draw_list called with NULL user_data");
...
void _engine_add_polyline_to_draw_list(
  ...
  log_assert("ENGINE", user_data,
             "_engine_add_polyline_to_draw_list called with NULL user_data");
  log_assert("ENGINE", points,
             "_engine_add_polyline_to_draw_list called with NULL points");
  log_assert("ENGINE", point_count > 0,
             "_engine_add_polyline_to_draw_list called with point_count <= 0");
  ...
  draw_list_object_set_polyline_stroke_color(obj, stroke_r, stroke_g, stroke_b,
                                             stroke_a);
  draw_list_object_set_bounds(obj, screen_x, screen_y, 0,
                              0); // Polylines don't use width/height
  draw_list_object_set_z_index(obj, z_index);
}
...
EseRenderList *_engine_get_render_list(EseEngine *engine) {
  log_assert("ENGINE", engine,
             "_engine_get_render_list called with NULL engine");
...
int _engine_entity_find(void *data, void *user_data) {
  ...
  log_assert("ENGINE", user_data,
             "_engine_entity_find called with NULL user_data");
