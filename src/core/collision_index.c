...
static void _collision_index_convert_cell_to_dbvh(EseCollisionIndex *index,
                                                  int center_x, int center_y);
static void _collision_index_emit_pair_if_new(EseCollisionIndex *index,
                                              EseHashMap *seen, EseArray *pairs,
                                              EseEntity *a, EseEntity *b);
...
static void _dbvh_query_pairs(DBVHNode *root, EseArray *pairs,
                              EseCollisionIndex *index, EseHashMap *seen);
...
float _collision_index_calculate_average_bin_count(EseCollisionIndex *index) {
  ...
  return non_empty_bins > 0 ? (float)total_entities / (float)non_empty_bins
                            : 0.0f;
}
...
static DBVHNode *_dbvh_node_create(EseEntity *entity) {
  DBVHNode *node =
      memory_manager.malloc(sizeof(DBVHNode), MMTAG_COLLISION_INDEX);
  if (!node)
    return NULL;
  node->entity = entity;
  node->left = NULL;
  ...
  if (entity && entity->collision_world_bounds) {
    ...
    node->bounds_x = node->bounds_y = node->bounds_width = node->bounds_height =
        0;
  }
  return node;
}
...
static int _dbvh_get_balance(DBVHNode *node) {
  return node ? _dbvh_get_height(node->left) - _dbvh_get_height(node->right)
              : 0;
}
...
static DBVHNode *_dbvh_insert(DBVHNode *root, EseEntity *entity) {
  ...
  root->height =
      1 + fmax(_dbvh_get_height(root->left), _dbvh_get_height(root->right));
...
static void _collision_index_emit_pair_if_new(EseCollisionIndex *index,
  ...
  if (a == b)
    return;
  if (a->id && b->id &&
      strcmp(ese_uuid_get_value(a->id), ese_uuid_get_value(b->id)) == 0)
    return;
  ...
  CollisionPair *pair = (CollisionPair *)memory_manager.malloc(
      sizeof(CollisionPair), MMTAG_COLLISION_INDEX);
  pair->entity_a = a;
  pair->entity_b = b;
...
static void _dbvh_query_pairs(DBVHNode *root, EseArray *pairs,
  ...
  // cross-boundary: test DBVH entities against neighboring grid bins (outside
  // 3x3)
...
static void _collision_index_convert_cell_to_dbvh(EseCollisionIndex *index,
                                                  int center_x, int center_y) {
  EseCollisionIndexKey center_key =
      _collision_index_compute_key(center_x, center_y);
  // already converted?
  ...
  for (int dx = -1; dx <= 1; dx++) {
    for (int dy = -1; dy <= 1; dy++) {
      EseCollisionIndexKey k =
          _collision_index_compute_key(center_x + dx, center_y + dy);
      if (!int_hashmap_get(index->bins, k))
        return; // missing -> skip conversion
    }
  }
  // collect entities
  EseArray *entities = array_create(64, NULL);
  for (int dx = -1; dx <= 1; dx++) {
    for (int dy = -1; dy <= 1; dy++) {
      ...
      EseDoubleLinkedList *list =
          (EseDoubleLinkedList *)int_hashmap_get(index->bins, key);
      if (!list)
        continue;
  ...
  if (root) {
    ...
    for (int dx = -1; dx <= 1; dx++) {
      for (int dy = -1; dy <= 1; dy++) {
        EseCollisionIndexKey k =
            _collision_index_compute_key(center_x + dx, center_y + dy);
        // int_hashmap_remove returns the stored value but does not call the
        // map's free_fn. Free the returned list explicitly to avoid leaking
        // list nodes.
    ...
    log_debug("COLLISION_INDEX",
              "Converted 3x3 centered (%d,%d) to DBVH with %zu entities",
...
EseCollisionIndex *collision_index_create(void) {
  EseCollisionIndex *index =
      memory_manager.malloc(sizeof(EseCollisionIndex), MMTAG_COLLISION_INDEX);
  index->cell_size = COLLISION_INDEX_DEFAULT_CELL_SIZE;
  index->bins = int_hashmap_create((EseIntHashMapFreeFn)dlist_free);
  index->dbvh_regions =
      int_hashmap_create((EseIntHashMapFreeFn)_dbvh_node_destroy);
...
void collision_index_insert(EseCollisionIndex *index, EseEntity *entity) {
  ...
  if (now - index->last_auto_tune_time >=
      COLLISION_INDEX_AUTO_TUNE_COOLDOWN_SECONDS) {
...
void collision_index_get_neighbors(EseCollisionIndex *index, int cell_x,
  ...
  log_assert("COLLISION_INDEX", neighbors,
             "get_neighbors called with NULL neighbors");
  log_assert("COLLISION_INDEX", neighbor_count,
             "get_neighbors called with NULL neighbor_count");
  *neighbor_count = 0;
  for (int dx = -1; dx <= 1; dx++) {
    for (int dy = -1; dy <= 1; dy++) {
      if (dx == 0 && dy == 0)
        continue;
      EseDoubleLinkedList *l =
          collision_index_get_cell(index, cell_x + dx, cell_y + dy);
      if (l && dlist_size(l) > 0) {
...
void collision_index_auto_tune(EseCollisionIndex *index) {
  ...
  log_debug("COLLISION_INDEX",
            "Auto-tuned cell_size to %f based on %zu samples (avg diag: %f)",
...
EseArray *collision_index_get_pairs(EseCollisionIndex *index) {
  ...
  EseHashMap *seen = hashmap_create(
      NULL); // defensive dedupe across DBVHs, WE DONT OWN THE CONTENT!
  ...
  while (int_hashmap_iter_next(bin_iter, &bin_key, &bin_value)) {
    ...
    if (dlist_size(cell_list) >= 2) {
      ...
      while (dlist_iter_next(outer, &a_val)) {
        ...
        while (dlist_iter_next(inner, &b_val)) {
          ...
          _collision_index_emit_pair_if_new(index, seen, index->collision_pairs,
                                            a, b);
        }
    ...
    for (int dx = -1; dx <= 1; dx++) {
      for (int dy = -1; dy <= 1; dy++) {
        ...
        EseCollisionIndexKey nkey = _collision_index_compute_key(nx, ny);
        if (nkey <= bin_key)
          continue; // ensures each unordered cell-pair handled once
        // skip neighbor if it's a DBVH center (DBVH handled cross-boundary
        // checks)
        if (int_hashmap_get(index->dbvh_regions, nkey))
          continue;
        EseDoubleLinkedList *neighbor_list =
        ...
        while (dlist_iter_next(cell_it, &c_val)) {
          ...
          while (dlist_iter_next(neigh_it, &n_val)) {
            ...
            _collision_index_emit_pair_if_new(index, seen,
                                              index->collision_pairs, c, n);
          }
