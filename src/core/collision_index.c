...
static void _collision_index_convert_cell_to_dbvh(EseCollisionIndex *index, int center_x,
                                                  int center_y);
static void _collision_index_emit_pair_if_new(EseCollisionIndex *index, EseHashMap *seen,
...
static void _dbvh_query_pairs(DBVHNode *root, EseArray *pairs, EseCollisionIndex *index,
                              EseHashMap *seen);
...
static void _collision_index_emit_pair_if_new(EseCollisionIndex *index, EseHashMap *seen,
    ...
    CollisionPair *pair =
        (CollisionPair *)memory_manager.malloc(sizeof(CollisionPair), MMTAG_COLLISION_INDEX);
    pair->entity_a = a;
    pair->entity_b = b;
...
static void _collision_index_convert_cell_to_dbvh(EseCollisionIndex *index, int center_x,
    ...
    if (root) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                EseCollisionIndexKey k = _collision_index_compute_key(center_x + dx, center_y + dy);
                // int_hashmap_remove returns the stored value but does not call
                // the map's free_fn. Free the returned list explicitly to avoid
                // leaking list nodes.
        ...
        log_debug("COLLISION_INDEX", "Converted 3x3 centered (%d,%d) to DBVH with %zu entities",
...
void collision_index_get_neighbors(EseCollisionIndex *index, int cell_x, int cell_y,
                                   EseDoubleLinkedList **neighbors, size_t *neighbor_count) {
...
void collision_index_auto_tune(EseCollisionIndex *index) {
    ...
    log_debug("COLLISION_INDEX", "Auto-tuned cell_size to %f based on %zu samples (avg diag: %f)",
...
EseArray *collision_index_get_pairs(EseCollisionIndex *index) {
    ...
    EseHashMap *seen =
        hashmap_create(NULL); // defensive dedupe across DBVHs, WE DONT OWN THE CONTENT!
    ...
    while (int_hashmap_iter_next(bin_iter, &bin_key, &bin_value)) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                ...
                // skip neighbor if it's a DBVH center (DBVH handled
                ...
                while (dlist_iter_next(cell_it, &c_val)) {
                    ...
                    while (dlist_iter_next(neigh_it, &n_val)) {
                        ...
                        _collision_index_emit_pair_if_new(index, seen, index->collision_pairs, c,
                                                          n);
                    }
