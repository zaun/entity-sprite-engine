...
typedef struct {
    ...
    char prefix[ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE + 1]; /** ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE
...
EseConsole *console_create(void) {
    ...
    console->history =
        memory_manager.malloc(sizeof(EseConsoleLine) * ESE_CONSOLE_MAX_HISTORY, MMTAG_CONSOLE);
...
void console_draw(EseConsole *console, EseAssetManager *manager, int view_width, int view_height,
    ...
    callbacks->draw_rect(0, 0, z_index, view_width, console_height, 0.0f, true, bg_r, bg_g, bg_b,
                         bg_a, user_data);

    // Draw bottom border rectangle (2 pixels high)
    int border_y = console_height;
    z_index = UINT64_MAX - 1;
    callbacks->draw_rect(0, border_y, z_index, view_width, 2, 0.0f, true, border_r, border_g,
                         border_b, border_a, user_data);
    ...
    for (int i = 0; i < num_lines_to_draw; i++) {
        // Calculate line_index to show oldest lines first (at the top of the
        // text block)
        ...
        case ESE_CONSOLE_INFO:
            // Blue dot
            callbacks->draw_rect(dot_x, dot_y - dot_radius, z_index, dot_radius * 2, dot_radius * 2,
                                 0.0f, true, 0, 100, 255, 255, user_data);
            break;
        case ESE_CONSOLE_WARN:
            // Orange dot
            callbacks->draw_rect(dot_x, dot_y - dot_radius, z_index, dot_radius * 2, dot_radius * 2,
                                 0.0f, true, 255, 165, 0, 255, user_data);
            break;
        case ESE_CONSOLE_ERROR:
            // Red dot
            callbacks->draw_rect(dot_x, dot_y - dot_radius, z_index, dot_radius * 2, dot_radius * 2,
                                 0.0f, true, 255, 0, 0, 255, user_data);
            break;
        ...
        for (int j = 0; j < ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE;
            ...
            if (c >= 32 && c <= 126) { // Printable ASCII
                ...
                if (letter) {
                    ...
                    callbacks->draw_texture(prefix_x, y_pos, w, h, z_index, texture_id, x1, y1, x2,
                                            y2, w, h, user_data);
                }
            } else {
        ...
        for (int j = 0; line->message[j]; j++) {
            ...
            if (c >= 32 && c <= 126) { // Printable ASCII
                ...
                if (letter) {
                    ...
                    sprite_get_frame(letter, 0, &texture_id, &x1, &y1, &x2, &y2, &w, &h);
                    callbacks->draw_texture(message_x, current_y, w, h, z_index, texture_id, x1, y1,
                                            x2, y2, w, h, user_data);
                }
