 ...
 *          The prefix is limited to ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE
 * characters plus null terminator.
 */
typedef struct {
  EseConsoleLineType type; /** The type of console line */
  char prefix[ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE +
              1]; /** ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE chars + null
                     terminator */
...
EseConsole *console_create(void) {
  EseConsole *console =
      memory_manager.malloc(sizeof(EseConsole), MMTAG_CONSOLE);
  console->history = memory_manager.malloc(
      sizeof(EseConsoleLine) * ESE_CONSOLE_MAX_HISTORY, MMTAG_CONSOLE);
...
void console_add_line(EseConsole *console, EseConsoleLineType type,
  ...
  // Copy prefix (ensure it fits in ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE
...
void console_set_draw_line_count(EseConsole *console, int line_count) {
  log_assert("CONSOLE", console,
             "console_set_draw_line_count called with NULL console");
...
void console_draw(EseConsole *console, EseAssetManager *manager, int view_width,
  ...
  log_assert("CONSOLE", callbacks->draw_rect,
             "console_draw called with NULL draw_rect callback");
  ...
  uint64_t z_index = UINT64_MAX - 2;
  callbacks->draw_rect(0, 0, z_index, view_width, console_height, 0.0f, true,
                       bg_r, bg_g, bg_b, bg_a, user_data);

  // Draw bottom border rectangle (2 pixels high)
  int border_y = console_height;
  z_index = UINT64_MAX - 1;
  callbacks->draw_rect(0, border_y, z_index, view_width, 2, 0.0f, true,
                       border_r, border_g, border_b, border_a, user_data);

  // Draw console text lines
  int bottom_padding = 5; // Padding from the bottom of the console area
  int num_lines_to_draw = console->draw_line_count < console->history_size
                              ? console->draw_line_count
                              : console->history_size;
  z_index = UINT64_MAX;

  for (int i = 0; i < num_lines_to_draw; i++) {
    // Calculate line_index to show oldest lines first (at the top of the text
    // block)
    ...
    // Calculate y_pos to anchor the text block to the bottom of the console
    // area The newest line (i = num_lines_to_draw - 1) will be at
    // console_height - bottom_padding - line_height The oldest line (i = 0)
    // will be at console_height - bottom_padding - (num_lines_to_draw *
    // line_height)
    int y_pos = console_height - bottom_padding -
                ((num_lines_to_draw - i) * line_height);

    // Draw type indicator dot at start of line
    int dot_x = 5;
    int dot_y =
        y_pos + (console->font_char_height / 2); // Center vertically with text
    int dot_radius = console->font_char_width / 2;

    switch (line->type) {
    case ESE_CONSOLE_INFO:
      // Blue dot
      callbacks->draw_rect(dot_x, dot_y - dot_radius, z_index, dot_radius * 2,
                           dot_radius * 2, 0.0f, true, 0, 100, 255, 255,
                           user_data);
      break;
    case ESE_CONSOLE_WARN:
      // Orange dot
      callbacks->draw_rect(dot_x, dot_y - dot_radius, z_index, dot_radius * 2,
                           dot_radius * 2, 0.0f, true, 255, 165, 0, 255,
                           user_data);
      break;
    case ESE_CONSOLE_ERROR:
      // Red dot
      callbacks->draw_rect(dot_x, dot_y - dot_radius, z_index, dot_radius * 2,
                           dot_radius * 2, 0.0f, true, 255, 0, 0, 255,
                           user_data);
      break;
    ...
    // Draw prefix (e.g., "SYSTEM:", "INFO:") - always
    // ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE chars wide
    int prefix_x =
        5 + (dot_radius * 2) + 4; // Start after the dot with some spacing
    for (int j = 0; j < ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE;
         j++) { // Always draw exactly ESE_CONSOLE_ESE_CONSOLE_PREFIX_SIZE
                // characters
      char c = line->prefix[j];
      if (c >= 32 && c <= 126) { // Printable ASCII
        char sprite_name[64];
        snprintf(sprite_name, sizeof(sprite_name),
                 "fonts:console_font_10x20_%03d", (int)c);
        ...
        if (letter) {
          ...
          callbacks->draw_texture(prefix_x, y_pos, w, h, z_index, texture_id,
                                  x1, y1, x2, y2, w, h, user_data);
        }
      } else {
        // Draw space for non-printable or missing characters
        char sprite_name[64];
        snprintf(sprite_name, sizeof(sprite_name),
                 "fonts:console_font_10x20_032"); // Space character
    ...
    for (int j = 0; line->message[j]; j++) {
      ...
      if (c >= 32 && c <= 126) { // Printable ASCII
        ...
        if (letter) {
          ...
          sprite_get_frame(letter, 0, &texture_id, &x1, &y1, &x2, &y2, &w, &h);
          callbacks->draw_texture(message_x, current_y, w, h, z_index,
                                  texture_id, x1, y1, x2, y2, w, h, user_data);
        }
