...
EseEngine *engine_create(const char *startup_script) {
  ...
  lua_engine_add_registry_key(engine->lua_engine->runtime, LUA_ENGINE_KEY,
                              engine->lua_engine);
  ...
  lua_engine_add_function(engine->lua_engine, "asset_load_script",
                          _lua_asset_load_script);
  lua_engine_add_function(engine->lua_engine, "asset_load_atlas",
                          _lua_asset_load_atlas);
  lua_engine_add_function(engine->lua_engine, "asset_load_shader",
                          _lua_asset_load_shader);
  lua_engine_add_function(engine->lua_engine, "asset_load_map",
                          _lua_asset_load_map);
  lua_engine_add_function(engine->lua_engine, "asset_get_map",
                          _lua_asset_get_map);
  lua_engine_add_function(engine->lua_engine, "set_pipeline",
                          _lua_set_pipeline);
  lua_engine_add_function(engine->lua_engine, "detect_collision",
                          _lua_detect_collision);
  ...
  lua_engine_add_global(engine->lua_engine, "InputState",
                        engine->input_state->lua_ref);
  ...
  lua_engine_add_global(engine->lua_engine, "Camera",
                        engine->camera_state->lua_ref);
  ...
  if (startup_script) {
    ...
    engine->startup_ref =
        lua_engine_instance_script(engine->lua_engine, startup_script);
  }
...
void engine_start(EseEngine *engine) {
  ...
  if (engine->startup_ref != LUA_NOREF) {
    lua_engine_run_function(engine->lua_engine, engine->startup_ref, LUA_NOREF,
                            "startup", 0, NULL, NULL);
  }
...
void engine_set_renderer(EseEngine *engine, EseRenderer *renderer) {
  ...
  // Add fonts
  asset_manager_create_font_atlas(engine->asset_manager, "console_font_10x20",
                                  console_font_10x20, 256, 10, 20);
}
...
void engine_update(EseEngine *engine, float delta_time,
  ...
  // Update the input state
  memcpy(engine->input_state->keys_down, state->keys_down,
         sizeof(state->keys_down));
  memcpy(engine->input_state->keys_pressed, state->keys_pressed,
         sizeof(state->keys_pressed));
  memcpy(engine->input_state->keys_released, state->keys_released,
         sizeof(state->keys_released));
  memcpy(engine->input_state->mouse_down, state->mouse_down,
         sizeof(state->mouse_down));
  memcpy(engine->input_state->mouse_clicked, state->mouse_clicked,
         sizeof(state->mouse_clicked));
  ...
  profile_start(PROFILE_ENG_UPDATE_SECTION);
  EseArray *collision_hits = collision_resolver_solve(
      engine->collision_resolver, spatial_pairs, engine->lua_engine);
  ...
  // based on the camera position and view dimensions passed to it. Each visible
  // entity may contribute multiple draw calls to the list.
  profile_start(PROFILE_ENG_UPDATE_SECTION);
  ...
  if (engine->draw_console) {
    ...
    console_draw(engine->console, engine->asset_manager,
                 ese_display_get_viewport_width(engine->display_state),
                 ese_display_get_viewport_height(engine->display_state),
                 &console_callbacks, engine->draw_list);
  }
  ...
  render_list_set_size(render_list,
                       ese_display_get_viewport_width(engine->display_state),
                       ese_display_get_viewport_height(engine->display_state));
...
EseEntity **engine_detect_collision_rect(EseEngine *engine, EseRect *rect,
  ...
  EseEntity **results = memory_manager.malloc(
      sizeof(EseEntity *) * (max_count + 1), MMTAG_ENGINE);
  if (!results) {
...
EseEntity **engine_find_by_tag(EseEngine *engine, const char *tag,
  ...
  EseEntity **result = memory_manager.malloc(
      sizeof(EseEntity *) * (max_count + 1), MMTAG_ENGINE);
  if (!result) {
...
void engine_add_to_console(EseEngine *engine, EseConsoleLineType type,
  ...
  log_assert("ENGINE", message,
             "engine_add_to_console called with NULL message");
...
EseEntity *engine_find_by_id(EseEngine *engine, const char *uuid_string) {
  ...
  log_assert("ENGINE", uuid_string,
             "engine_find_by_id called with NULL uuid_string");
  ...
  while (dlist_iter_next(iter, &entity_value)) {
    ...
    if (entity && entity->active &&
        strcmp(ese_uuid_get_value(entity->id), uuid_string) == 0) {
      dlist_iter_free(iter);
      return entity;
    }
  }
...
void engine_pubsub_sub(EseEngine *engine, const char *name, EseEntity *entity,
  ...
  log_assert("ENGINE", function_name,
             "engine_pubsub_sub called with NULL function_name");
...
void engine_pubsub_unsub(EseEngine *engine, const char *name, EseEntity *entity,
  ...
  log_assert("ENGINE", function_name,
             "engine_pubsub_unsub called with NULL function_name");
