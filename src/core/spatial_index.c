...
static bool _pair_is_potential_collision(EseEntity *a, EseEntity *b) {
  ...
  for (size_t i = 0; i < a->component_count; i++) {
    ...
    if (comp->type == ENTITY_COMPONENT_COLLIDER) {
      a_has_collider = true;
      EseEntityComponentCollider *col =
...
static void _emit_pair_if_new(SpatialIndex *index, EseHashMap *seen,
                              EseArray *pairs, EseEntity *a, EseEntity *b) {
  if (!a || !b || !seen || !pairs)
    return;
  if (a == b)
    return;
  ...
  SpatialPair *pair = (SpatialPair *)memory_manager.malloc(
      sizeof(SpatialPair), MMTAG_COLLISION_INDEX);
  pair->a = a;
  pair->b = b;
...
static void _dbvh_query_pairs(DBVHNode *root, EseArray *pairs,
                              SpatialIndex *index, EseHashMap *seen) {
  if (!root || !index || !pairs)
    return;
...
static void _convert_cell_to_dbvh(SpatialIndex *index, int center_x,
                                  int center_y) {
  ...
  for (int dx = -1; dx <= 1; dx++) {
    for (int dy = -1; dy <= 1; dy++) {
      SpatialIndexKey k =
          _spatial_index_compute_key(center_x + dx, center_y + dy);
...
SpatialIndex *spatial_index_create(void) {
  SpatialIndex *index =
      memory_manager.malloc(sizeof(SpatialIndex), MMTAG_COLLISION_INDEX);
...
void spatial_index_insert(SpatialIndex *index, EseEntity *entity) {
  ...
  if (now - index->last_auto_tune_time >=
      SPATIAL_INDEX_AUTO_TUNE_COOLDOWN_SECONDS) {
