 ...
 * ============================
 * 
 * This file implements a hybrid spatial indexing system that combines a uniform grid
 * with Dynamic Bounding Volume Hierarchies (DBVH) for efficient collision detection.
 * The system automatically adapts to entity density and distribution patterns.
 * 
 ...
 * 2. AABB prechecking: Avoid expensive collision tests for non-overlapping entities
 * 3. Pair deduplication: Prevent duplicate collision pairs using sorted entity IDs
...
static void _free_spatial_pair(void *ptr) { if (ptr) memory_manager.free(ptr); }
...
static bool _pair_is_potential_collision(EseEntity *a, EseEntity *b) {
    ...
    for (size_t i = 0; i < a->component_count; i++) {
        ...
        if (comp->type == ENTITY_COMPONENT_MAP) a_has_map = true;
        if (comp->type == ENTITY_COMPONENT_COLLIDER) {
            a_has_collider = true;
            EseEntityComponentCollider *col = (EseEntityComponentCollider*)comp->data;
            if (col) a_collider_map_interaction = col->map_interaction;
        }
    }
    for (size_t j = 0; j < b->component_count; j++) {
        EseEntityComponent *comp = b->components[j];
        if (!comp || !comp->active) continue;
        if (comp->type == ENTITY_COMPONENT_MAP) b_has_map = true;
        if (comp->type == ENTITY_COMPONENT_COLLIDER) {
            b_has_collider = true;
            EseEntityComponentCollider *col = (EseEntityComponentCollider*)comp->data;
            if (col) b_collider_map_interaction = col->map_interaction;
        }
    }

    if (a_has_collider && b_has_collider) return true;
    if (a_has_map && b_has_collider && b_collider_map_interaction) return true;
    if (b_has_map && a_has_collider && a_collider_map_interaction) return true;
    return false;
}
...
static float _calculate_average_bin_count(SpatialIndex *index) {
	...
	while (int_hashmap_iter_next(iter, &key, &value)) {
		...
		if (sz > 0) { total_entities += sz; non_empty_bins++; }
	}
	int_hashmap_iter_free(iter);
	return non_empty_bins > 0 ? (float)total_entities / (float)non_empty_bins : 0.0f;
}

static void _emit_pair_if_new(SpatialIndex *index, EseHashMap *seen, EseArray *pairs, EseEntity *a, EseEntity *b) {
...
static void _dbvh_query_pairs(DBVHNode *root, EseArray *pairs, SpatialIndex *index, EseHashMap *seen) {
	...
	for (size_t i = 0; i < array_size(entities); i++) {
		...
		for (size_t j = i + 1; j < array_size(entities); j++) {
			...
			if (_pair_is_potential_collision(a, b) &&
				a->collision_world_bounds && b->collision_world_bounds &&
				ese_rect_intersects(a->collision_world_bounds, b->collision_world_bounds)) {
				_emit_pair_if_new(index, seen, pairs, a, b);
			}
		}
	}
	int cx = root->region_center_x; int cy = root->region_center_y;
...
static void _convert_cell_to_dbvh(SpatialIndex *index, int center_x, int center_y) {
	...
	for (int dx = -1; dx <= 1; dx++) {
		for (int dy = -1; dy <= 1; dy++) {
			...
			if (!int_hashmap_get(index->bins, k)) return;
		}
	}
	EseArray *entities = array_create(64, NULL);
	for (int dx = -1; dx <= 1; dx++) {
	...
	if (root) {
		...
		for (int dx = -1; dx <= 1; dx++) {
			for (int dy = -1; dy <= 1; dy++) {
				...
				if (removed) { dlist_free(removed); }
			}
		}
		int_hashmap_set(index->dbvh_regions, center_key, root);
		log_debug("SPATIAL_INDEX", "Converted 3x3 centered (%d,%d) to DBVH with %zu entities", center_x, center_y, array_size(entities));
	}
	array_destroy(entities);
}
...
void spatial_index_insert(SpatialIndex *index, EseEntity *entity) {
	...
	for (int cx = min_cell_x; cx <= max_cell_x; cx++) {
		for (int cy = min_cell_y; cy <= max_cell_y; cy++) {
			...
			EseDoubleLinkedList *list = (EseDoubleLinkedList*)int_hashmap_get(index->bins, key);
			if (!list) { list = dlist_create(NULL); int_hashmap_set(index->bins, key, list); }
...
void spatial_index_auto_tune(SpatialIndex *index) {
	...
	float total = 0.0f; size_t samples = 0;
	EseIntHashMapIter *iter = int_hashmap_iter_create(index->bins);
	uint64_t key; void *value;
	while (int_hashmap_iter_next(iter, &key, &value)) {
		EseDoubleLinkedList *list = (EseDoubleLinkedList*)value;
	...
	int_hashmap_iter_free(iter);
	if (samples == 0) { index->cell_size = SPATIAL_INDEX_DEFAULT_CELL_SIZE; return; }
	float avg = total / (float)samples;
	float new_size = fmaxf(32.0f, avg * 2.0f);
	index->cell_size = new_size;
	log_debug("SPATIAL_INDEX", "Auto-tuned cell_size to %f based on %zu samples (avg diag: %f)", new_size, samples, avg);
}
...
EseArray *spatial_index_get_pairs(SpatialIndex *index) {
	...
	uint64_t bin_key; void *bin_value;
	...
	uint64_t dbvh_key; void *dbvh_value;
	...
	while (int_hashmap_iter_next(bin_iter, &bin_key, &bin_value)) {
		...
		if (dlist_size(cell_list) >= 2) {
			void *a_val; EseDListIter *outer = dlist_iter_create(cell_list);
		...
		for (int dx = -1; dx <= 1; dx++) {
			for (int dy = -1; dy <= 1; dy++) {
				...
				int nx = cell_x + dx; int ny = cell_y + dy;
				SpatialIndexKey nkey = _spatial_index_compute_key(nx, ny);
				if (nkey <= bin_key) continue;
				if (int_hashmap_get(index->dbvh_regions, nkey)) continue;
				EseDoubleLinkedList *neighbor_list = (EseDoubleLinkedList*)int_hashmap_get(index->bins, nkey);
				if (!neighbor_list || dlist_size(neighbor_list) == 0) continue;
				...
				while (dlist_iter_next(cell_it, &c_val)) {
					...
					// Component-kind prefilter and AABB precheck
					if (_pair_is_potential_collision(c, n) &&
						c->collision_world_bounds && n->collision_world_bounds &&
						ese_rect_intersects(c->collision_world_bounds, n->collision_world_bounds)) {
