 ...
 *
 * @details The functions declared here provide the main entry points for
 * creating, destroying, and controlling the engine from a native application.
 */
#ifndef ESE_ENGINE_H
#define ESE_ENGINE_H
 ...
 *
 * @details This function allocates and initializes a new engine, including its
 * renderer, asset manager, and Lua engine. It also loads a startup script.
 *
 ...
 *
 * @details This function deallocates all memory associated with the engine,
 * including the renderer, render lists, entities, and Lua engine.
 *
 ...
 *
 * @details This function assigns a renderer to the engine and sets up the
 * render list. If a renderer was previously set, the old asset manager is
 * destroyed and a new one is created.
 *
 * @param engine A pointer to the EseEngine instance.
 * @param renderer A pointer to the renderer to set, or NULL to clear the
 * renderer.
 *
 * @note This function asserts that the `engine` pointer is not NULL.
 */
 ...
 *
 * @details This function appends an entity to the engine's internal list,
 * making it part of the game loop for updates and rendering.
 *
 * @param engine A pointer to the EseEngine instance.
 * @param entity A pointer to the EseEntity to add.
 *
 * @note This function asserts that both the `engine` and `entity` pointers are
 * not NULL.
 * @warning The engine will take ownership of the `entity` pointer, and it will
 * be freed when the engine is destroyed.
 */
 ...
 *
 * @details This function should be called once by the native application to
 * begin the engine's lifecycle, typically by running the `startup` function in
 * the main Lua script.
 *
 * @param engine A pointer to the EseEngine instance.
 *
 ...
 *
 * @details This function processes input, updates all entities, and prepares
 * the render lists for the next frame. It should be called by the native
 * application on every frame.
 *
 * @param engine A pointer to the EseEngine instance.
 ...
 *
 * @details This function checks all active entities in the engine against the
 * specified rectangle and returns those that intersect with it.
 *
 * @param engine A pointer to the EseEngine instance.
 ...
 *
 * @details This function looks up a sprite by its ID in the engine's asset
 * manager and returns a pointer to it if found.
 *
 * @param engine A pointer to the EseEngine instance.
...
/**
 * @brief Gets the camera state.
 *
 * @param engine Pointer to the EseEngine instance.
 * @return Pointer to the camera state.
 */
 ...
 *
 * @param engine Pointer to the EseEngine instance.
 * @return Pointer to the display state.
 */
EseDisplay *engine_get_display(EseEngine *engine);

/**
 * @brief Gets the draw list.
 *
 * @param engine Pointer to the EseEngine instance.
 * @return Pointer to the draw list.
 */
