...
static JobResult _system_job_worker(void *thread_data, const void *user_data,
  ...
  if (job_data && job_data->sys && job_data->sys->vt &&
      job_data->sys->vt->update) {
 ...
 */
static void _system_job_cleanup(ese_job_id_t job_id, void *user_data,
                                void *result) {
  (void)job_id;
  (void)result;
...
EseSystemManager *system_manager_create(const EseSystemManagerVTable *vt,
  ...
  log_assert("SYSTEM_MANAGER", vt,
             "system_manager_create called with NULL vtable");
...
void system_manager_destroy(EseSystemManager *sys, EseEngine *eng) {
  log_assert("SYSTEM_MANAGER", sys,
             "system_manager_destroy called with NULL system");
  log_assert("SYSTEM_MANAGER", eng,
             "system_manager_destroy called with NULL engine");
...
void engine_add_system(EseEngine *eng, EseSystemManager *sys) {
  log_assert("SYSTEM_MANAGER", eng,
             "engine_add_system called with NULL engine");
  log_assert("SYSTEM_MANAGER", sys,
             "engine_add_system called with NULL system");

  if (eng->sys_count == eng->sys_cap) {
    size_t nc = eng->sys_cap ? eng->sys_cap * 2 : 4;
    eng->systems = memory_manager.realloc(
        eng->systems, sizeof(EseSystemManager *) * nc, MMTAG_ENGINE);
    eng->sys_cap = nc;
  }
...
void engine_run_phase(EseEngine *eng, EseSystemPhase phase, float dt,
  ...
  if (parallel && eng->job_queue) {
    ...
    job_ids = memory_manager.malloc(sizeof(ese_job_id_t) * eng->sys_count,
                                    MMTAG_ENGINE);
  }
  ...
  for (size_t i = 0; i < eng->sys_count; i++) {
    ...
    if (parallel && eng->job_queue) {
      ...
      SystemJobData *job_data =
          memory_manager.malloc(sizeof(SystemJobData), MMTAG_ENGINE);
      job_data->sys = s;
      job_data->eng = eng;
      job_data->dt = dt;
      ...
      ese_job_id_t job_id =
          ese_job_queue_push(eng->job_queue, _system_job_worker,
...
void engine_notify_comp_add(EseEngine *eng, EseEntityComponent *c) {
  log_assert("SYSTEM_MANAGER", eng,
             "engine_notify_comp_add called with NULL engine");
  log_assert("SYSTEM_MANAGER", c,
             "engine_notify_comp_add called with NULL component");
...
void engine_notify_comp_rem(EseEngine *eng, EseEntityComponent *c) {
  log_assert("SYSTEM_MANAGER", eng,
             "engine_notify_comp_rem called with NULL engine");
  log_assert("SYSTEM_MANAGER", c,
             "engine_notify_comp_rem called with NULL component");
