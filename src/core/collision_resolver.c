 ...
 * =================================
 * 
 * This file implements a collision resolution system that processes spatial pairs
 * from the spatial index and generates detailed collision events with state tracking.
 * The resolver handles both entity-to-entity collisions and entity-to-map collisions,
 * providing enter/leave/stay state transitions for game logic.
 * 
 ...
 *    - Test specific component combinations (collider vs collider, collider vs map)
 ...
 * =====================
 * 
 * 1. AABB prefiltering: Skip expensive component tests for non-overlapping pairs
 ...
 * 4. Component dispatch: Route to appropriate collision testing based on component types
...
static bool _pair_involves_map_collision(EseEntity *a, EseEntity *b) {
    ...
    // Map collision if one side is map and the other is collider with map interaction enabled
...
static bool _get_map_pair_entities(EseEntity *a, EseEntity *b, EseEntity **out_collider, EseEntity **out_map) {
    if (!a || !b || !out_collider || !out_map) return false;
    ...
    if (a_has_map && b_has_collider && b_collider_map_interaction) {
        *out_map = a; *out_collider = b; return true;
    }
    if (b_has_map && a_has_collider && a_collider_map_interaction) {
        *out_map = b; *out_collider = a; return true;
    }
    return false;
}
...
CollisionResolver *collision_resolver_create(void) {
	CollisionResolver *resolver = memory_manager.malloc(sizeof(CollisionResolver), MMTAG_COLLISION_INDEX);
...
EseArray *collision_resolver_solve(CollisionResolver *resolver, EseArray *pairs, EseLuaEngine *engine) {
    ...
    for (size_t i = 0; i < array_size(pairs); i++) {
        ...
        bool aabb_overlap = has_bounds ? ese_rect_intersects(a->collision_world_bounds, b->collision_world_bounds) : false;
        ...
        if (!aabb_overlap) {
            ...
            // If it's a LEAVE state, we still need to create a hit for the exit callback
            if (state == COLLISION_STATE_LEAVE) {
                EseCollisionHit *exit_hit = ese_collision_hit_create(engine);
                EseCollisionKind kind = _pair_involves_map_collision(a, b) ? COLLISION_KIND_MAP : COLLISION_KIND_COLLIDER;
        ...
        if (currently_colliding) {
            ...
            SpatialPair *pair_data = memory_manager.malloc(sizeof(SpatialPair), MMTAG_COLLISION_INDEX);
            pair_data->a = a;
            pair_data->b = b;
        ...
        if (tmp_hits) array_destroy(tmp_hits);
        memory_manager.free((void*)canonical_key);
    }
    

    // Process exit collisions for entities that were colliding but are no longer in pairs
    ...
    while (hashmap_iter_next(prev_iter, &key, &value)) {
        if (hashmap_get(current_collisions, key) == NULL) {
            // This pair was colliding before but is not in current pairs -> EXIT
