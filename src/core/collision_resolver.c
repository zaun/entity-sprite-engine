...
struct CollisionResolver {
  ...
  EseHashMap
      *previous_collisions; // canonical keys seen colliding in previous solve
};
...
static bool _get_map_pair_entities(EseEntity *a, EseEntity *b,
                                   EseEntity **out_collider,
                                   EseEntity **out_map) {
...
CollisionResolver *collision_resolver_create(void) {
  ...
  resolver->previous_collisions =
      hashmap_create((void (*)(void *))memory_manager.free);
  return resolver;
}

void collision_resolver_destroy(CollisionResolver *resolver) {
  log_assert("COLLISION_RESOLVER", resolver,
             "destroy called with NULL resolver");
...
EseArray *collision_resolver_solve(CollisionResolver *resolver, EseArray *pairs,
  ...
  for (size_t i = 0; i < array_size(pairs); i++) {
    ...
    if (!aabb_overlap) {
      ...
      profile_stop(PROFILE_ENTITY_COMPONENT_DISPATCH,
                   "entity_component_pair_dispatch");
  ...
  // Process exit collisions for entities that were colliding but are no longer
  // in pairs
  EseHashMapIter *prev_iter =
      hashmap_iter_create(resolver->previous_collisions);
  const char *key;
  void *value;
