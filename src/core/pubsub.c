...
static EseArray *_ese_get_or_create_topic_subscriptions(EsePubSub *pub_sub,
                                                        const char *name);
...
EsePubSub *ese_pubsub_create(void) {
  ...
  log_assert("pub_sub", pub_sub->topics != NULL,
             "Failed to create topics hashmap");
...
static EseArray *_ese_get_or_create_topic_subscriptions(EsePubSub *pub_sub,
  ...
  if (subscriptions == NULL) {
    ...
    log_assert("pub_sub", subscriptions != NULL,
               "Failed to create subscriptions array");
...
void ese_pubsub_sub(EsePubSub *pub_sub, const char *name, EseEntity *entity,
  ...
  EseArray *subscriptions =
      _ese_get_or_create_topic_subscriptions(pub_sub, name);

  EseSubscription *subscription =
      memory_manager.malloc(sizeof(EseSubscription), MMTAG_PUB_SUB);
  log_assert("pub_sub", subscription != NULL,
             "Failed to allocate subscription");

  subscription->entity = entity;
  subscription->function_name =
      memory_manager.strdup(function_name, MMTAG_PUB_SUB);
...
void ese_pubsub_unsub(EsePubSub *pub_sub, const char *name, EseEntity *entity,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (subscription != NULL && subscription->entity == entity &&
      ...
      // Remove from array; array_remove_at will free the item if a free_fn was
      // set. Since the array was created with free_fn = NULL, free explicitly
      // here.
