 ...
 *
 * @brief This file contains the private declarations for the EseEngine struct
 * and its internal helper functions.
 *
 * @details These declarations are not intended for public use and provide the
 * internal implementation details for the engine's core functionality, such as
 * managing render lists and entities.
 */
 ...
 *
 * @details This private function determines which of the two render lists is
 * currently active and clears its contents, preparing it for the next frame's
 * drawing commands.
 *
 ...
 *
 * @details This private function is a callback used to add a new render object
 * with specified position, texture, and z-index to the currently active render
 * list.
 *
 ...
 * @param texture_x1 The x1-coordinate of the texture on the sprite sheet
 * (normalized).
 * @param texture_y2 The y1-coordinate of the texture on the sprite sheet
 * (normalized).
 * @param texture_x1 The x2-coordinate of the texture on the sprite sheet
 * (normalized).
 * @param texture_y2 The y2-coordinate of the texture on the sprite sheet
 * (normalized).
 * @param width
 * @param
 ...
 *
 * @details This private function flips the active render list flag and sets the
 * renderer to use the newly active list, effectively preparing for the next
 * frame while the previous one is being rendered.
 *
 * @param engine A pointer to the EseEngine instance.
 ...
 *
 * @details This private function returns a pointer to the render list that is
 * currently being populated with draw commands.
 *
 * @param engine A pointer to the EseEngine instance.
 ...
 *
 * @details This private function creates a new entity, initializes it, and
 * appends it to the engine's internal list of entities.
 *
 * @param engine A pointer to the EseEngine instance.
 ...
 *
 * @note This function asserts that the engine pointer is not NULL.
 * @warning The `id` string is used to find the entity later, so it must be
 * unique. The new entity is appended to the engine's `entities` list and will
 * be owned by the engine.
 */
 ...
 *
 * @details This private function searches through the engine's entity list to
 * find an entity with a matching string ID.
 *
 * @param engine A pointer to the EseEngine instance.
 * @param id The string ID of the entity to find.
 * @return A pointer to the found EseEntity, or NULL if no entity with the given
 * ID is found.
 *
 * @note This function asserts that both the engine and id pointers are not
 * NULL.
 */
