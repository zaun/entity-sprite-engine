...
typedef struct EseJob {
    ...
    main_thread_job_callback callback; /** Function to execute on main thread when job completes */
    main_thread_job_cleanup cleanup;   /** Function to execute on main thread when job destroyed */
...
typedef struct EseJobQueue {
    ...
    worker_init_function init_fn;     /** Function to run on worker thread initialization */
    worker_deinit_function deinit_fn; /** Function to run on worker thread deinitialization */
} EseJobQueue;
 ...
 */
static EseJob *_create_job(EseJobQueue *q, ese_worker_id_t worker_id, worker_thread_job_function fn,
    ...
    for (;;) {
        ...
        while (dlist_iter_next(it, &val)) {
            ...
            if (!potential_future_work && j->executor_worker == worker->id) {
                if (j->state == JOB_RESULTS_READY || j->state == JOB_RESULTS_PROCESSED) {
            ...
            if (!shutdown_mode && !picked) {
                bool eligible = (j->state == JOB_PENDING) && (j->target_worker == ESE_WORKER_ANY ||
        ...
        if (!is_canceled_local && picked->fn) {
            picked->worker_result =
                picked->fn(worker->thread_data, picked->user_data, &is_canceled_local);
        }
 ...
 * @return ese_job_id_t Job ID or ESE_JOB_NOT_QUEUED on failure
 */
ese_job_id_t ese_job_queue_push_on_worker(EseJobQueue *q, ese_worker_id_t worker_id,
...
int32_t ese_job_queue_status(EseJobQueue *q, ese_job_id_t job_id) {
    ...
    if (state == JOB_RESULTS_READY || state == JOB_RESULTS_PROCESSED || state == JOB_EXECUTED ||
        state == JOB_COMPLETED) {
...
int32_t ese_job_queue_wait_for_completion(EseJobQueue *q, ese_job_id_t job_id, size_t timeout_ms) {
    ...
    while (state_snapshot < JOB_RESULTS_READY && state_snapshot != JOB_CANCELED) {
        if (timeout_ms == 0) {
            ...
            rc = ese_cond_wait_timeout(q->global_cond, q->global_mutex, (int)timeout_ms);
            if (rc == 1) {
                ese_mutex_unlock(q->global_mutex);
        ...
        if (state == JOB_RESULTS_READY) {
            ...
            if (job->worker_result.result && job->worker_result.size > 0) {
                ...
                if (job->worker_result.copy_fn && job->worker_result.free_fn) {
                    ...
                    void *main_copy = memory_manager.malloc(job->worker_result.size, MMTAG_TEMP);
...
void ese_job_queue_destroy(EseJobQueue *q) {
    ...
    for (;;) {
        ...
        while (dlist_iter_next(it, &val)) {
            ...
            if (s == JOB_RESULTS_READY || s == JOB_RESULTS_PROCESSED || s == JOB_RUNNING) {
                need_workers = true;
                break;
            }
        }
