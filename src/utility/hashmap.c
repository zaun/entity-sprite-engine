...
static void hashmap_resize(EseHashMap *map) {
  ...
  EseHashNode **new_buckets =
      memory_manager.calloc(new_capacity, sizeof(EseHashNode *), MMTAG_HASHMAP);
...
size_t hashmap_size(EseHashMap *map) { return map ? map->size : 0; }
...
EseHashMapIter *hashmap_iter_create(EseHashMap *map) {
  ...
  EseHashMapIter *iter =
      memory_manager.malloc(sizeof(EseHashMapIter), MMTAG_HASHMAP);
  iter->map = map;
  iter->bucket = 0;
  iter->node = NULL;
  return iter;
