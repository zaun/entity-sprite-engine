...
static unsigned int _hash_grouped(const char* group, const char* id) {
    unsigned int h = 5381;
    if (group) while (*group) h = ((h << 5) + h) + (unsigned char)(*group++);
    if (id) while (*id) h = ((h << 5) + h) + (unsigned char)(*id++);
    return h;
}
...
void grouped_hashmap_set(EseGroupedHashMap* map, const char* group, const char* id, void* value) {
    ...
    while (node) {
        if (_key_equals(&node->key, group, id)) {
            if (map->free_fn) map->free_fn(node->value);
            node->value = value;
            return;
        }
        node = node->next;
...
void* grouped_hashmap_remove(EseGroupedHashMap* map, const char* group, const char* id) {
    ...
    log_error("HASHMAP", "Warning: grouped_hashmap_remove could not find group '%s', id '%s'", group, id);
    return NULL;
}
...
int grouped_hashmap_iter_next(EseGroupedHashMapIter* iter, const char** group, const char** id, void** value) {
    ...
    if (iter->node) {
        if (group) *group = iter->node->key.group;
        if (id) *id = iter->node->key.id;
