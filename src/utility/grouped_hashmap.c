...
static int _key_equals(const EseGroupedKey *a, const char *group, const char *id) {
    return strcmp(a->group, group) == 0 && strcmp(a->id, id) == 0;
}

static EseHashNode *_create_node(const char *group, const char *id, void *value) {
    if (!group || !id || !value) {
...
static void hashmap_resize(EseGroupedHashMap *map) {
    ...
    EseHashNode **new_buckets =
        memory_manager.calloc(new_capacity, sizeof(EseHashNode *), MMTAG_GROUP_HASHMAP);
    if (!new_buckets) {
        log_error("HASHMAP", "Error: memory_manager.calloc failed during resize to %zu",
                  new_capacity);
        return;
    }
    for (size_t i = 0; i < map->capacity; i++) {
        ...
        while (node) {
            ...
            unsigned int idx = _hash_grouped(node->key.group, node->key.id) % new_capacity;
...
void grouped_hashmap_set(EseGroupedHashMap *map, const char *group, const char *id, void *value) {
    ...
    if (!node) {
        log_error("HASHMAP", "Error: _create_node failed for group '%s', id '%s'", group, id);
        return;
    }
...
void *grouped_hashmap_remove(EseGroupedHashMap *map, const char *group, const char *id) {
    if (!map || !group || !id) {
        log_error("HASHMAP", "Error: grouped_hashmap_remove received null argument");
        return NULL;
    }
    unsigned int idx = _hash_grouped(group, id) % map->capacity;
    EseHashNode *node = map->buckets[idx];
    ...
    log_error("HASHMAP", "Warning: grouped_hashmap_remove could not find group '%s', id '%s'",
              group, id);
    return NULL;
}
...
void grouped_hashmap_remove_group(EseGroupedHashMap *map, const char *group) {
    if (!map || !group) {
        log_error("HASHMAP", "Error: grouped_hashmap_remove_group received null argument");
        return;
    }
    for (size_t i = 0; i < map->capacity; i++) {
        EseHashNode *node = map->buckets[i];
...
EseGroupedHashMapIter *grouped_hashmap_iter_create(EseGroupedHashMap *map) {
    if (!map) {
        log_error("HASHMAP", "Error: grouped_hashmap_iter_create received null map");
        return NULL;
    }
...
int grouped_hashmap_iter_next(EseGroupedHashMapIter *iter, const char **group, const char **id,
                              void **value) {
    if (!iter) {
