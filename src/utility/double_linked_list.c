...
static void _dlist_remove(EseDoubleLinkedList *list, EseDListNode *node) {
...
EseDoubleLinkedList *dlist_create(DListFreeFn free_fn) {
  EseDoubleLinkedList *list =
      memory_manager.malloc(sizeof(EseDoubleLinkedList), MMTAG_LINKED_LIST);
  list->head = NULL;
  list->tail = NULL;
  list->size = 0;
...
EseDoubleLinkedList *dlist_copy(EseDoubleLinkedList *list,
                                DListCopyFn copy_fn) {
...
void dlist_append(EseDoubleLinkedList *list, void *value) {
  ...
  EseDListNode *node =
      memory_manager.malloc(sizeof(EseDListNode), MMTAG_LINKED_LIST);
  node->value = value;
  node->prev = list->tail;
  node->next = NULL;
  if (list->tail)
...
EseDListIter *dlist_iter_create(EseDoubleLinkedList *list) {
  ...
  EseDListIter *iter =
      memory_manager.malloc(sizeof(EseDListIter), MMTAG_LINKED_LIST_ITER);
