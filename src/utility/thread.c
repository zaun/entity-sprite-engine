...
static BOOL CALLBACK ese_thread_init_slot_counter(PINIT_ONCE once, PVOID param, PVOID *ctx) {
    (void)once;
    (void)param;
    (void)ctx;
    /* start at 1 to reserve 0 for the main thread */
...
static int ese_thread_ensure_slot_counter(void) {
    BOOL ok = InitOnceExecuteOnce(&g_slot_once, ese_thread_init_slot_counter, NULL, NULL);
...
void ese_thread_detach(EseThread th) {
    ...
    /* Mark as detached; the trampoline will free the struct when the thread
     * exits
     */
    th->detached = 1;
}
...
bool ese_atomic_int_compare_exchange(EseAtomicInt *a, int *expected, int desired) {
    if (!a || !expected)
        return false;
#if defined(ESE_HAVE_C11_ATOMICS)
    return atomic_compare_exchange_strong_explicit(&a->v, expected, desired, memory_order_seq_cst,
    ...
    *expected = (int)old;
    return false;
#elif defined(ESE_HAVE_GNU_ATOMICS)
    return __atomic_compare_exchange_n(&a->v, expected, desired, false, __ATOMIC_SEQ_CST,
                                       __ATOMIC_SEQ_CST);
#else
    if (a->v == *expected) {
        a->v = desired;
...
#elif defined(ESE_HAVE_GNU_ATOMICS)
    return (size_t)__atomic_fetch_add(&a->v, (unsigned long long)v, __ATOMIC_SEQ_CST);
#else
...
bool ese_atomic_size_t_compare_exchange(EseAtomicSizeT *a, size_t *expected, size_t desired) {
    ...
    *expected = (size_t)old;
    ...
    bool ok = __atomic_compare_exchange_n(&a->v, &exp, (unsigned long long)desired, false,
