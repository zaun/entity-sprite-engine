...
static EseIntHashNode *create_int_node(uint64_t key, void *value) {
  ...
  EseIntHashNode *node =
      memory_manager.malloc(sizeof(EseIntHashNode), MMTAG_HASHMAP);
  node->key = key;
  node->value = value;
  node->next = NULL;
  return node;
...
static void int_hashmap_resize(EseIntHashMap *map) {
  ...
  EseIntHashNode **new_buckets = memory_manager.calloc(
      new_capacity, sizeof(EseIntHashNode *), MMTAG_HASHMAP);
...
EseIntHashMap *int_hashmap_create(EseIntHashMapFreeFn free_fn) {
  EseIntHashMap *map =
      memory_manager.malloc(sizeof(EseIntHashMap), MMTAG_HASHMAP);
  map->capacity = INITIAL_CAPACITY;
  map->size = 0;
  map->free_fn = free_fn;
  map->buckets = memory_manager.calloc(map->capacity, sizeof(EseIntHashNode *),
                                       MMTAG_HASHMAP);
  return map;
}
...
size_t int_hashmap_size(EseIntHashMap *map) { return map ? map->size : 0; }

EseIntHashMapIter *int_hashmap_iter_create(EseIntHashMap *map) {
  if (!map)
    return NULL;

  EseIntHashMapIter *iter =
      memory_manager.malloc(sizeof(EseIntHashMapIter), MMTAG_HASHMAP);
  iter->map = map;
  iter->bucket = 0;
  iter->node = NULL;
  return iter;
...
int int_hashmap_iter_next(EseIntHashMapIter *iter, uint64_t *key,
                          void **value) {
  if (!iter)
    return 0;
