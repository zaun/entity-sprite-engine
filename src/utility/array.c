...
EseArray *array_create(size_t initial_capacity, ArrayFreeFn free_fn) {
  ...
  array->elements =
      memory_manager.malloc(sizeof(void *) * initial_capacity, MMTAG_ARRAY);
  if (!array->elements) {
  ...
  log_verbose("ARRAY", "array_create %p capacity=%zu", (void *)array,
              initial_capacity);
  return array;
}
...
void array_destroy(EseArray *array) {
  ...
  log_verbose("ARRAY", "array_destroy %p", (void *)array);
  memory_manager.free(array);
}

static bool array_resize(EseArray *array, size_t new_capacity) {
  void **new_elements = memory_manager.realloc(
      array->elements, sizeof(void *) * new_capacity, MMTAG_ARRAY);
  if (!new_elements)
    return false;
