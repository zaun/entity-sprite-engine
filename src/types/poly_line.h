 ...
 * @details Provides polyline operations, point management, and JSON
 * serialization
 *
...
/**
 * @brief Represents a polyline with points, stroke properties, and fill
 * properties.
 *
 ...
 *
 * @details Allocates memory for a new EsePolyLine and initializes to empty
 * state. The polyline is created without Lua references and must be explicitly
 ...
 *
 * @warning The returned EsePolyLine must be freed with ese_poly_line_destroy()
 ...
 *
 * @details This function creates a deep copy of an EsePolyLine object. It
 * allocates a new EsePolyLine struct and copies all members including the
 * points collection. The copy is created without Lua references and must be
 * explicitly referenced with ese_poly_line_ref() if Lua access is desired.
 *
...
/**
 * @brief Destroys a EsePolyLine object, managing memory based on Lua
 * references.
 *
 * @details If the polyline has no Lua references (lua_ref == LUA_NOREF), frees
 * memory immediately. If the polyline has Lua references, decrements the
 ...
 *
 * @details Returns a pointer to the internal float array containing point
 * coordinates. The array contains x,y pairs for each point (x1, y1, x2, y2,
...
/**
 * @brief Adds a watcher callback to be notified when any polyline property
 * changes.
 *
 * @details The callback will be called whenever any property of the polyline is
 * modified. Multiple watchers can be registered on the same polyline.
 *
 ...
 *
 * @details Creates and registers the "PolyLineProxyMeta" metatable with
 * __index, __newindex,
 *          __gc, __tostring metamethods for property access and garbage
 * collection. This allows EsePolyLine objects to be used naturally from Lua
 * with dot notation. Also creates the global "PolyLine" table with "new"
 * constructor.
 *
 ...
 *
 * @details If the polyline has no Lua references (lua_ref == LUA_NOREF),
 * creates a new proxy table. If the polyline has Lua references, retrieves the
 * existing proxy table from the registry.
 *
...
/**
 * @brief Extracts a EsePolyLine pointer from a Lua userdata object with type
 * safety.
 *
 ...
 *          type checking to ensure the object is a valid EsePolyLine proxy
 * table with the correct metatable and userdata pointer.
 *
 * @param L Lua state pointer
 * @param idx Stack index of the Lua EsePolyLine object
 * @return Pointer to the EsePolyLine object, or NULL if extraction fails or
 * type check fails
 *
...
/**
 * @brief References a EsePolyLine object for Lua access with reference
 * counting.
 *
 * @details If poly_line->lua_ref is LUA_NOREF, pushes the polyline to Lua and
 * references it, setting lua_ref_count to 1. If poly_line->lua_ref is already
 * set, increments the reference count by 1. This prevents the polyline from
 ...
 *
 * @details Creates a cJSON object representing the polyline with type
 * "POLY_LINE" and all properties including points, colors, and styling. Only
 * serializes the geometric and styling data, not Lua-related fields.
 *
 ...
 *          and all properties including points, colors, and styling. The
 * polyline is created with the specified engine and must be explicitly
 * referenced with ese_poly_line_ref() if Lua access is desired.
 *
