...
static void _ese_display_set_lua_ref_count(EseDisplay *display,
                                           int lua_ref_count);
 ...
 *
 * Allocates memory for a new EseDisplay and initializes all fields to safe
 * defaults. The display starts in windowed mode with zero dimensions, 1.0
 * aspect ratio, and no Lua state or references.
 *
 * @return Pointer to the newly created EseDisplay, or NULL on allocation
 * failure
 */
static EseDisplay *_ese_display_make() {
  EseDisplay *display =
      (EseDisplay *)memory_manager.malloc(sizeof(EseDisplay), MMTAG_DISPLAY);
...
EseDisplay *ese_display_create(EseLuaEngine *engine) {
  log_assert("DISPLAY_STATE", engine,
             "ese_display_create called with NULL engine");
  log_assert("DISPLAY_STATE", memory_manager.malloc,
             "memory_manager.malloc is NULL");

  EseDisplay *display = _ese_display_make();
  log_assert("DISPLAY_STATE", display,
             "ese_display_create failed to allocate memory");
...
EseDisplay *ese_display_copy(const EseDisplay *src) {
  ...
  EseDisplay *copy =
      (EseDisplay *)memory_manager.malloc(sizeof(EseDisplay), MMTAG_DISPLAY);
  log_assert("DISPLAY_STATE", copy,
             "ese_display_copy failed to allocate memory");
...
void ese_display_lua_push(EseDisplay *display) {
  log_assert("DISPLAY", display,
             "ese_display_lua_push called with NULL display");

  if (ese_display_get_lua_ref(display) == LUA_NOREF) {
    // Lua-owned: create a new userdata
    EseDisplay **ud = (EseDisplay **)lua_newuserdata(
        ese_display_get_state(display), sizeof(EseDisplay *));
    *ud = display;
    ...
    lua_rawgeti(ese_display_get_state(display), LUA_REGISTRYINDEX,
                ese_display_get_lua_ref(display));
  }
}
...
void ese_display_ref(EseDisplay *display) {
  ...
  if (ese_display_get_lua_ref(display) == LUA_NOREF) {
    ...
    *ud = display;
    ...
    _ese_display_set_lua_ref(
        display, luaL_ref(ese_display_get_state(display), LUA_REGISTRYINDEX));
    _ese_display_set_lua_ref_count(display, 1);
  } else {
    // Already referenced - just increment count
    _ese_display_set_lua_ref_count(display,
                                   ese_display_get_lua_ref_count(display) + 1);
  }
...
void ese_display_unref(EseDisplay *display) {
  ...
  if (ese_display_get_lua_ref(display) != LUA_NOREF &&
      ese_display_get_lua_ref_count(display) > 0) {
    _ese_display_set_lua_ref_count(display,
                                   ese_display_get_lua_ref_count(display) - 1);

    if (ese_display_get_lua_ref_count(display) == 0) {
      // No more references - remove from registry
      luaL_unref(ese_display_get_state(display), LUA_REGISTRYINDEX,
                 ese_display_get_lua_ref(display));
      _ese_display_set_lua_ref(display, LUA_NOREF);
    }
  }
...
void ese_display_set_dimensions(EseDisplay *display, int width, int height) {
  log_assert("DISPLAY_STATE", display,
             "ese_display_set_dimensions called with NULL display");
...
void ese_display_set_fullscreen(EseDisplay *display, bool fullscreen) {
  log_assert("DISPLAY_STATE", display,
             "ese_display_set_fullscreen called with NULL display");
  display->fullscreen = fullscreen;
}

void ese_display_set_viewport(EseDisplay *display, int width, int height) {
  log_assert("DISPLAY_STATE", display,
             "ese_display_set_viewport called with NULL display");
