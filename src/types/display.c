...
// Lua integration
void ese_display_lua_init(EseLuaEngine *engine) { _ese_display_lua_init(engine); }
...
void ese_display_lua_push(EseDisplay *display) {
    ...
    if (ese_display_get_lua_ref(display) == LUA_NOREF) {
        ...
        EseDisplay **ud =
            (EseDisplay **)lua_newuserdata(ese_display_get_state(display), sizeof(EseDisplay *));
        *ud = display;
...
void ese_display_ref(EseDisplay *display) {
    ...
    if (ese_display_get_lua_ref(display) == LUA_NOREF) {
        ...
        *ud = display;
        ...
        _ese_display_set_lua_ref(display,
                                 luaL_ref(ese_display_get_state(display), LUA_REGISTRYINDEX));
