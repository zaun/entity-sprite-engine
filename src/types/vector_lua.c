...
static int _ese_vector_lua_gc(lua_State *L) {
  ...
  if (vector) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this vector was referenced from C and should not
    // be freed.
...
static int _ese_vector_lua_new(lua_State *L) {
  ...
  if (lua_type(L, 1) != LUA_TNUMBER) {
    profile_cancel(PROFILE_LUA_VECTOR_NEW);
    return luaL_error(L,
                      "Vector.new(number, number) arguments must be numbers");
  }
...
static int _ese_vector_lua_set_direction(lua_State *L) {
  ...
  if (n_args == 3) {
    if (lua_type(L, 2) != LUA_TSTRING || lua_type(L, 3) != LUA_TNUMBER) {
      return luaL_error(
          L, "vector:magnitude(string, number) takes a string and a number");
    }
  } else {
    return luaL_error(L,
                      "vector:set_direction(string, number) takes 2 arguments");
  }
