 ...
 *
 * Provides read access to arc properties (x, y, radius, start_angle, end_angle)
 * from Lua. When a Lua script accesses arc.x, arc.y, etc., this function is
 * called to retrieve the values. Also provides access to methods like
 * contains_point, intersects_rect, get_length, and get_point_at_angle.
 *
 * @param L Lua state
 ...
 *
 * Provides write access to arc properties (x, y, radius, start_angle,
 * end_angle) from Lua. When a Lua script assigns to arc.x, arc.y, etc., this
 * function is called to update the values.
 *
 * @param L Lua state
 ...
 * The format includes the memory address and current
 * x,y,radius,start_angle,end_angle values.
 *
 * @param L Lua state
 ...
 *
 * Creates a new EseArc from Lua with specified center, radius, and angle
 * parameters. This function is called when Lua code executes `Arc.new(x, y,
 * radius, start_angle, end_angle)`. It validates the arguments, creates the
 * underlying EseArc, and returns a proxy table that provides access to the
 * arc's properties and methods.
 *
 * @param L Lua state
 ...
 *
 * Creates a new EseArc at the origin (0,0) with unit radius and full circle
 * angles from Lua. This function is called when Lua code executes `Arc.zero()`.
 * It's a convenience constructor for creating arcs at the default position and
 * parameters.
 *
 * @param L Lua state
 * @return Number of values pushed onto the stack (always 1 - the proxy table)
 ...
 *
 * Calculates and returns the arc length using the formula: radius *
 * angle_difference. Handles angle wrapping automatically.
 *
 * @param L Lua state
 ...
 *
 * Calculates the coordinates of a point that lies along the arc at the given
 * angle. Returns success status and x,y coordinates if the angle is within the
 * arc's range.
 *
 * @param L Lua state
 * @return Number of values pushed onto the stack (3 for success: true, x, y; 1
 * for failure: false)
 */
static int _ese_arc_lua_get_point_at_angle(lua_State *L) {
EseArc *arc = (EseArc *)lua_engine_instance_method_normalize(
 ...
 *
 * Converts an EseArc to a JSON string representation. This function is called
 * when Lua code executes `arc:toJSON()`. It serializes the arc's fields to JSON
 ...
 *
 * Creates a new EseArc from a JSON string. Called as
