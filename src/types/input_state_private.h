 ...
 * The public API in input_state.h provides read-only access through getter
 * functions.
 */
...
#define MOUSE_BUTTON_COUNT                                                     \
  8 /** The total number of mouse buttons supported.                           \
     */
...
struct EseInputState {
  ...
  bool mouse_down[InputMouse_MAX];     /** State of mouse buttons currently held
                                          down. */
  bool mouse_clicked[InputMouse_MAX];  /** Mouse buttons that were pressed this
                                          frame. */
  bool mouse_released[InputMouse_MAX]; /** Mouse buttons that were released this
                                          frame. */

  lua_State *state;  /** A pointer to the Lua state. */
  int lua_ref;       /** A reference to the Lua userdata object . */
  int lua_ref_count; /** Number of times this input state has been referenced in
                        C */
};
