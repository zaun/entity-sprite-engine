...
static int _ese_ray_lua_tostring(lua_State *L) {
    ...
    snprintf(buf, sizeof(buf), "Ray: %p (x=%.2f, y=%.2f, dx=%.2f, dy=%.2f)", 
             (void*)ray, ese_ray_get_x(ray), ese_ray_get_y(ray), ese_ray_get_dx(ray), ese_ray_get_dy(ray));
...
void _ese_ray_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, RAY_PROXY_META, 
        _ese_ray_lua_index, 
        _ese_ray_lua_newindex, 
        _ese_ray_lua_gc, 
