...
static int _ese_ray_lua_gc(lua_State *L) {
    ...
    if (ray) {
        ...
        // so we can free it.
        // If lua_ref != LUA_NOREF, this ray was referenced from C and should
        // not be freed.
...
static int _ese_ray_lua_tostring(lua_State *L) {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "Ray: %p (x=%.2f, y=%.2f, dx=%.2f, dy=%.2f)", (void *)ray,
             ese_ray_get_x(ray), ese_ray_get_y(ray), ese_ray_get_dx(ray), ese_ray_get_dy(ray));
...
void _ese_ray_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, RAY_PROXY_META, _ese_ray_lua_index, _ese_ray_lua_newindex,
                               _ese_ray_lua_gc, _ese_ray_lua_tostring);

    // Create global Ray table with functions
    const char *keys[] = {"new", "zero", "fromJSON"};
    lua_CFunction functions[] = {_ese_ray_lua_new, _ese_ray_lua_zero, _ese_ray_lua_from_json};
