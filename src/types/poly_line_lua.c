...
static int _ese_poly_line_lua_tostring(lua_State *L) {
    ...
    snprintf(buf, sizeof(buf), "PolyLine: %p (type=%s, points=%zu, stroke_width=%.2f)", 
             (void*)poly_line, type_str, ese_poly_line_get_point_count(poly_line), ese_poly_line_get_stroke_width(poly_line));
...
static int _ese_poly_line_lua_get_point(lua_State *L) {
    ...
    log_error("POLY_LINE", "get_point index=%zu x=%.2f y=%.2f count=%zu", index, x, y, ese_poly_line_get_point_count(poly_line));

    EseLuaEngine *engine = (EseLuaEngine *)lua_engine_get_registry_key(ese_poly_line_get_state(poly_line), LUA_ENGINE_KEY);
 ...
 * 
 * Sets up the Lua metatable and global PolyLine table with constructors and methods.
 * This function is called by the public ese_poly_line_lua_init function.
 * 
