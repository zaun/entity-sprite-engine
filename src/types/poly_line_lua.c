...
static int _ese_poly_line_lua_gc(lua_State *L) {
  ...
  if (poly_line) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this polyline was referenced from C and should
    // not be freed.
...
static int _ese_poly_line_lua_newindex(lua_State *L) {
  ...
  if (strcmp(key, "type") == 0) {
    ...
    profile_stop(PROFILE_LUA_POLY_LINE_NEWINDEX,
                 "poly_line_lua_newindex (setter)");
    return 0;
  } else if (strcmp(key, "stroke_width") == 0) {
    if (!lua_isnumber(L, 3)) {
      profile_cancel(PROFILE_LUA_POLY_LINE_NEWINDEX);
    ...
    if (vtype == LUA_TNIL || vtype == LUA_TNONE) {
      EseColor *current_stroke_color =
          ese_poly_line_get_stroke_color(poly_line);
  ...
  profile_stop(PROFILE_LUA_POLY_LINE_NEWINDEX,
               "poly_line_lua_newindex (invalid)");
...
static int _ese_poly_line_lua_tostring(lua_State *L) {
  ...
  char buf[256];
  snprintf(
      buf, sizeof(buf), "PolyLine: %p (type=%s, points=%zu, stroke_width=%.2f)",
...
static int _ese_poly_line_lua_remove_point(lua_State *L) {
  ...
  profile_stop(PROFILE_LUA_POLY_LINE_REMOVE_POINT,
               "poly_line_lua_remove_point");
  return 0;
}
...
static int _ese_poly_line_lua_get_point(lua_State *L) {
  ...
  log_error("POLY_LINE", "get_point index=%zu x=%.2f y=%.2f count=%zu", index,
            x, y, ese_poly_line_get_point_count(poly_line));
...
static int _ese_poly_line_lua_get_point_count(lua_State *L) {
  ...
  profile_stop(PROFILE_LUA_POLY_LINE_GET_POINT_COUNT,
               "poly_line_lua_get_point_count");
  return 1;
}
...
static int _ese_poly_line_lua_clear_points(lua_State *L) {
  ...
  profile_stop(PROFILE_LUA_POLY_LINE_CLEAR_POINTS,
               "poly_line_lua_clear_points");
  return 0;
}
...
void _ese_poly_line_lua_init(EseLuaEngine *engine) {
  ...
  lua_CFunction functions[] = {_ese_poly_line_lua_new,
                               _ese_poly_line_lua_from_json};
