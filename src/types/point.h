 ...
 *
 * @details This structure stores the x and y coordinates of a point in 2D
 * space.
 */
 ...
 *
 * @warning The returned EsePoint must be freed with ese_point_destroy() to
 ...
 *
 * @details This function creates a deep copy of an EsePoint object. It
 * allocates a new EsePoint struct and copies all numeric members. The copy is
 * created without Lua references and must be explicitly referenced with
 * ese_point_ref() if Lua access is desired.
 *
 ...
 *
 * @details If the point has no Lua references (lua_ref == LUA_NOREF), frees
 * memory immediately. If the point has Lua references, decrements the reference
...
/**
 * @brief Adds a watcher callback to be notified when any point property
 * changes.
 *
 * @details The callback will be called whenever any property (x, y) of the
 * point is modified. Multiple watchers can be registered on the same point.
 *
 ...
 *
 * @details Creates and registers the "PointProxyMeta" metatable with __index,
 * __newindex,
 *          __gc, __tostring metamethods for property access and garbage
 * collection. This allows EsePoint objects to be used naturally from Lua with
 * dot notation. Also creates the global "Point" table with "new" and "zero"
 * constructors.
 *
 ...
 *
 * @details If the point has no Lua references (lua_ref == LUA_NOREF), creates a
 * new proxy table. If the point has Lua references, retrieves the existing
...
/**
 * @brief Extracts a EsePoint pointer from a Lua userdata object with type
 * safety.
 *
 ...
 * @return Pointer to the EsePoint object, or NULL if extraction fails or type
 * check fails
 *
 ...
 *
 * @details If point->lua_ref is LUA_NOREF, pushes the point to Lua and
 * references it, setting lua_ref_count to 1. If point->lua_ref is already set,
 * increments the reference count by 1. This prevents the point from being
 ...
 *          and must be explicitly referenced with ese_point_ref() if Lua access
 * is desired.
 *
