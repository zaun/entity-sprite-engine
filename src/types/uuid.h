 ...
 *
 * @warning The returned EseUUID must be freed with ese_uuid_destroy() to
 ...
 *
 * @details This function creates a deep copy of an EseUUID object. It allocates
 * a new EseUUID struct and copies all string members. The copy is created
 * without Lua references and must be explicitly referenced with ese_uuid_ref()
 * if Lua access is desired.
 *
 ...
 *
 * @details If the uuid has no Lua references (lua_ref == LUA_NOREF), frees
 * memory immediately. If the uuid has Lua references, decrements the reference
 ...
 * functions. Allocates memory for a new EseUUID and initializes all fields to
 * safe defaults. The UUID starts with no Lua state or references, and a new
 ...
 *
 * @details Creates and registers the "UUIDProxyMeta" metatable with __index,
 * __newindex,
 *          __gc, __tostring metamethods for property access and garbage
 * collection. This allows EseUUID objects to be used naturally from Lua with
 * dot notation. Also creates the global "UUID" table with "new" constructor.
 *
 ...
 *
 * @details If the uuid has no Lua references (lua_ref == LUA_NOREF), creates a
 * new proxy table. If the uuid has Lua references, retrieves the existing proxy
...
/**
 * @brief Extracts a EseUUID pointer from a Lua userdata object with type
 * safety.
 *
 * @details Retrieves the C EseUUID pointer from the "__ptr" field of a Lua
 *          table that was created by ese_uuid_lua_push(). Performs type
 * checking to ensure the object is a valid EseUUID proxy table.
 *
 * @param L Lua state pointer
 * @param idx Stack index of the Lua EseUUID object
 * @return Pointer to the EseUUID object, or NULL if extraction fails or type
 * check fails
 *
 ...
 *
 * @details If uuid->lua_ref is LUA_NOREF, pushes the uuid to Lua and references
 * it, setting lua_ref_count to 1. If uuid->lua_ref is already set, increments
 ...
 *          and must be explicitly referenced with ese_uuid_ref() if Lua access
 * is desired.
 *
