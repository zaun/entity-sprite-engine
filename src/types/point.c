 ...
 * @details Implements point operations, watcher system, Lua integration, and
 * JSON serialization
 *
 ...
 *
 * Allocates memory for a new EsePoint and initializes all fields to safe
 * defaults. The point starts at origin (0,0) with no Lua state or watchers.
 *
 * @return Pointer to the newly created EsePoint, or NULL on allocation failure
 */
EsePoint *_ese_point_make() {
  EsePoint *point =
      (EsePoint *)memory_manager.malloc(sizeof(EsePoint), MMTAG_POINT);
  point->x = 0.0f;
  point->y = 0.0f;
  point->state = NULL;
...
void _ese_point_make_point_notify_watchers(EsePoint *point) {
  if (!point || point->watcher_count == 0)
    return;
...
void ese_point_lua_push(EsePoint *point) {
  ...
  if (ese_point_get_lua_ref(point) == LUA_NOREF) {
    ...
    EsePoint **ud = (EsePoint **)lua_newuserdata(ese_point_get_state(point),
                                                 sizeof(EsePoint *));
    *ud = point;
    ...
    lua_rawgeti(ese_point_get_state(point), LUA_REGISTRYINDEX,
                ese_point_get_lua_ref(point));
  }
}
...
void ese_point_ref(EsePoint *point) {
  ...
  if (ese_point_get_lua_ref(point) == LUA_NOREF) {
    ...
    *ud = point;
    ...
    _ese_point_set_lua_ref(
        point, luaL_ref(ese_point_get_state(point), LUA_REGISTRYINDEX));
...
void ese_point_unref(EsePoint *point) {
  ...
  if (ese_point_get_lua_ref(point) != LUA_NOREF &&
      ese_point_get_lua_ref_count(point) > 0) {
    _ese_point_set_lua_ref_count(point, ese_point_get_lua_ref_count(point) - 1);

    if (ese_point_get_lua_ref_count(point) == 0) {
      // No more references - remove from registry
      luaL_unref(ese_point_get_state(point), LUA_REGISTRYINDEX,
                 ese_point_get_lua_ref(point));
      _ese_point_set_lua_ref(point, LUA_NOREF);
    }
  }
...
float ese_point_distance(const EsePoint *point1, const EsePoint *point2) {
  log_assert("POINT", point1,
             "ese_point_distance called with NULL first point");
  log_assert("POINT", point2,
             "ese_point_distance called with NULL second point");
...
float ese_point_distance_squared(const EsePoint *point1,
  ...
  log_assert("POINT", point1,
             "ese_point_distance_squared called with NULL first point");
  log_assert("POINT", point2,
             "ese_point_distance_squared called with NULL second point");
...
int ese_point_get_lua_ref_count(const EsePoint *point) {
  log_assert("POINT", point,
             "ese_point_get_lua_ref_count called with NULL point");
...
bool ese_point_add_watcher(EsePoint *point, EsePointWatcherCallback callback,
  ...
  log_assert("POINT", callback,
             "ese_point_add_watcher called with NULL callback");
  ...
  if (point->watcher_count == 0) {
    ...
    point->watchers = memory_manager.malloc(
        sizeof(EsePointWatcherCallback) * point->watcher_capacity, MMTAG_POINT);
    point->watcher_userdata = memory_manager.malloc(
        sizeof(void *) * point->watcher_capacity, MMTAG_POINT);
    point->watcher_count = 0;
  }
  ...
  if (point->watcher_count >= point->watcher_capacity) {
    ...
    EsePointWatcherCallback *new_watchers = memory_manager.realloc(
        point->watchers, sizeof(EsePointWatcherCallback) * new_capacity,
        MMTAG_POINT);
    void **new_userdata = memory_manager.realloc(
        point->watcher_userdata, sizeof(void *) * new_capacity, MMTAG_POINT);
...
bool ese_point_remove_watcher(EsePoint *point, EsePointWatcherCallback callback,
  ...
  log_assert("POINT", callback,
             "ese_point_remove_watcher called with NULL callback");

  for (size_t i = 0; i < point->watcher_count; i++) {
    if (point->watchers[i] == callback &&
        point->watcher_userdata[i] == userdata) {
...
size_t ese_point_sizeof(void) { return sizeof(EsePoint); }
 ...
 * and must be explicitly referenced with ese_point_ref() if Lua access is
 * desired.
 *
...
EsePoint *ese_point_deserialize(EseLuaEngine *engine, const cJSON *data) {
  ...
  if (!cJSON_IsObject(data)) {
    log_error("POINT",
              "Point deserialization failed: data is not a JSON object");
    return NULL;
  }
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
      strcmp(type_item->valuestring, "POINT") != 0) {
    log_error("POINT",
              "Point deserialization failed: invalid or missing type field");
    return NULL;
  }
  ...
  if (!x_item || !cJSON_IsNumber(x_item)) {
    log_error("POINT",
              "Point deserialization failed: invalid or missing x field");
    return NULL;
  }
  ...
  if (!y_item || !cJSON_IsNumber(y_item)) {
    log_error("POINT",
              "Point deserialization failed: invalid or missing y field");
    return NULL;
  }
