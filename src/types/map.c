...
bool _allocate_cells_array(EseMap *map) {
  if (!map || map->width == 0 || map->height == 0 || !ese_map_get_engine(map))
    return false;
  ...
  for (uint32_t y = 0; y < map->height; y++) {
    ...
    for (uint32_t x = 0; x < map->width; x++) {
      EseMapCell *cell =
          ese_map_cell_create((EseLuaEngine *)ese_map_get_engine(map), map);
      map->cells[y][x] = cell;
...
void ese_map_lua_push(EseMap *map) {
  ...
  if (ese_map_get_lua_ref(map) == LUA_NOREF) {
    ...
    EseMap **ud =
        (EseMap **)lua_newuserdata(ese_map_get_state(map), sizeof(EseMap *));
    *ud = map;
    ...
    lua_rawgeti(ese_map_get_state(map), LUA_REGISTRYINDEX,
                ese_map_get_lua_ref(map));
  }
}
...
void ese_map_destroy(EseMap *map) {
  ...
  if (map->tileset)
    ese_tileset_destroy(map->tileset);
  if (map->title)
    memory_manager.free(map->title);
  if (map->author)
    memory_manager.free(map->author);
  if (map->watchers)
    memory_manager.free(map->watchers);
  if (map->watcher_userdata)
    memory_manager.free(map->watcher_userdata);
...
void ese_map_ref(EseMap *map) {
  ...
  if (ese_map_get_lua_ref(map) == LUA_NOREF) {
    ...
    *ud = map;
    ...
    _ese_map_set_lua_ref(map,
                         luaL_ref(ese_map_get_state(map), LUA_REGISTRYINDEX));
...
void ese_map_unref(EseMap *map) {
  ...
  if (ese_map_get_lua_ref(map) != LUA_NOREF &&
      ese_map_get_lua_ref_count(map) > 0) {
    _ese_map_set_lua_ref_count(map, ese_map_get_lua_ref_count(map) - 1);

    if (ese_map_get_lua_ref_count(map) == 0) {
      // No more references - remove from registry
      luaL_unref(ese_map_get_state(map), LUA_REGISTRYINDEX,
                 ese_map_get_lua_ref(map));
      _ese_map_set_lua_ref(map, LUA_NOREF);
    }
  }
  ...
  for (uint32_t y = 0; y < copy_height; y++) {
    for (uint32_t x = 0; x < copy_width; x++) {
      if (old_cells[y][x]) {
        ...
        ese_map_cell_add_watcher(map->cells[y][x], _ese_map_on_cell_changed,
                                 map);
      }
    }
  }
...
bool ese_map_add_watcher(EseMap *map, EseMapWatcherCallback callback,
  ...
  if (map->watcher_count == 0) {
    ...
    map->watchers = memory_manager.malloc(
        sizeof(EseMapWatcherCallback) * map->watcher_capacity, MMTAG_MAP);
    map->watcher_userdata = memory_manager.malloc(
        sizeof(void *) * map->watcher_capacity, MMTAG_MAP);
    map->watcher_count = 0;
  }
  ...
  if (map->watcher_count >= map->watcher_capacity) {
    ...
    EseMapWatcherCallback *new_watchers = memory_manager.realloc(
        map->watchers, sizeof(EseMapWatcherCallback) * new_capacity, MMTAG_MAP);
    void **new_userdata = memory_manager.realloc(
        map->watcher_userdata, sizeof(void *) * new_capacity, MMTAG_MAP);
...
bool ese_map_remove_watcher(EseMap *map, EseMapWatcherCallback callback,
  ...
  log_assert("MAP", callback,
             "ese_map_remove_watcher called with NULL callback");
...
EseMapType ese_map_type_from_string(const char *type_str) {
  ...
  if (strcmp(type_str, "grid") == 0)
    return MAP_TYPE_GRID;
  if (strcmp(type_str, "hex_point_up") == 0)
    return MAP_TYPE_HEX_POINT_UP;
  if (strcmp(type_str, "hex_flat_up") == 0)
    return MAP_TYPE_HEX_FLAT_UP;
  if (strcmp(type_str, "iso") == 0)
    return MAP_TYPE_ISO;
  return MAP_TYPE_GRID;
}
