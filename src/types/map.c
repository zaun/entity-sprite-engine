...
bool _allocate_cells_array(EseMap *map) {
    ...
    map->cells =
        (EseMapCell ***)memory_manager.malloc(sizeof(EseMapCell **) * map->height, MMTAG_MAP);
    if (!map->cells)
        return false;

    for (uint32_t y = 0; y < map->height; y++) {
        map->cells[y] =
            (EseMapCell **)memory_manager.malloc(sizeof(EseMapCell *) * map->width, MMTAG_MAP);
...
// Private static setters for Lua state management
static void _ese_map_set_lua_ref(EseMap *map, int lua_ref) { map->lua_ref = lua_ref; }

static void _ese_map_set_lua_ref_count(EseMap *map, int lua_ref_count) {
    map->lua_ref_count = lua_ref_count;
}

static void _ese_map_set_state(EseMap *map, lua_State *state) { map->state = state; }

static void _ese_map_set_engine(EseMap *map, EseLuaEngine *engine) { map->engine = engine; }
...
bool ese_map_add_watcher(EseMap *map, EseMapWatcherCallback callback, void *userdata) {
    ...
    if (map->watcher_count == 0) {
        ...
        map->watchers =
            memory_manager.malloc(sizeof(EseMapWatcherCallback) * map->watcher_capacity, MMTAG_MAP);
        map->watcher_userdata =
            memory_manager.malloc(sizeof(void *) * map->watcher_capacity, MMTAG_MAP);
        map->watcher_count = 0;
    }
    ...
    if (map->watcher_count >= map->watcher_capacity) {
        ...
        void **new_userdata =
            memory_manager.realloc(map->watcher_userdata, sizeof(void *) * new_capacity, MMTAG_MAP);
