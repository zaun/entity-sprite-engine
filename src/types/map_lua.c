...
static int _ese_map_lua_gc(lua_State *L) {
    ...
    if (map && !map->destroyed) {
        ...
        // so we can free it.
        // If lua_ref != LUA_NOREF, this map was referenced from C and should
        // not be freed.
...
static int _ese_map_lua_tostring(lua_State *L) {
    ...
    char buf[160];
    snprintf(buf, sizeof(buf), "Map: %p (title=%s, width=%zu, height=%zu, type=%s)", (void *)map,
             ese_map_get_title(map) ? ese_map_get_title(map) : "(null)", ese_map_get_width(map),
             ese_map_get_height(map), ese_map_type_to_string(ese_map_get_type(map)));
...
void _ese_map_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, MAP_PROXY_META, _ese_map_lua_index, _ese_map_lua_newindex,
                               _ese_map_lua_gc, _ese_map_lua_tostring);
