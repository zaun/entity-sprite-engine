...
// Lua integration
void ese_tileset_lua_init(EseLuaEngine *engine) { _ese_tileset_lua_init(engine); }
...
void ese_tileset_lua_push(EseTileSet *tiles) {
    ...
    if (ese_tileset_get_lua_ref(tiles) == LUA_NOREF) {
        ...
        EseTileSet **ud =
            (EseTileSet **)lua_newuserdata(ese_tileset_get_state(tiles), sizeof(EseTileSet *));
        *ud = tiles;
...
bool ese_tileset_add_sprite(EseTileSet *tiles, uint8_t tile_id, const char *sprite_id,
    ...
    if (mapping->sprite_count >= mapping->sprite_capacity) {
        size_t new_capacity =
            mapping->sprite_capacity == 0 ? INITIAL_SPRITE_CAPACITY : mapping->sprite_capacity * 2;
