/**
 * @file tileset.c
 * @brief Implementation of tile system that maps tile IDs to weighted sprite
 * lists
 * @details Implements weighted random selection, Lua integration, and sprite
 * management
 *
...
static void _ese_tileset_set_lua_ref_count(EseTileSet *tiles,
                                           int lua_ref_count);
 ...
 *
 * Allocates memory for a new EseTileSet and initializes all fields to safe
 * defaults. The tileset starts with empty mappings and no Lua state.
 *
 * @return Pointer to the newly created EseTileSet, or NULL on allocation
 * failure
 */
EseTileSet *_ese_tileset_make() {
  EseTileSet *tiles =
      (EseTileSet *)memory_manager.malloc(sizeof(EseTileSet), MMTAG_TILESET);
  if (!tiles)
    return NULL;
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (src_mapping->sprite_count > 0) {
      ...
      dst_mapping->sprites = (EseSpriteWeight *)memory_manager.malloc(
          sizeof(EseSpriteWeight) * dst_mapping->sprite_capacity,
          MMTAG_TILESET);
      ...
      for (size_t j = 0; j < src_mapping->sprite_count; j++) {
        ...
        dst_mapping->sprites[j].sprite_id =
            (char *)memory_manager.malloc(len + 1, MMTAG_TILESET);
        ...
        strcpy(dst_mapping->sprites[j].sprite_id,
               src_mapping->sprites[j].sprite_id);
...
size_t ese_tileset_sizeof(void) { return sizeof(EseTileSet); }
...
void ese_tileset_lua_push(EseTileSet *tiles) {
  ...
  if (ese_tileset_get_lua_ref(tiles) == LUA_NOREF) {
    ...
    EseTileSet **ud = (EseTileSet **)lua_newuserdata(
        ese_tileset_get_state(tiles), sizeof(EseTileSet *));
    *ud = tiles;
    ...
    lua_rawgeti(ese_tileset_get_state(tiles), LUA_REGISTRYINDEX,
                ese_tileset_get_lua_ref(tiles));
  }
}
...
void ese_tileset_ref(EseTileSet *tiles) {
  ...
  if (ese_tileset_get_lua_ref(tiles) == LUA_NOREF) {
    ...
    *ud = tiles;
    ...
    _ese_tileset_set_lua_ref_count(tiles,
                                   ese_tileset_get_lua_ref_count(tiles) + 1);
  }
...
int ese_tileset_get_lua_ref(const EseTileSet *tiles) {
  log_assert("TILESET", tiles,
             "ese_tileset_get_lua_ref called with NULL tiles");
  return tiles->lua_ref;
}

int ese_tileset_get_lua_ref_count(const EseTileSet *tiles) {
  log_assert("TILESET", tiles,
             "ese_tileset_get_lua_ref_count called with NULL tiles");
  return tiles->lua_ref_count;
}

uint32_t ese_tileset_get_rng_seed(const EseTileSet *tiles) {
  log_assert("TILESET", tiles,
             "ese_tileset_get_rng_seed called with NULL tiles");
...
bool ese_tileset_add_sprite(EseTileSet *tiles, uint8_t tile_id,
  ...
  if (mapping->sprite_count >= mapping->sprite_capacity) {
    size_t new_capacity = mapping->sprite_capacity == 0
                              ? INITIAL_SPRITE_CAPACITY
                              : mapping->sprite_capacity * 2;
    EseSpriteWeight *new_array = (EseSpriteWeight *)memory_manager.realloc(
        mapping->sprites, sizeof(EseSpriteWeight) * new_capacity,
        MMTAG_TILESET);
    if (!new_array)
      return false;
...
bool ese_tileset_update_sprite_weight(EseTileSet *tiles, uint8_t tile_id,
                                      const char *sprite_id,
                                      uint16_t new_weight) {
  if (!tiles || !sprite_id || new_weight == 0)
    return false;
