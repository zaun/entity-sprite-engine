...
typedef struct EsePolyLine {
  ...
  size_t point_capacity; /** Capacity of the points array (in number of points,
                            not floats) */
...
static void _ese_poly_line_set_lua_ref_count(EsePolyLine *poly_line,
                                             int lua_ref_count);
...
EsePolyLine *_ese_poly_line_make() {
  EsePolyLine *poly_line = (EsePolyLine *)memory_manager.malloc(
      sizeof(EsePolyLine), MMTAG_POLY_LINE);
...
EsePolyLine *ese_poly_line_copy(const EsePolyLine *source) {
  ...
  EsePolyLine *copy = (EsePolyLine *)memory_manager.malloc(sizeof(EsePolyLine),
                                                           MMTAG_POLY_LINE);
  ...
  if (source->point_count > 0) {
    copy->points = memory_manager.malloc(
        sizeof(float) * source->point_count * 2, MMTAG_POLY_LINE);
    memcpy(copy->points, source->points,
           sizeof(float) * source->point_count * 2);
  } else {
    copy->points = NULL;
  }
...
bool ese_poly_line_add_point(EsePolyLine *poly_line, EsePoint *point) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_add_point called with NULL poly_line");
  ...
  if (poly_line->point_count == 0) {
    ...
    poly_line->points = memory_manager.malloc(
        sizeof(float) * poly_line->point_capacity * 2, MMTAG_POLY_LINE);
    poly_line->point_count = 0;
  }
...
bool ese_poly_line_remove_point(EsePolyLine *poly_line, size_t index) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_remove_point called with NULL poly_line");
...
EsePoint *ese_poly_line_get_point(const EsePolyLine *poly_line, size_t index) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_get_point called with NULL poly_line");
  ...
  EseLuaEngine *engine = (EseLuaEngine *)lua_engine_get_registry_key(
      poly_line->state, LUA_ENGINE_KEY);
...
size_t ese_poly_line_get_point_count(const EsePolyLine *poly_line) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_get_point_count called with NULL poly_line");
  return poly_line->point_count;
}

void ese_poly_line_clear_points(EsePolyLine *poly_line) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_clear_points called with NULL poly_line");
...
const float *ese_poly_line_get_points(const EsePolyLine *poly_line) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_get_points called with NULL poly_line");
...
float ese_poly_line_get_point_y(const EsePolyLine *poly_line, size_t index) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_get_point_y called with NULL poly_line");
  if (index >= poly_line->point_count) {
    return 0.0f;
  }
  size_t coord_index = index * 2;
...
void ese_poly_line_set_type(EsePolyLine *poly_line, EsePolyLineType type) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_set_type called with NULL poly_line");
  poly_line->type = type;
  _ese_poly_line_notify_watchers(poly_line);
}

void ese_poly_line_set_stroke_width(EsePolyLine *poly_line,
                                    float stroke_width) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_set_stroke_width called with NULL poly_line");
  poly_line->stroke_width = stroke_width;
  _ese_poly_line_notify_watchers(poly_line);
}

void ese_poly_line_set_stroke_color(EsePolyLine *poly_line,
                                    EseColor *stroke_color) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_set_stroke_color called with NULL poly_line");
  poly_line->stroke_color = stroke_color;
  _ese_poly_line_notify_watchers(poly_line);
}

void ese_poly_line_set_fill_color(EsePolyLine *poly_line,
                                  EseColor *fill_color) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_set_fill_color called with NULL poly_line");
...
void ese_poly_line_set_state(EsePolyLine *poly_line, lua_State *state) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_set_state called with NULL poly_line");
  poly_line->state = state;
}
...
bool ese_poly_line_add_watcher(EsePolyLine *poly_line,
                               ...
                               void *userdata) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_add_watcher called with NULL poly_line");
  log_assert("POLY_LINE", callback,
             "poly_line_add_watcher called with NULL callback");
  ...
  if (poly_line->watcher_count == 0) {
    ...
    poly_line->watchers = memory_manager.malloc(
        sizeof(EsePolyLineWatcherCallback) * poly_line->watcher_capacity,
        MMTAG_POLY_LINE);
    poly_line->watcher_userdata = memory_manager.malloc(
        sizeof(void *) * poly_line->watcher_capacity, MMTAG_POLY_LINE);
    poly_line->watcher_count = 0;
  }
  ...
  if (poly_line->watcher_count >= poly_line->watcher_capacity) {
    ...
    EsePolyLineWatcherCallback *new_watchers = memory_manager.realloc(
        ...
        MMTAG_POLY_LINE);
    void **new_userdata =
        memory_manager.realloc(poly_line->watcher_userdata,
                               sizeof(void *) * new_capacity, MMTAG_POLY_LINE);
...
bool ese_poly_line_remove_watcher(EsePolyLine *poly_line,
                                  ...
                                  void *userdata) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_remove_watcher called with NULL poly_line");
  log_assert("POLY_LINE", callback,
             "poly_line_remove_watcher called with NULL callback");

  for (size_t i = 0; i < poly_line->watcher_count; i++) {
    if (poly_line->watchers[i] == callback &&
        poly_line->watcher_userdata[i] == userdata) {
...
void ese_poly_line_lua_push(EsePolyLine *poly_line) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_lua_push called with NULL poly_line");

  if (poly_line->lua_ref == LUA_NOREF) {
    // Lua-owned: create a new userdata
    EsePolyLine **ud = (EsePolyLine **)lua_newuserdata(poly_line->state,
                                                       sizeof(EsePolyLine *));
    *ud = poly_line;
...
EsePolyLine *ese_poly_line_lua_get(lua_State *L, int idx) {
  ...
  EsePolyLine **ud =
      (EsePolyLine **)luaL_testudata(L, idx, POLY_LINE_PROXY_META);
  if (!ud) {
...
void ese_poly_line_ref(EsePolyLine *poly_line) {
  log_assert("POLY_LINE", poly_line,
             "poly_line_ref called with NULL poly_line");
...
cJSON *ese_poly_line_serialize(const EsePolyLine *poly_line) {
  log_assert("POLY_LINE", poly_line,
             "ese_poly_line_serialize called with NULL poly_line");

  cJSON *json = cJSON_CreateObject();
  if (!json) {
    log_error("POLY_LINE",
              "Failed to create cJSON object for poly_line serialization");
    return NULL;
  }
  ...
  if (!type || !cJSON_AddItemToObject(json, "type", type)) {
    log_error("POLY_LINE",
              "Failed to add type field to poly_line serialization");
    cJSON_Delete(json);
    return NULL;
  }
  ...
  if (!poly_type || !cJSON_AddItemToObject(json, "poly_type", poly_type)) {
    log_error("POLY_LINE",
              "Failed to add poly_type field to poly_line serialization");
    cJSON_Delete(json);
    return NULL;
  }
  ...
  if (!stroke_width ||
      !cJSON_AddItemToObject(json, "stroke_width", stroke_width)) {
    log_error("POLY_LINE",
              "Failed to add stroke_width field to poly_line serialization");
    cJSON_Delete(json);
    return NULL;
  }
  ...
  if (poly_line->stroke_color) {
    ...
    if (!stroke_color_json ||
        !cJSON_AddItemToObject(json, "stroke_color", stroke_color_json)) {
      log_error("POLY_LINE",
                "Failed to add stroke_color field to poly_line serialization");
      cJSON_Delete(json);
      return NULL;
    }
  }
  ...
  if (poly_line->fill_color) {
    ...
    if (!fill_color_json ||
        !cJSON_AddItemToObject(json, "fill_color", fill_color_json)) {
      log_error("POLY_LINE",
                "Failed to add fill_color field to poly_line serialization");
      cJSON_Delete(json);
      return NULL;
    }
  }
  ...
  if (!points_array || !cJSON_AddItemToObject(json, "points", points_array)) {
    log_error("POLY_LINE",
              "Failed to add points array to poly_line serialization");
    cJSON_Delete(json);
    return NULL;
  }
  ...
  for (size_t i = 0; i < poly_line->point_count; i++) {
    ...
    if (!point_array) {
      log_error("POLY_LINE",
                "Failed to create point array for poly_line serialization");
      cJSON_Delete(json);
      return NULL;
    }
    ...
    if (!x || !y || !cJSON_AddItemToArray(point_array, x) ||
      ...
      log_error("POLY_LINE",
                "Failed to add point coordinates to poly_line serialization");
    ...
    if (!cJSON_AddItemToArray(points_array, point_array)) {
      log_error(
          "POLY_LINE",
          "Failed to add point to points array in poly_line serialization");
      cJSON_Delete(point_array);
      cJSON_Delete(json);
      return NULL;
    }
 ...
 */
EsePolyLine *ese_poly_line_deserialize(EseLuaEngine *engine,
                                       const cJSON *data) {
  log_assert("POLY_LINE", data,
             "ese_poly_line_deserialize called with NULL data");

  if (!cJSON_IsObject(data)) {
    log_error("POLY_LINE",
              "PolyLine deserialization failed: data is not a JSON object");
    return NULL;
  }
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
    ...
    log_error("POLY_LINE",
              "PolyLine deserialization failed: invalid or missing type field");
    return NULL;
  }
  ...
  if (!poly_type_item || !cJSON_IsString(poly_type_item)) {
    log_error(
        "POLY_LINE",
        "PolyLine deserialization failed: invalid or missing poly_type field");
    return NULL;
  }
  ...
  if (strcmp(poly_type_item->valuestring, "OPEN") == 0) {
    ...
    log_error("POLY_LINE",
              "PolyLine deserialization failed: invalid poly_type value");
    return NULL;
  }
  ...
  if (!points_item || !cJSON_IsArray(points_item)) {
    log_error(
        "POLY_LINE",
        "PolyLine deserialization failed: invalid or missing points array");
    return NULL;
  }
  ...
  if (point_count == 0) {
    log_error("POLY_LINE",
              "PolyLine deserialization failed: empty points array");
    return NULL;
  }
  ...
  for (size_t i = 0; i < point_count; i++) {
    ...
    if (!point_item || !cJSON_IsArray(point_item) ||
        cJSON_GetArraySize(point_item) != 2) {
      log_error("POLY_LINE",
                "PolyLine deserialization failed: invalid point format");
    ...
    if (!x_item || !cJSON_IsNumber(x_item) || !y_item ||
        !cJSON_IsNumber(y_item)) {
      log_error("POLY_LINE",
                "PolyLine deserialization failed: invalid point coordinates");
      ese_poly_line_destroy(poly_line);
      return NULL;
    }
