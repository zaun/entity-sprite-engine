...
static int _ese_camera_lua_gc(lua_State *L) {
  ...
  if (camera_state) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this camera was referenced from C and should not
    // be freed.
 ...
 * accesses camera.position, camera.rotation, or camera.scale, this function is
 * called to retrieve the values. Position returns an EsePoint object, rotation
 * and scale return numbers.
 *
 * @param L Lua state
 ...
 * assigns to camera.rotation, camera.scale, or camera.position, this function
 * is called to update the values. Position must be set through an EsePoint
 * object.
 *
 * @param L Lua state
...
static int _ese_camera_lua_newindex(lua_State *L) {
  ...
  if (strcmp(key, "rotation") == 0) {
    ...
    profile_stop(PROFILE_LUA_CAMERA_NEWINDEX,
                 "ese_camera_lua_newindex (rotation)");
    return 0;
  } else if (strcmp(key, "scale") == 0) {
    if (!lua_isnumber(L, 3)) {
      profile_cancel(PROFILE_LUA_CAMERA_NEWINDEX);
    ...
    ese_point_set_x(ese_camera_get_position(camera_state),
                    ese_point_get_x(new_position_point));
    ese_point_set_y(ese_camera_get_position(camera_state),
                    ese_point_get_y(new_position_point));
    profile_stop(PROFILE_LUA_CAMERA_NEWINDEX,
                 "ese_camera_lua_newindex (position)");
    return 0;
  }
  profile_stop(PROFILE_LUA_CAMERA_NEWINDEX,
               "ese_camera_lua_newindex (invalid)");
 ...
 * The format includes the memory address, position coordinates, rotation, and
 * scale values.
 *
 * @param L Lua state
...
static int _ese_camera_lua_tostring(lua_State *L) {
  ...
  snprintf(buf, sizeof(buf),
           "Camera: %p (pos=(%.2f, %.2f), rot=%.2f, scale=%.2f)",
           (void *)camera_state,
           ese_point_get_x(ese_camera_get_position(camera_state)),
           ese_point_get_y(ese_camera_get_position(camera_state)),
           ese_camera_get_rotation(camera_state),
           ese_camera_get_scale(camera_state));
...
void _ese_camera_lua_init(EseLuaEngine *engine) {
  // Create metatable
  lua_engine_new_object_meta(engine, CAMERA_META, _ese_camera_lua_index,
                             _ese_camera_lua_newindex, _ese_camera_lua_gc,
