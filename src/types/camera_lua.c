...
static int _ese_camera_lua_gc(lua_State *L) {
    ...
    if (camera_state) {
        ...
        // so we can free it.
        // If lua_ref != LUA_NOREF, this camera was referenced from C and should
        // not be freed.
...
static int _ese_camera_lua_tostring(lua_State *L) {
    ...
    snprintf(buf, sizeof(buf), "Camera: %p (pos=(%.2f, %.2f), rot=%.2f, scale=%.2f)",
             (void *)camera_state, ese_point_get_x(ese_camera_get_position(camera_state)),
...
void _ese_camera_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, CAMERA_META, _ese_camera_lua_index, _ese_camera_lua_newindex,
                               _ese_camera_lua_gc, _ese_camera_lua_tostring);
}
