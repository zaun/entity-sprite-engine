 ...
 *
 * @details This structure stores the position, rotation, and scale of a camera
 * in 2D space. The position is represented as a EsePoint object.
 */
typedef struct {
 ...
 *
 * @details Allocates memory for a new EseCamera and initializes to default
 * values (position at (0, 0), rotation 0, scale 1.0). The position EsePoint is
 * C-owned. The camera is created without Lua references and must be explicitly
 ...
 *
 * @warning The returned EseCamera must be freed with ese_camera_destroy() to
 ...
 *
 * @details This function creates a deep copy of an EseCamera object. It
 * allocates a new EseCamera struct and copies all members, including creating a
 * new EsePoint for the position. The copy is created without Lua references and
 * must be explicitly referenced with ese_camera_ref() if Lua access is desired.
 *
 ...
 *
 * @details If the camera has no Lua references (lua_ref == LUA_NOREF), frees
 * memory immediately. If the camera has Lua references, decrements the
 ...
 *
 * @details Creates and registers the "CameraStateProxyMeta" metatable with
 * __index, __newindex,
 *          __gc, __tostring metamethods for property access and garbage
 * collection. This allows EseCamera objects to be used naturally from Lua with
 * dot notation. Also creates the global "Camera" table with "new" constructor.
 *
 * @param engine EseLuaEngine pointer where the EseCamera type will be
 * registered
 */
 ...
 *
 * @details If the camera has no Lua references (lua_ref == LUA_NOREF), creates
 * a new proxy table. If the camera has Lua references, retrieves the existing
...
/**
 * @brief Extracts a EseCamera pointer from a Lua userdata object with type
 * safety.
 *
 ...
 * @return Pointer to the EseCamera object, or NULL if extraction fails or type
 * check fails
 *
 ...
 *
 * @details If camera_state->lua_ref is LUA_NOREF, pushes the camera to Lua and
 * references it, setting lua_ref_count to 1. If camera_state->lua_ref is
 * already set, increments the reference count by 1. This prevents the camera
 * from being garbage collected while C code holds references to it.
 *
