...
static int _ese_display_lua_tostring(lua_State *L) {
    ...
    snprintf(buf, sizeof(buf), "Display: %p (%dx%d, %s, viewport: %dx%d)", 
             (void *)display, ese_display_get_width(display), ese_display_get_height(display),
 ...
 * 
 * @details This function registers the 'DisplayMeta' metatable in the Lua registry,
...
void _ese_display_lua_init(EseLuaEngine *engine) {
    ...
    log_assert("DISPLAY_STATE", engine->runtime, "_ese_display_lua_init called with NULL engine->runtime");

    // Create metatable
    lua_engine_new_object_meta(engine, DISPLAY_META, 
