...
void ese_ray_lua_push(EseRay *ray) {
  ...
  if (ese_ray_get_lua_ref(ray) == LUA_NOREF) {
    ...
    EseRay **ud =
        (EseRay **)lua_newuserdata(ese_ray_get_state(ray), sizeof(EseRay *));
    *ud = ray;
    ...
    lua_rawgeti(ese_ray_get_state(ray), LUA_REGISTRYINDEX,
                ese_ray_get_lua_ref(ray));
  }
}
...
EseRay *ese_ray_deserialize(EseLuaEngine *engine, const cJSON *data) {
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
      strcmp(type_item->valuestring, "RAY") != 0) {
    log_error("RAY",
              "Ray deserialization failed: invalid or missing type field");
    return NULL;
  }
...
bool ese_ray_intersects_rect(const EseRay *ray, const EseRect *rect) {
  ...
  if (dx != 0.0f) {
    ...
    if (t2 < t_far)
      t_far = t2;
  } else if (x < ese_rect_get_x(rect) ||
             x > ese_rect_get_x(rect) + ese_rect_get_width(rect)) {
    return false;
  }
  ...
  if (dy != 0.0f) {
    ...
    if (t2 < t_far)
      t_far = t2;
  } else if (y < ese_rect_get_y(rect) ||
             y > ese_rect_get_y(rect) + ese_rect_get_height(rect)) {
    return false;
  }
