...
cJSON *ese_rect_serialize(const EseRect *rect) {
  ...
  // Add rotation
  cJSON *rotation =
      cJSON_CreateNumber((double)rad_to_deg(ese_rect_get_rotation(rect)));
...
EseRect *ese_rect_deserialize(EseLuaEngine *engine, const cJSON *data) {
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
      strcmp(type_item->valuestring, "RECT") != 0) {
    log_error("RECT",
              "Rect deserialization failed: invalid or missing type field");
    return NULL;
  }
  ...
  if (!x_item || !cJSON_IsNumber(x_item)) {
    log_error("RECT",
              "Rect deserialization failed: invalid or missing x field");
    return NULL;
  }
  ...
  if (!y_item || !cJSON_IsNumber(y_item)) {
    log_error("RECT",
              "Rect deserialization failed: invalid or missing y field");
    return NULL;
  }
  ...
  if (!width_item || !cJSON_IsNumber(width_item)) {
    log_error("RECT",
              "Rect deserialization failed: invalid or missing width field");
    return NULL;
  }
  ...
  if (!height_item || !cJSON_IsNumber(height_item)) {
    log_error("RECT",
              "Rect deserialization failed: invalid or missing height field");
    return NULL;
  }
...
void ese_rect_lua_push(EseRect *rect) {
  ...
  if (ese_rect_get_lua_ref(rect) == LUA_NOREF) {
    ...
    EseRect **ud = (EseRect **)lua_newuserdata(ese_rect_get_state(rect),
                                               sizeof(EseRect *));
    *ud = rect;
    ...
    lua_rawgeti(ese_rect_get_state(rect), LUA_REGISTRYINDEX,
                ese_rect_get_lua_ref(rect));
  }
}
...
bool ese_rect_contains_point(const EseRect *rect, float x, float y) {
  ...
  return (localX >= -halfW && localX <= halfW && localY >= -halfH &&
          localY <= halfH);
}
...
bool ese_rect_add_watcher(EseRect *rect, EseRectWatcherCallback callback,
  ...
  log_assert("RECT", callback,
             "ese_rect_add_watcher called with NULL callback");
  ...
  if (rect->watcher_count == 0) {
    ...
    rect->watcher_userdata = memory_manager.malloc(
        sizeof(void *) * rect->watcher_capacity, MMTAG_RECT);
    rect->watcher_count = 0;
  }
  ...
  if (rect->watcher_count >= rect->watcher_capacity) {
    ...
    EseRectWatcherCallback *new_watchers = memory_manager.realloc(
        rect->watchers, sizeof(EseRectWatcherCallback) * new_capacity,
        MMTAG_RECT);
    void **new_userdata = memory_manager.realloc(
        rect->watcher_userdata, sizeof(void *) * new_capacity, MMTAG_RECT);
...
bool ese_rect_remove_watcher(EseRect *rect, EseRectWatcherCallback callback,
  ...
  log_assert("RECT", callback,
             "ese_rect_remove_watcher called with NULL callback");

  for (size_t i = 0; i < rect->watcher_count; i++) {
    if (rect->watchers[i] == callback &&
        rect->watcher_userdata[i] == userdata) {
