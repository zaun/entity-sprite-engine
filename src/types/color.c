...
static EseColor *_ese_color_make() {
  EseColor *color =
      (EseColor *)memory_manager.malloc(sizeof(EseColor), MMTAG_COLOR);
  color->r = 0.0f;
  color->g = 0.0f;
  color->b = 0.0f;
  color->a = 1.0f;
 ...
 */
static void _ese_color_set_lua_ref(EseColor *color, int lua_ref) {
  log_assert("COLOR", color != NULL,
             "_ese_color_set_lua_ref: color cannot be NULL");
...
static void _ese_color_set_lua_ref_count(EseColor *color, int lua_ref_count) {
  log_assert("COLOR", color != NULL,
             "_ese_color_set_lua_ref_count: color cannot be NULL");
...
static void _ese_color_set_state(EseColor *color, lua_State *state) {
  log_assert("COLOR", color != NULL,
             "_ese_color_set_state: color cannot be NULL");
  color->state = state;
}
...
int ese_color_get_lua_ref_count(const EseColor *color) {
  log_assert("COLOR", color,
             "ese_color_get_lua_ref_count called with NULL color");
...
bool ese_color_add_watcher(EseColor *color, EseColorWatcherCallback callback,
  ...
  log_assert("COLOR", callback,
             "ese_color_add_watcher called with NULL callback");
  ...
  if (color->watcher_count == 0) {
    ...
    color->watcher_userdata = memory_manager.malloc(
        sizeof(void *) * color->watcher_capacity, MMTAG_COLOR);
    color->watcher_count = 0;
  }
  ...
  if (color->watcher_count >= color->watcher_capacity) {
    ...
    EseColorWatcherCallback *new_watchers = memory_manager.realloc(
        color->watchers, sizeof(EseColorWatcherCallback) * new_capacity,
        MMTAG_COLOR);
    void **new_userdata = memory_manager.realloc(
        color->watcher_userdata, sizeof(void *) * new_capacity, MMTAG_COLOR);
...
bool ese_color_remove_watcher(EseColor *color, EseColorWatcherCallback callback,
  ...
  log_assert("COLOR", callback,
             "ese_color_remove_watcher called with NULL callback");

  for (size_t i = 0; i < color->watcher_count; i++) {
    if (color->watchers[i] == callback &&
        color->watcher_userdata[i] == userdata) {
...
void ese_color_lua_push(EseColor *color) {
  ...
  if (ese_color_get_lua_ref(color) == LUA_NOREF) {
    ...
    EseColor **ud = (EseColor **)lua_newuserdata(ese_color_get_state(color),
                                                 sizeof(EseColor *));
    *ud = color;
    ...
    lua_rawgeti(ese_color_get_state(color), LUA_REGISTRYINDEX,
                ese_color_get_lua_ref(color));
  }
}
...
void ese_color_unref(EseColor *color) {
  ...
  if (ese_color_get_lua_ref(color) != LUA_NOREF &&
      ese_color_get_lua_ref_count(color) > 0) {
    _ese_color_set_lua_ref_count(color, ese_color_get_lua_ref_count(color) - 1);

    if (ese_color_get_lua_ref_count(color) == 0) {
      // No more references - remove from registry
      luaL_unref(ese_color_get_state(color), LUA_REGISTRYINDEX,
                 ese_color_get_lua_ref(color));
      _ese_color_set_lua_ref(color, LUA_NOREF);
    }
  }
...
bool ese_color_set_hex(EseColor *color, const char *hex_string) {
  ...
  log_assert("COLOR", hex_string,
             "ese_color_set_hex called with NULL hex_string");
...
EseColor *ese_color_deserialize(EseLuaEngine *engine, const cJSON *data) {
  ...
  if (!cJSON_IsObject(data)) {
    log_error("COLOR",
              "Color deserialization failed: data is not a JSON object");
    return NULL;
  }
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
      strcmp(type_item->valuestring, "COLOR") != 0) {
    log_error("COLOR",
              "Color deserialization failed: invalid or missing type field");
    return NULL;
  }
  ...
  if (!r_item || !cJSON_IsNumber(r_item)) {
    log_error("COLOR",
              "Color deserialization failed: invalid or missing r field");
    return NULL;
  }
  ...
  if (!g_item || !cJSON_IsNumber(g_item)) {
    log_error("COLOR",
              "Color deserialization failed: invalid or missing g field");
    return NULL;
  }
  ...
  if (!b_item || !cJSON_IsNumber(b_item)) {
    log_error("COLOR",
              "Color deserialization failed: invalid or missing b field");
    return NULL;
  }
  ...
  if (!a_item || !cJSON_IsNumber(a_item)) {
    log_error("COLOR",
              "Color deserialization failed: invalid or missing a field");
    return NULL;
  }
