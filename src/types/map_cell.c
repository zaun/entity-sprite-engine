...
static void _ese_map_cell_set_lua_ref_count(EseMapCell *cell,
                                            int lua_ref_count);
...
static EseMapCell *_ese_map_cell_make(EseMap *map) {
  ...
  cell->tile_ids = (int *)memory_manager.malloc(
      sizeof(int) * INITIAL_LAYER_CAPACITY, MMTAG_MAP_CELL);
...
EseMapCell *ese_map_cell_copy(const EseMapCell *source) {
  ...
  copy->tile_ids = (int *)memory_manager.malloc(
      sizeof(int) * source->layer_capacity, MMTAG_MAP_CELL);
  if (!copy->tile_ids) {
  ...
  copy->data =
      source->data; // Shallow copy - caller responsible for deep copy if needed
...
void ese_map_cell_lua_init(EseLuaEngine *engine) {
  log_assert("MAPCELL", engine,
             "ese_map_cell_lua_init called with NULL engine");
...
void ese_map_cell_lua_push(EseMapCell *cell) {
  ...
  if (ese_map_cell_get_lua_ref(cell) == LUA_NOREF) {
    ...
    EseMapCell **ud = (EseMapCell **)lua_newuserdata(
        ese_map_cell_get_state(cell), sizeof(EseMapCell *));
    *ud = cell;
...
void ese_map_cell_ref(EseMapCell *cell) {
  ...
  if (ese_map_cell_get_lua_ref(cell) == LUA_NOREF) {
    ...
    *ud = cell;
    ...
    _ese_map_cell_set_lua_ref(
        cell, luaL_ref(ese_map_cell_get_state(cell), LUA_REGISTRYINDEX));
    _ese_map_cell_set_lua_ref_count(cell, 1);
  } else {
    // Already referenced - just increment count
    _ese_map_cell_set_lua_ref_count(cell,
                                    ese_map_cell_get_lua_ref_count(cell) + 1);
  }
...
void ese_map_cell_unref(EseMapCell *cell) {
  ...
  if (ese_map_cell_get_lua_ref(cell) != LUA_NOREF &&
      ese_map_cell_get_lua_ref_count(cell) > 0) {
    _ese_map_cell_set_lua_ref_count(cell,
                                    ese_map_cell_get_lua_ref_count(cell) - 1);
...
int ese_map_cell_get_lua_ref_count(const EseMapCell *cell) {
  log_assert("MAPCELL", cell,
             "ese_map_cell_get_lua_ref_count called with NULL cell");
...
bool ese_map_cell_add_watcher(EseMapCell *cell,
  ...
  log_assert("MAPCELL", callback,
             "ese_map_cell_add_watcher called with NULL callback");

  if (cell->watcher_count == 0) {
    cell->watcher_capacity = 4;
    cell->watchers = memory_manager.malloc(sizeof(EseMapCellWatcherCallback) *
                                               cell->watcher_capacity,
                                           MMTAG_MAP_CELL);
    cell->watcher_userdata = memory_manager.malloc(
        sizeof(void *) * cell->watcher_capacity, MMTAG_MAP_CELL);
    cell->watcher_count = 0;
  }
  ...
  if (cell->watcher_count >= cell->watcher_capacity) {
    ...
    EseMapCellWatcherCallback *new_watchers = memory_manager.realloc(
        cell->watchers, sizeof(EseMapCellWatcherCallback) * new_capacity,
        MMTAG_MAP_CELL);
    void **new_userdata = memory_manager.realloc(
        cell->watcher_userdata, sizeof(void *) * new_capacity, MMTAG_MAP_CELL);
...
bool ese_map_cell_remove_watcher(EseMapCell *cell,
                                 ...
                                 void *userdata) {
  log_assert("MAPCELL", cell,
             "ese_map_cell_remove_watcher called with NULL cell");
  log_assert("MAPCELL", callback,
             "ese_map_cell_remove_watcher called with NULL callback");

  for (size_t i = 0; i < cell->watcher_count; i++) {
    if (cell->watchers[i] == callback &&
        cell->watcher_userdata[i] == userdata) {
