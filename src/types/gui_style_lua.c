...
static int _ese_gui_style_lua_gc(lua_State *L) {
  ...
  if (style) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this style was referenced from C and should not
    // be freed.
...
static int _ese_gui_style_color_proxy_newindex(lua_State *L) {
  ...
  void (*setter)(EseGuiStyle *, EseGuiStyleVariant, const EseColor *) =
      (void (*)(EseGuiStyle *, EseGuiStyleVariant,
                const EseColor *))lua_touserdata(L, lua_upvalueindex(2));
...
static int _ese_gui_style_lua_index(lua_State *L) {
  ...
  if (strcmp(key, "border_width") == 0) {
    ...
    _ese_gui_style_create_color_proxy(L, style, ese_gui_style_get_bg,
                                      ese_gui_style_set_bg);
    profile_stop(PROFILE_LUA_GUI_STYLE_INDEX, "gui_style_lua_index (getter)");
    return 1;
    ...
    _ese_gui_style_create_color_proxy(L, style, ese_gui_style_get_text,
                                      ese_gui_style_set_text);
    profile_stop(PROFILE_LUA_GUI_STYLE_INDEX, "gui_style_lua_index (getter)");
    return 1;
    ...
    _ese_gui_style_create_color_proxy(L, style,
                                      ese_gui_style_get_selection_color,
...
void _ese_gui_style_lua_init(EseLuaEngine *engine) {
  ...
  lua_CFunction functions[] = {_ese_gui_style_lua_new,
                               _ese_gui_style_lua_from_json};
