 ...
 *
 * Handles cleanup when a Lua proxy table for an EseMapCell is garbage
 * collected. Only frees the underlying EseMapCell if it has no C-side
 * references.
 *
 * @param L Lua state
...
static int _ese_map_cell_lua_gc(lua_State *L) {
    ...
    if (cell) {
        ...
        // so we can free it.
        // If lua_ref != LUA_NOREF, this cell was referenced from C and should
        // not be freed.
...
static int _ese_map_cell_lua_tostring(lua_State *L) {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "MapCell: %p (layers=%zu, flags=%u, dynamic=%d)", (void *)cell,
             ese_map_cell_get_layer_count(cell), ese_map_cell_get_flags(cell),
             ese_map_cell_get_is_dynamic(cell));
...
static int _ese_map_cell_lua_add_layer(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in add_layer");
...
static int _ese_map_cell_lua_remove_layer(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in remove_layer");
...
static int _ese_map_cell_lua_get_layer(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in get_layer");
...
static int _ese_map_cell_lua_set_layer(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in set_layer");
...
static int _ese_map_cell_lua_clear_layers(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in clear_layers");
...
static int _ese_map_cell_lua_has_flag(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in has_flag");
...
static int _ese_map_cell_lua_set_flag(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in set_flag");
...
static int _ese_map_cell_lua_clear_flag(lua_State *L) {
    EseMapCell *cell = ese_map_cell_lua_get(L, 1);
    if (!cell)
        return luaL_error(L, "Invalid MapCell in clear_flag");
 ...
 *
 * Sets up the Lua metatable and global MapCell table with constructors and
 * methods. This function is called by the public ese_map_cell_lua_init
 * function.
 *
...
void _ese_map_cell_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, MAP_CELL_PROXY_META, _ese_map_cell_lua_index,
                               _ese_map_cell_lua_newindex, _ese_map_cell_lua_gc,
