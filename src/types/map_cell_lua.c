...
static int _ese_map_cell_lua_gc(lua_State *L) {
  ...
  if (cell) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this cell was referenced from C and should not
    // be freed.
...
static int _ese_map_cell_lua_newindex(lua_State *L) {
  ...
  if (strcmp(key, "isDynamic") == 0) {
    ...
    profile_stop(PROFILE_LUA_map_cell_NEWINDEX,
                 "mapcell_lua_newindex (setter)");
    return 0;
  } else if (strcmp(key, "flags") == 0) {
...
static int _ese_map_cell_lua_tostring(lua_State *L) {
  ...
  snprintf(buf, sizeof(buf), "MapCell: %p (layers=%zu, flags=%u, dynamic=%d)",
           (void *)cell, ese_map_cell_get_layer_count(cell),
           ese_map_cell_get_flags(cell), ese_map_cell_get_is_dynamic(cell));
...
static int _ese_map_cell_lua_add_layer(lua_State *L) {
  ...
  if (tile_id < -1 || tile_id > 255) {
    return luaL_error(L,
                      "add_layer(tile_id) requires a number >= -1 and <= 255");
  }
