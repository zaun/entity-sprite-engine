 ...
 *
 * @details This structure stores a ray defined by an origin point and direction
 * vector. The internal structure is hidden from the public API.
 */
 ...
 *
 * @details Allocates memory for a new EseRay and initializes to origin (0,0)
 * with direction (1,0). The ray is created without Lua references and must be
 * explicitly referenced with ese_ray_ref() if Lua access is desired.
 *
 * @param engine Pointer to a EseLuaEngine
 * @return Pointer to newly created EseRay object
 *
 * @warning The returned EseRay must be freed with ese_ray_destroy() to prevent
 * memory leaks
 */
 ...
 *
 * @details This function creates a deep copy of an EseRay object. It allocates
 * a new EseRay struct and copies all numeric members. The copy is created
 * without Lua references and must be explicitly referenced with ese_ray_ref()
 ...
 *
 * @details If the ray has no Lua references (lua_ref == LUA_NOREF), frees
 * memory immediately. If the ray has Lua references, decrements the reference
 ...
 *
 * @details Creates and registers the "RayProxyMeta" metatable with __index,
 * __newindex,
 *          __gc, __tostring metamethods for property access and garbage
 * collection. This allows EseRay objects to be used naturally from Lua with dot
 * notation. Also creates the global "Ray" table with "new" and "zero"
 * constructors.
 *
 ...
 *
 * @details If the ray has no Lua references (lua_ref == LUA_NOREF), creates a
 * new proxy table. If the ray has Lua references, retrieves the existing proxy
 ...
 * @return Pointer to the EseRay object, or NULL if extraction fails or type
 * check fails
 *
 ...
 *
 * @details If ray->lua_ref is LUA_NOREF, pushes the ray to Lua and references
 * it, setting lua_ref_count to 1. If ray->lua_ref is already set, increments
