 ...
 *
 * @details If the tileset has no Lua references (lua_ref == LUA_NOREF), creates
 * a new userdata. If the tileset has Lua references, retrieves the existing
...
/**
 * @brief Extracts a EseTileSet pointer from a Lua userdata object with type
 * safety.
 *
 ...
 * @return Pointer to the EseTileSet object, or NULL if extraction fails or type
 * check fails
 *
 ...
 *
 * @details If tiles->lua_ref is LUA_NOREF, pushes the tileset to Lua and
 * references it, setting lua_ref_count to 1. If tiles->lua_ref is already set,
 * increments the reference count by 1. This prevents the tileset from being
 ...
 *
 * @details Allocates memory for a new EseTileSet and initializes all mappings
 * to empty. The tileset is created without Lua references and must be
 * explicitly referenced with tileset_ref() if Lua access is desired.
 *
 * @param engine Pointer to a EseLuaEngine
 * @return Pointer to newly created EseTileSet object
 *
 * @warning The returned EseTileSet must be freed with tileset_destroy() to
 ...
 *
 * @details This function creates a deep copy of an EseTileSet object. It
 * allocates a new EseTileSet struct and copies all members, including the
 * sprite mappings. The copy is created without Lua references and must be
 * explicitly referenced with tileset_ref() if Lua access is desired.
 *
 ...
 *
 * @details If the tileset has no Lua references (lua_ref == LUA_NOREF), frees
 * memory immediately. If the tileset has Lua references, decrements the
 ...
 * functions. Allocates memory for a new EseTileSet and initializes all fields
 * to safe defaults. The tileset starts with empty mappings and no Lua state.
 *
