 ...
 * 
 * @details Creates and registers the "MapCellProxyMeta" metatable with __index and 
 *          __newindex metamethods for property access. This allows EseMapCell objects
 ...
 * 
 * @warning The returned EseMapCell must be freed with mapcell_destroy() to prevent memory leaks
 */
 ...
 *          struct and copies all members including the tile_ids array and data.
 * 
 ...
 * 
 * @details Frees the memory allocated by mapcell_create(), including tile_ids array.
 * 
 ...
 * 
 * @details Retrieves the C EseMapCell pointer from the "__ptr" field of a Lua
 *          table that was created by _mapcell_lua_push(). Performs
 *          type checking to ensure the object is a valid EseMapCell proxy table
