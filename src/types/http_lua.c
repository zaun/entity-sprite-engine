...
extern void _ese_http_request_set_lua_ref_count(EseHttpRequest *request,
                                                int count);
...
static int _ese_http_request_lua_gc(lua_State *L) {
  // Get from userdata
  EseHttpRequest **ud =
      (EseHttpRequest **)luaL_testudata(L, 1, HTTP_PROXY_META);
  if (!ud || !*ud) {
  ...
  if (request) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this request was referenced from C and should
    // not be freed.
...
static int _ese_http_request_lua_tostring(lua_State *L) {
  ...
  char buffer[256];
  if (done) {
    snprintf(buffer, sizeof(buffer), "HTTP Request: %s (status: %d)",
             url ? url : "unknown", status);
  } else {
    snprintf(buffer, sizeof(buffer), "HTTP Request: %s (pending)",
             url ? url : "unknown");
  }
...
static int _ese_http_request_lua_start(lua_State *L) {
  EseHttpRequest *request =
      (EseHttpRequest *)lua_touserdata(L, lua_upvalueindex(1));
  if (!request) {
  ...
  log_debug("HTTP", "Lua start() called for URL: %s",
            ese_http_request_get_url(request));
  int result = ese_http_request_start(request);
  if (result != 0) {
    log_debug("HTTP", "Failed to start HTTP request from Lua (error: %d)",
              result);
...
static int _ese_http_request_lua_new(lua_State *L) {
  ...
  EseHttpRequest **ud =
      (EseHttpRequest **)lua_newuserdata(L, sizeof(EseHttpRequest *));
  *ud = request;
...
void ese_http_request_ref(EseHttpRequest *request) {
  ...
  if (ref == LUA_NOREF) {
    ...
    *ud = request;
    ...
    // Reference it
    ref = luaL_ref(L, LUA_REGISTRYINDEX);
    // Note: We need to store this ref in the request, but we don't have access
    // to the struct This is a limitation of the current design - we'd need to
    // modify the struct
  } else {
