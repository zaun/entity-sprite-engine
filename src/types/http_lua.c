 ...
 * 
 * Handles cleanup when a Lua proxy table for an EseHttpRequest is garbage collected.
 * Only frees the underlying EseHttpRequest if it has no C-side references.
 * 
 * @param L Lua state
...
static int _ese_http_request_lua_gc(lua_State *L) {
    ...
    if (request) {
        // If lua_ref == LUA_NOREF, there are no more references to this request, 
        // so we can free it.
        // If lua_ref != LUA_NOREF, this request was referenced from C and should not be freed.
 ...
 * 
 * Provides read access to request properties (url, status, body, headers, done) from Lua. 
 * When a Lua script accesses request.url or request.status, this function is called to 
 * retrieve the values. Also provides access to methods like start and set_timeout.
 * 
 * @param L Lua state
 ...
 * 
 * Provides write access to request properties from Lua. When a Lua script assigns
 * to request.property, this function is called to set the values.
 * 
 * @param L Lua state
 * @return Always returns 0 (no values pushed)
 */
...
static int _ese_http_request_lua_tostring(lua_State *L) {
    ...
    char buffer[256];
    if (done) {
        snprintf(buffer, sizeof(buffer), "HTTP Request: %s (status: %d)", url ? url : "unknown", status);
    } else {
...
void ese_http_request_ref(EseHttpRequest *request) {
    ...
    if (ref == LUA_NOREF) {
        ...
        *ud = request;
        ...
        // Reference it
        ref = luaL_ref(L, LUA_REGISTRYINDEX);
        // Note: We need to store this ref in the request, but we don't have access to the struct
        // This is a limitation of the current design - we'd need to modify the struct
    } else {
