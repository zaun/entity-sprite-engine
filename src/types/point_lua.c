...
static int _ese_point_lua_gc(lua_State *L) {
  ...
  if (point) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this point was referenced from C and should not
    // be freed.
...
static int _ese_point_lua_tostring(lua_State *L) {
  ...
  char buf[64];
  snprintf(buf, sizeof(buf), "(x=%.3f, y=%.3f)", ese_point_get_x(point),
           ese_point_get_y(point));
 ...
 * is called when Lua code executes `Point.new(x, y)`. It validates the
 * arguments, creates the underlying EsePoint, and returns a proxy table that
 * provides access to the point's properties and methods.
 *
 * @param L Lua state
...
static int _ese_point_lua_distance(lua_State *L) {
  ...
  if (!point1 || !point2) {
    profile_cancel(PROFILE_LUA_POINT_ZERO);
    return luaL_error(L,
                      "Point.distance(point, point) arguments must be points");
  }
 ...
 *
 * Creates a new EsePoint from a JSON string. This function is called when Lua
 * code executes `Point.fromJSON(json_string)`. It parses the JSON string,
 * validates it contains point data, and creates a new point with the specified
 * coordinates.
 *
 * @param L Lua state
 * @return Number of values pushed onto the stack (always 1 - the proxy table)
...
static int _ese_point_lua_from_json(lua_State *L) {
  ...
  if (!json) {
    log_error("POINT", "Point.fromJSON: failed to parse JSON string: %s",
 ...
 *
 * Converts an EsePoint to a JSON string representation. This function is called
 * when Lua code executes `point:toJSON()`. It serializes the point's
 * coordinates to JSON format and returns the string.
 *
 * @param L Lua state
 ...
 *
 * Sets up the Lua metatable and global Point table with constructors and
 * methods. This function is called by the public ese_point_lua_init function.
 *
 * @param engine EseLuaEngine pointer where the EsePoint type will be registered
 */
void _ese_point_lua_init(EseLuaEngine *engine) {
  // Create metatable
  lua_engine_new_object_meta(engine, POINT_PROXY_META, _ese_point_lua_index,
                             _ese_point_lua_newindex, _ese_point_lua_gc,
  ...
  lua_CFunction functions[] = {_ese_point_lua_new, _ese_point_lua_zero,
                               _ese_point_lua_distance,
                               _ese_point_lua_from_json};
