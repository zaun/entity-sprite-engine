 ...
 * @details Provides a simple API for making HTTP requests with callbacks and timeouts
 * 
 ...
 * 
 * @details This callback is invoked when an HTTP request completes, either successfully
 * or with an error. The callback receives the status code, headers, raw response data,
 * body text, and user data pointer.
 * 
 ...
 * URL, timeout settings, callback, and user data.
 */
 ...
 * 
 * @details This function should only be called on requests that have not been started.
 * Started requests free themselves automatically after the callback completes.
 * 
 ...
 * 
 * @details Sets both connection and receive timeouts. A timeout of 0 uses system defaults.
 * 
 ...
 * Allocates memory for a new EseHttpRequest and initializes all fields to safe defaults.
 * The request starts with no URL, no Lua state or references.
 * 
 * @return Pointer to the newly created EseHttpRequest, or NULL on allocation failure
 */
