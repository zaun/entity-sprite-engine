...
const char *const input_state_key_names[] = {
    ...
    "KP_9", "KP_DECIMAL", "KP_ENTER", "KP_PLUS", "KP_MINUS", "KP_MULTIPLY",
    "KP_DIVIDE",
    // Mouse buttons
...
static void _ese_input_state_set_lua_ref_count(EseInputState *input,
                                               int lua_ref_count);
...
static EseInputState *_input_state_make() {
  EseInputState *input = (EseInputState *)memory_manager.malloc(
      sizeof(EseInputState), MMTAG_INPUT_STATE);
...
bool ese_input_state_get_key_down(const EseInputState *input, EseInputKey key) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_key_down called with NULL input");
...
bool ese_input_state_get_key_pressed(const EseInputState *input,
                                     EseInputKey key) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_key_pressed called with NULL input");
  log_assert("INPUT_STATE", (key >= 0 && key < InputKey_MAX),
...
bool ese_input_state_get_key_released(const EseInputState *input,
                                      EseInputKey key) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_key_released called with NULL input");
  log_assert("INPUT_STATE", (key >= 0 && key < InputKey_MAX),
...
bool ese_input_state_get_mouse_down(const EseInputState *input, int button) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_mouse_down called with NULL input");
...
bool ese_input_state_get_mouse_clicked(const EseInputState *input, int button) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_mouse_clicked called with NULL input");
  log_assert("INPUT_STATE", (button >= 0 && button < MOUSE_BUTTON_COUNT),
             "ese_input_state_get_mouse_clicked called with invalid button");
  return input->mouse_clicked[button];
}

bool ese_input_state_get_mouse_released(const EseInputState *input,
                                        int button) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_mouse_released called with NULL input");
  log_assert("INPUT_STATE", (button >= 0 && button < MOUSE_BUTTON_COUNT),
...
lua_State *ese_input_state_get_state(const EseInputState *input) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_state called with NULL input");
  return input->state;
}

int ese_input_state_get_lua_ref_count(const EseInputState *input) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_lua_ref_count called with NULL input");
  return input->lua_ref_count;
}

int ese_input_state_get_lua_ref(const EseInputState *input) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_get_lua_ref called with NULL input");
...
EseInputState *ese_input_state_copy(const EseInputState *src) {
  ...
  log_assert("INPUT_STATE", memory_manager.malloc,
             "memory_manager.malloc is NULL");

  EseInputState *copy = (EseInputState *)memory_manager.malloc(
      sizeof(EseInputState), MMTAG_INPUT_STATE);
  log_assert("INPUT_STATE", copy,
             "ese_input_state_copy failed to allocate memory");
  ...
  memcpy(copy->mouse_released, src->mouse_released,
         sizeof(src->mouse_released));
...
void ese_input_state_lua_push(EseInputState *input) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_lua_push called with NULL input");

  if (ese_input_state_get_lua_ref(input) == LUA_NOREF) {
    // Lua-owned: create a new userdata
    EseInputState **ud = (EseInputState **)lua_newuserdata(
        ese_input_state_get_state(input), sizeof(EseInputState *));
    *ud = input;
...
EseInputState *ese_input_state_lua_get(lua_State *L, int idx) {
  log_assert("INPUT_STATE", L,
             "ese_input_state_lua_get called with NULL Lua state");
  ...
  EseInputState **ud =
      (EseInputState **)luaL_testudata(L, idx, INPUT_STATE_PROXY_META);
  if (!ud) {
...
void ese_input_state_ref(EseInputState *input) {
  log_assert("INPUT_STATE", input,
             "ese_input_state_ref called with NULL input");
  ...
  if (ese_input_state_get_lua_ref(input) == LUA_NOREF) {
    ...
    *ud = input;
    ...
    _ese_input_state_set_lua_ref(
        input, luaL_ref(ese_input_state_get_state(input), LUA_REGISTRYINDEX));
    _ese_input_state_set_lua_ref_count(input, 1);
  } else {
    // Already referenced - just increment count
    _ese_input_state_set_lua_ref_count(
        input, ese_input_state_get_lua_ref_count(input) + 1);
  }
