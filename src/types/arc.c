 ...
 * @details Implements arc operations, collision detection, Lua integration, and
 * JSON serialization
 *
 ...
 *
 * Allocates memory for a new EseArc and initializes all fields to safe
 * defaults. The arc starts at origin (0,0) with unit radius, full circle
 * angles, and no Lua state or references.
 *
 ...
 */
float ese_arc_get_x(const EseArc *arc) { return arc->x; }
 ...
 */
float ese_arc_get_y(const EseArc *arc) { return arc->y; }
 ...
 */
float ese_arc_get_radius(const EseArc *arc) { return arc->radius; }
 ...
 */
float ese_arc_get_start_angle(const EseArc *arc) { return arc->start_angle; }
 ...
 */
float ese_arc_get_end_angle(const EseArc *arc) { return arc->end_angle; }
 ...
 */
lua_State *ese_arc_get_state(const EseArc *arc) { return arc->state; }
 ...
 */
int ese_arc_get_lua_ref(const EseArc *arc) { return arc->lua_ref; }
 ...
 */
int ese_arc_get_lua_ref_count(const EseArc *arc) { return arc->lua_ref_count; }
 ...
 */
static void _ese_arc_set_lua_ref_count(EseArc *arc, int lua_ref_count) {
  log_assert("ARC", arc != NULL,
             "_ese_arc_set_lua_ref_count: arc cannot be NULL");
...
// Lua integration
void ese_arc_lua_init(EseLuaEngine *engine) { _ese_arc_lua_init(engine); }
...
void ese_arc_lua_push(EseArc *arc) {
  ...
  if (ese_arc_get_lua_ref(arc) == LUA_NOREF) {
    ...
    EseArc **ud =
        (EseArc **)lua_newuserdata(ese_arc_get_state(arc), sizeof(EseArc *));
    *ud = arc;
    ...
    lua_rawgeti(ese_arc_get_state(arc), LUA_REGISTRYINDEX,
                ese_arc_get_lua_ref(arc));
  }
}
...
void ese_arc_unref(EseArc *arc) {
  ...
  if (ese_arc_get_lua_ref(arc) != LUA_NOREF &&
      ese_arc_get_lua_ref_count(arc) > 0) {
    _ese_arc_set_lua_ref_count(arc, ese_arc_get_lua_ref_count(arc) - 1);

    if (ese_arc_get_lua_ref_count(arc) == 0) {
      // No more references - remove from registry
      luaL_unref(ese_arc_get_state(arc), LUA_REGISTRYINDEX,
                 ese_arc_get_lua_ref(arc));
...
cJSON *ese_arc_serialize(const EseArc *arc) {
  ...
  if (!start_angle ||
      !cJSON_AddItemToObject(json, "start_angle", start_angle)) {
...
EseArc *ese_arc_deserialize(EseLuaEngine *engine, const cJSON *data) {
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
      strcmp(type_item->valuestring, "ARC") != 0) {
    log_error("ARC",
              "Arc deserialization failed: invalid or missing type field");
    return NULL;
  }
  ...
  if (!radius_item || !cJSON_IsNumber(radius_item)) {
    log_error("ARC",
              "Arc deserialization failed: invalid or missing radius field");
    return NULL;
  }
  ...
  if (!start_angle_item || !cJSON_IsNumber(start_angle_item)) {
    log_error(
        "ARC",
        "Arc deserialization failed: invalid or missing start_angle field");
    return NULL;
  }
  ...
  if (!end_angle_item || !cJSON_IsNumber(end_angle_item)) {
    log_error("ARC",
              "Arc deserialization failed: invalid or missing end_angle field");
    return NULL;
  }
