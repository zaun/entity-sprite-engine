...
void ese_uuid_lua_push(EseUUID *uuid) {
  ...
  if (ese_uuid_get_lua_ref(uuid) == LUA_NOREF) {
    ...
    EseUUID **ud = (EseUUID **)lua_newuserdata(ese_uuid_get_state(uuid),
                                               sizeof(EseUUID *));
    *ud = uuid;
    ...
    lua_rawgeti(ese_uuid_get_state(uuid), LUA_REGISTRYINDEX,
                ese_uuid_get_lua_ref(uuid));
  }
}
...
EseUUID *ese_uuid_deserialize(EseLuaEngine *engine, const cJSON *data) {
  ...
  if (!type_item || !cJSON_IsString(type_item) ||
      strcmp(type_item->valuestring, "UUID") != 0) {
    log_error("UUID",
              "UUID deserialization failed: invalid or missing type field");
    return NULL;
  }
  ...
  if (!value_item || !cJSON_IsString(value_item)) {
    log_error("UUID",
              "UUID deserialization failed: invalid or missing value field");
    return NULL;
  }
