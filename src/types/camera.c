...
static void _ese_camera_set_lua_ref_count(EseCamera *camera_state, int lua_ref_count);
...
void ese_camera_set_position(EseCamera *camera_state, EsePoint *position) {
    log_assert("CAMERA", camera_state != NULL, "ese_camera_set_position: camera_state cannot be NULL");
    log_assert("CAMERA", position != NULL, "ese_camera_set_position: position cannot be NULL");
...
void ese_camera_set_rotation(EseCamera *camera_state, float rotation) {
    log_assert("CAMERA", camera_state != NULL, "ese_camera_set_rotation: camera_state cannot be NULL");
...
void ese_camera_set_scale(EseCamera *camera_state, float scale) {
    log_assert("CAMERA", camera_state != NULL, "ese_camera_set_scale: camera_state cannot be NULL");
 ...
 */
static void _ese_camera_set_lua_ref(EseCamera *camera_state, int lua_ref) {
    log_assert("CAMERA", camera_state != NULL, "_ese_camera_set_lua_ref: camera_state cannot be NULL");
 ...
 */
static void _ese_camera_set_lua_ref_count(EseCamera *camera_state, int lua_ref_count) {
    log_assert("CAMERA", camera_state != NULL, "_ese_camera_set_lua_ref_count: camera_state cannot be NULL");
 ...
 */
static void _ese_camera_set_state(EseCamera *camera_state, lua_State *state) {
    log_assert("CAMERA", camera_state != NULL, "_ese_camera_set_state: camera_state cannot be NULL");
...
EseCamera *ese_camera_copy(const EseCamera *source) {
    ...
    ese_camera_set_position(copy, ese_point_copy(ese_camera_get_position(source)));
...
void ese_camera_lua_push(EseCamera *camera_state) {
    ...
    if (ese_camera_get_lua_ref(camera_state) == LUA_NOREF) {
        ...
        EseCamera **ud = (EseCamera **)lua_newuserdata(ese_camera_get_state(camera_state), sizeof(EseCamera *));
        *ud = camera_state;
        ...
        lua_rawgeti(ese_camera_get_state(camera_state), LUA_REGISTRYINDEX, ese_camera_get_lua_ref(camera_state));
    }
}
...
void ese_camera_ref(EseCamera *camera_state) {
    ...
    if (ese_camera_get_lua_ref(camera_state) == LUA_NOREF) {
        ...
        *ud = camera_state;
        ...
        _ese_camera_set_lua_ref_count(camera_state, ese_camera_get_lua_ref_count(camera_state) + 1);
    }
...
void ese_camera_unref(EseCamera *camera_state) {
    ...
    if (ese_camera_get_lua_ref(camera_state) != LUA_NOREF && ese_camera_get_lua_ref_count(camera_state) > 0) {
        _ese_camera_set_lua_ref_count(camera_state, ese_camera_get_lua_ref_count(camera_state) - 1);
        
        if (ese_camera_get_lua_ref_count(camera_state) == 0) {
            // No more references - remove from registry
            luaL_unref(ese_camera_get_state(camera_state), LUA_REGISTRYINDEX, ese_camera_get_lua_ref(camera_state));
            _ese_camera_set_lua_ref(camera_state, LUA_NOREF);
        }
    }
