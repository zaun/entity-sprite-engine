...
void ese_vector_lua_push(EseVector *vector) {
    ...
    if (ese_vector_get_lua_ref(vector) == LUA_NOREF) {
        ...
        EseVector **ud = (EseVector **)lua_newuserdata(ese_vector_get_state(vector), sizeof(EseVector *));
        *ud = vector;
        ...
        lua_rawgeti(ese_vector_get_state(vector), LUA_REGISTRYINDEX, ese_vector_get_lua_ref(vector));
    }
}
...
void ese_vector_ref(EseVector *vector) {
    ...
    if (ese_vector_get_lua_ref(vector) == LUA_NOREF) {
        ...
        *ud = vector;
        ...
        _ese_vector_set_lua_ref_count(vector, ese_vector_get_lua_ref_count(vector) + 1);
    }
...
void vector_unref(EseVector *vector) {
    ...
    if (ese_vector_get_lua_ref(vector) != LUA_NOREF && ese_vector_get_lua_ref_count(vector) > 0) {
        ...
        if (new_count == 0) {
            ...
            luaL_unref(ese_vector_get_state(vector), LUA_REGISTRYINDEX, ese_vector_get_lua_ref(vector));
