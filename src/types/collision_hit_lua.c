...
static int _ese_collision_hit_lua_index(lua_State *L) {
    ...
    if (strcmp(key, "kind") == 0) {
        ...
        if (ese_collision_hit_get_kind(hit) == COLLISION_KIND_COLLIDER && ese_collision_hit_get_rect(hit)) {
        ...
        if (ese_collision_hit_get_kind(hit) == COLLISION_KIND_MAP && ese_collision_hit_get_map(hit)) {
...
static int _ese_collision_hit_lua_tostring(lua_State *L) {
    ...
    char buf[160];
    snprintf(buf, sizeof(buf), "EseCollisionHit: %p (kind=%d, state=%d)", (void*)hit, (int)ese_collision_hit_get_kind(hit), (int)ese_collision_hit_get_state(hit));
