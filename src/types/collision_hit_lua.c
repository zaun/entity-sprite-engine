...
static int _ese_collision_hit_lua_tostring(lua_State *L) {
    ...
    char buf[160];
    snprintf(buf, sizeof(buf), "EseCollisionHit: %p (kind=%d, state=%d)", (void *)hit,
             (int)ese_collision_hit_get_kind(hit), (int)ese_collision_hit_get_state(hit));
...
void _ese_collision_hit_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, COLLISION_HIT_META, _ese_collision_hit_lua_index,
