 ...
 * 
 * @details If the color has no Lua references (lua_ref == LUA_NOREF), frees memory immediately.
 *          If the color has Lua references, decrements the reference counter.
 ...
 * 
 * @details The callback will be called whenever any property (r, g, b, a) of the color is modified.
 *          Multiple watchers can be registered on the same color.
 * 
 ...
 * 
 * @details Creates and registers the "ColorProxyMeta" metatable with __index, __newindex,
 *          __gc, __tostring metamethods for property access and garbage collection.
 *          This allows EseColor objects to be used naturally from Lua with dot notation.
 *          Also creates the global "Color" table with "new", "white", "black", "red", "green", "blue" constructors.
 * 
 ...
 * 
 * @details If the color has no Lua references (lua_ref == LUA_NOREF), creates a new
 *          proxy table. If the color has Lua references, retrieves the existing
 ...
 * @return Pointer to the EseColor object, or NULL if extraction fails or type check fails
 * 
 ...
 * 
 * @details If color->lua_ref is LUA_NOREF, pushes the color to Lua and references it,
 *          setting lua_ref_count to 1. If color->lua_ref is already set, increments
 *          the reference count by 1. This prevents the color from being garbage
