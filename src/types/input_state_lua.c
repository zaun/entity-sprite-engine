 ...
 *
 * Provides read-only access to key state arrays (keys_down, keys_pressed,
 * keys_released). This function is used as the __index metamethod for the proxy
 * tables created in the main index function.
 *
 * @param L Lua state
...
static int _input_state_keys_index(lua_State *L) {
  ...
  log_assert("INPUT_STATE", (key_idx >= 0 && key_idx < InputKey_MAX),
             "Invalid key index");
 ...
 *
 * Provides read-only access to mouse button states. This function is used as
 * the
 * __index metamethod for the mouse_buttons proxy table.
 *
 * @param L Lua state
 * @return Number of values pushed onto the stack (always 1 - boolean button
 * state)
 */
static int _input_state_mouse_down_index(lua_State *L) {
  EseInputState *input =
      (EseInputState *)lua_touserdata(L, lua_upvalueindex(1));
 ...
 *
 * Handles cleanup when a Lua proxy table for an EseInputState is garbage
 * collected. Only frees the underlying EseInputState if it has no C-side
 * references.
 *
 * @param L Lua state
...
static int _input_state_lua_gc(lua_State *L) {
  // Get from userdata
  EseInputState **ud =
      (EseInputState **)luaL_testudata(L, 1, INPUT_STATE_PROXY_META);
  if (!ud) {
  ...
  if (input) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this input was referenced from C and should not
    // be freed.
 ...
 * accesses input.mouse_x, input.keys_down, etc., this function is called to
 * retrieve the values. Creates read-only proxy tables for key arrays and mouse
 * button states.
 *
 * @param L Lua state
...
static int _input_state_lua_index(lua_State *L) {
  ...
  if (strcmp(key, "keys_down") == 0 || strcmp(key, "keys_pressed") == 0 ||
      strcmp(key, "keys_released") == 0) {
    bool *keys_array = NULL;
    if (strcmp(key, "keys_down") == 0)
      keys_array = (bool *)input->keys_down;
    else if (strcmp(key, "keys_pressed") == 0)
      keys_array = (bool *)input->keys_pressed;
    else if (strcmp(key, "keys_released") == 0)
      keys_array = (bool *)input->keys_released;
    ...
    profile_stop(PROFILE_LUA_INPUT_STATE_INDEX,
                 "input_state_lua_index (keys_table)");
    return 1;
  }
  ...
  if (strcmp(key, "mouse_down") == 0) {
    ...
    profile_stop(PROFILE_LUA_INPUT_STATE_INDEX,
                 "input_state_lua_index (mouse_down)");
    return 1;
  }
  if (strcmp(key, "mouse_clicked") == 0) {
    ...
    profile_stop(PROFILE_LUA_INPUT_STATE_INDEX,
                 "input_state_lua_index (mouse_clicked)");
    return 1;
  }
  if (strcmp(key, "mouse_released") == 0) {
    ...
    profile_stop(PROFILE_LUA_INPUT_STATE_INDEX,
                 "input_state_lua_index (mouse_released)");
    return 1;
  }
  ...
  if (strcmp(key, "KEY") == 0) {
    ...
    profile_stop(PROFILE_LUA_INPUT_STATE_INDEX,
                 "input_state_lua_index (KEY_table)");
    return 1;
  }

  profile_stop(PROFILE_LUA_INPUT_STATE_INDEX,
               "input_state_lua_index (invalid)");
  return 0;
}
 ...
 *
 * Provides write access to input state properties from Lua. Since input state
 * is read-only, this function always returns an error for any property
 * assignment attempts.
 *
 * @param L Lua state
 * @return Never returns (always calls luaL_error)
...
static int _input_state_lua_newindex(lua_State *L) {
  ...
  profile_stop(PROFILE_LUA_INPUT_STATE_NEWINDEX,
               "input_state_lua_newindex (error)");
 ...
 *
 * Converts an EseInputState to a human-readable string for debugging and
 * display. The format includes the memory address, mouse position, and visual
...
static int _input_state_lua_tostring(lua_State *L) {
  ...
  snprintf(buf, sizeof(buf),
           "Input: %p (mouse_x=%d, mouse_y=%d mouse_scroll_dx=%d, "
           "mouse_scroll_dy=%d)\n"
           "Down: "
           "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%"
           "c%c%c%c%c",
           (void *)input, ese_input_state_get_mouse_x(input),
           ese_input_state_get_mouse_y(input),
           ese_input_state_get_mouse_scroll_dx(input),
           ese_input_state_get_mouse_scroll_dy(input),
 ...
 *
 * @details This function registers the 'InputStateProxyMeta' metatable in the
 * Lua registry, which defines how the `EseInputState` C object interacts with
 * Lua scripts.
 *
...
void _ese_input_state_lua_init(EseLuaEngine *engine) {
  log_assert("INPUT_STATE", engine,
             "_ese_input_state_lua_init called with NULL engine");
  log_assert("INPUT_STATE", engine->runtime,
             "_ese_input_state_lua_init called with NULL engine->runtime");

  // Create main metatable
  lua_engine_new_object_meta(engine, INPUT_STATE_PROXY_META,
                             _input_state_lua_index, _input_state_lua_newindex,
                             _input_state_lua_gc, _input_state_lua_tostring);

  // Create KEY table metatable
  lua_engine_new_object_meta(engine, INPUT_STATE_PROXY_META "_KEY",
                             _input_state_key_index,
                             _input_state_readonly_error, NULL, NULL);
}
