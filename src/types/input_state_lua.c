...
static int _input_state_lua_gc(lua_State *L) {
    ...
    if (input) {
        ...
        // so we can free it.
        // If lua_ref != LUA_NOREF, this input was referenced from C and should
        // not be freed.
...
void _ese_input_state_lua_init(EseLuaEngine *engine) {
    ...
    lua_engine_new_object_meta(engine, INPUT_STATE_PROXY_META, _input_state_lua_index,
                               _input_state_lua_newindex, _input_state_lua_gc,
                               _input_state_lua_tostring);

    // Create KEY table metatable
    lua_engine_new_object_meta(engine, INPUT_STATE_PROXY_META "_KEY", _input_state_key_index,
