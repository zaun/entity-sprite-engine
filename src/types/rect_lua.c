...
static int _ese_rect_lua_gc(lua_State *L) {
    ...
    if (rect) {
        ...
        // so we can free it.
        // If lua_ref != LUA_NOREF, this rect was referenced from C and should
        // not be freed.
...
static int _ese_rect_lua_tostring(lua_State *L) {
    ...
    snprintf(buf, sizeof(buf), "(x=%.3f, y=%.3f, w=%.3f, h=%.3f, rot=%.3fdeg)",
             ...
             ese_rect_get_height(rect), (double)rad_to_deg(ese_rect_get_rotation(rect)));
...
static int _ese_rect_lua_new(lua_State *L) {
    ...
    if (n_args == 4) {
        if (lua_type(L, 1) != LUA_TNUMBER || lua_type(L, 2) != LUA_TNUMBER ||
            ...
            return luaL_error(L, "Rect.new(number, number, number, number) "
...
void _ese_rect_lua_init(EseLuaEngine *engine) {
    // Create metatable
    lua_engine_new_object_meta(engine, RECT_PROXY_META, _ese_rect_lua_index, _ese_rect_lua_newindex,
                               _ese_rect_lua_gc, _ese_rect_lua_tostring);
