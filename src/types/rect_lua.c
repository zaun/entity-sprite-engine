...
static int _ese_rect_lua_gc(lua_State *L) {
  ...
  if (rect) {
    ...
    // so we can free it.
    // If lua_ref != LUA_NOREF, this rect was referenced from C and should not
    // be freed.
 ...
 *
 * Provides read access to rect properties (x, y, width, height, rotation) from
 * Lua. When a Lua script accesses rect.x, rect.y, etc., this function is called
 * to retrieve the values. Also provides access to methods like contains_point,
 * intersects, and area.
 *
 * @param L Lua state
 ...
 *
 * Provides write access to rect properties (x, y, width, height, rotation) from
 * Lua. When a Lua script assigns to rect.x, rect.y, etc., this function is
 * called to update the values and notify any registered watchers of the change.
 *
 * @param L Lua state
 ...
 * The format includes the memory address and current x,y,width,height,rotation
 * values.
 *
 * @param L Lua state
...
static int _ese_rect_lua_tostring(lua_State *L) {
  ...
  char buf[160];
  snprintf(buf, sizeof(buf), "(x=%.3f, y=%.3f, w=%.3f, h=%.3f, rot=%.3fdeg)",
           ese_rect_get_x(rect), ese_rect_get_y(rect), ese_rect_get_width(rect),
           ese_rect_get_height(rect),
 ...
 * This function is called when Lua code executes `Rect.new(x, y, width,
 * height)`. It validates the arguments, creates the underlying EseRect, and
 * returns a proxy table that provides access to the rect's properties and
 * methods.
 *
 * @param L Lua state
...
static int _ese_rect_lua_new(lua_State *L) {
  ...
  if (n_args == 4) {
    if (lua_type(L, 1) != LUA_TNUMBER || lua_type(L, 2) != LUA_TNUMBER ||
      ...
      return luaL_error(
          L,
          "Rect.new(number, number, number, number) arguments must be numbers");
    }
  } else {
    profile_cancel(PROFILE_LUA_RECT_NEW);
    return luaL_error(
        L, "Rect.new(number, number, number, number) takes 4 arguments");
  }
 ...
 * This function is called when Lua code executes `Rect.zero()`. It's a
 * convenience constructor for creating rects at the default position and size.
 *
 * @param L Lua state
 * @return Number of values pushed onto the stack (always 1 - the proxy table)
 */
...
static int _ese_rect_lua_contains_point(lua_State *L) {
  ...
  if (n_args == 3) {
    if (lua_type(L, 2) != LUA_TNUMBER || lua_type(L, 3) != LUA_TNUMBER) {
      return luaL_error(
          L, "rect:contains_point(number, number) arguments must be numbers");
    }
    ...
    return luaL_error(
        L, "nrect:contains_point(point) takes 1 "
...
static int _ese_rect_lua_intersects(lua_State *L) {
  ...
  if (!other) {
    return luaL_error(L,
                      "rect:intersects(rect) requires another EseRect object");
  }
 ...
 *
 * Creates a new EseRect from a JSON string. This function is called when Lua
 * code executes `Rect.fromJSON(json_string)`. It parses the JSON string,
 * validates it contains rect data, and creates a new rect with the specified
...
static int _ese_rect_lua_from_json(lua_State *L) {
  ...
  if (!json) {
    log_error("RECT", "Rect.fromJSON: failed to parse JSON string: %s",
 ...
 *
 * Converts an EseRect to a JSON string representation. This function is called
 * when Lua code executes `rect:toJSON()`. It serializes the rect's coordinates,
 * dimensions, and rotation to JSON format and returns the string.
 *
 * @param L Lua state
...
void _ese_rect_lua_init(EseLuaEngine *engine) {
  // Create metatable
  lua_engine_new_object_meta(engine, RECT_PROXY_META, _ese_rect_lua_index,
                             _ese_rect_lua_newindex, _ese_rect_lua_gc,
                             _ese_rect_lua_tostring);

  // Create global Rect table with functions
  const char *keys[] = {"new", "zero", "fromJSON"};
  lua_CFunction functions[] = {_ese_rect_lua_new, _ese_rect_lua_zero,
                               _ese_rect_lua_from_json};
