...
typedef struct EseDrawListPolyLine {
  EseDrawListPoint
      points[POLYLINE_MAX_POINTS]; /** Array of points defining the polyline */
...
struct EseDrawListObject {
  ...
  EseDrawListObjectType
      type; /** Type of object (texture, rectangle, or polyline) */
  union {
...
struct EseDrawList {
  ...
  EseAtomicSizeT
      *objects_count;      /** Number of objects in use this frame (atomic) */
...
EseDrawList *draw_list_create(void) {
  EseDrawList *draw_list =
      memory_manager.malloc(sizeof(EseDrawList), MMTAG_DRAWLIST);
...
void draw_list_destroy(EseDrawList *draw_list) {
  log_assert("RENDER_LIST", draw_list,
             "draw_list_destroy called with NULL draw_list");
...
void draw_list_clear(EseDrawList *draw_list) {
  log_assert("RENDER_LIST", draw_list,
             "draw_list_clear called with NULL draw_list");
...
EseDrawListObject *draw_list_request_object(EseDrawList *draw_list) {
  log_assert("RENDER_LIST", draw_list,
             "draw_list_request_object called with NULL draw_list");
  ...
  if (index >= draw_list->objects_capacity) {
    size_t new_capacity = draw_list->objects_capacity
                              ? draw_list->objects_capacity * 2
                              : DRAW_LIST_INITIAL_CAPACITY;
    if (new_capacity <= index)
      new_capacity = index + 1;
    EseDrawListObject **new_objs = memory_manager.realloc(
        draw_list->objects, sizeof(EseDrawListObject *) * new_capacity,
        MMTAG_DRAWLIST);
    if (!new_objs) {
  ...
  if (obj->magic != EDL_OBJ_MAGIC) {
    log_error("RENDER_LIST", "object magic corrupted idx=%zu magic=0x%x", index,
              obj->magic);
    abort();
  }
  return obj;
...
void draw_list_sort(EseDrawList *draw_list) {
  log_assert("RENDER_LIST", draw_list,
             "draw_list_sort called with NULL draw_list");
  ...
  qsort(draw_list->objects, count, sizeof(EseDrawListObject *),
        _compare_draw_list_object_z);
  ese_mutex_unlock(draw_list->mutex);
}

size_t draw_list_get_object_count(const EseDrawList *draw_list) {
  log_assert("RENDER_LIST", draw_list,
             "draw_list_get_object_count called with NULL draw_list");
...
EseDrawListObject *draw_list_get_object(const EseDrawList *draw_list,
                                        size_t index) {
  log_assert("RENDER_LIST", draw_list,
             "draw_list_get_object called with NULL draw_list");
...
void draw_list_object_set_texture(EseDrawListObject *object,
  ...
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_texture called with NULL object");
...
void draw_list_object_set_rect_color(EseDrawListObject *object, unsigned char r,
  ...
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_rect_color called with NULL object");
...
void draw_list_object_set_bounds(EseDrawListObject *object, float x, float y,
                                 int w, int h) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_bounds called with NULL object");
  ...
  // Note: Polyline objects don't use width/height as they are defined by their
  // points
}

void draw_list_object_set_z_index(EseDrawListObject *object, uint64_t z_index) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_z_index called with NULL object");
...
uint64_t draw_list_object_get_z_index(const EseDrawListObject *object) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_z_index called with NULL object");
...
void draw_list_object_get_bounds(const EseDrawListObject *object, float *x,
                                 float *y, int *w, int *h) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_bounds called with NULL object");
...
void draw_list_object_get_texture(const EseDrawListObject *object,
                                  ...
                                  float *texture_y2) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_texture called with NULL object");
...
void draw_list_object_get_rect_color(const EseDrawListObject *object,
                                     ...
                                     bool *filled) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_rect_color called with NULL object");
...
void draw_list_object_set_polyline(EseDrawListObject *object,
                                   ...
                                   float stroke_width) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_polyline called with NULL object");
  log_assert("RENDER_LIST", points,
             "draw_list_object_set_polyline called with NULL points");
...
void draw_list_object_get_polyline(const EseDrawListObject *object,
                                   ...
                                   float *stroke_width) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_polyline called with NULL object");
  ...
  if (points)
    *points =
        (const float *)
            polyline_data->points; // Cast to float* for x1,y1,x2,y2,... format
  if (point_count)
...
void draw_list_object_set_polyline_color(EseDrawListObject *object,
  ...
  log_assert(
      "RENDER_LIST", object->type == DL_POLYLINE,
...
void draw_list_object_get_pivot(const EseDrawListObject *object, float *nx,
                                float *ny) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_pivot called with NULL object");
  if (nx)
    *nx = object->rot_x;
  if (ny)
    *ny = object->rot_y;
...
void draw_list_object_get_rotated_aabb(const EseDrawListObject *object,
                                       ...
                                       float *maxy) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_rotated_aabb called with NULL object");
...
void draw_list_object_set_mesh(EseDrawListObject *object,
  ...
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_mesh called with NULL object");
  log_assert("RENDER_LIST", verts,
             "draw_list_object_set_mesh called with NULL verts");
  log_assert("RENDER_LIST", indices,
             "draw_list_object_set_mesh called with NULL indices");
  log_assert("RENDER_LIST", texture_id,
             "draw_list_object_set_mesh called with NULL texture_id");
  log_assert(
      "RENDER_LIST", vert_count <= MESH_MAX_VERTS,
      "draw_list_object_set_mesh called with vert_count > MESH_MAX_VERTS");
  log_assert(
      "RENDER_LIST", idx_count <= MESH_MAX_INDICES,
...
void draw_list_object_get_mesh(const EseDrawListObject *object,
  ...
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_mesh called with NULL object");
...
void draw_list_object_set_scissor(EseDrawListObject *object, float scissor_x,
                                  ...
                                  float scissor_h) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_set_scissor called with NULL object");
...
void draw_list_object_get_scissor(const EseDrawListObject *object,
                                  ...
                                  float *scissor_h) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_get_scissor called with NULL object");
...
void draw_list_object_clear_scissor(EseDrawListObject *object) {
  log_assert("RENDER_LIST", object,
             "draw_list_object_clear_scissor called with NULL object");
