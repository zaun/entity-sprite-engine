...
static EseRenderBatch *_render_batch_create(void) {
  EseRenderBatch *batch =
      memory_manager.malloc(sizeof(EseRenderBatch), MMTAG_RENDERLIST);

  memset(batch, 0, sizeof(EseRenderBatch));
  batch->vertex_buffer = memory_manager.malloc(
      sizeof(EseVertex) * BATCH_INITIAL_CAPACITY * 4, MMTAG_RENDERLIST);
...
static void _render_batch_destroy(EseRenderBatch *batch) {
  log_assert("RENDER_LIST", batch,
             "_render_batch_destroy called with NULL batch");
...
static void _render_list_add_batch(EseRenderList *render_list,
                                   EseRenderBatch *batch) {
  log_assert("RENDER_LIST", render_list,
             "_render_list_add_batch called with NULL render_list");
  log_assert("RENDER_LIST", batch,
             "_render_list_add_batch called with NULL batch");

  if (render_list->batch_count >= render_list->batch_capacity) {
    // Grow draw list
    size_t new_capacity = render_list->batch_capacity * 2;
    EseRenderBatch **new_batches = memory_manager.realloc(
        render_list->batches, sizeof(EseRenderBatch *) * new_capacity,
        MMTAG_RENDERLIST);
    if (new_batches) {
...
static void _rotate_point(float lx, float ly, float px, float py, float rot,
                          float *ox, float *oy) {
  float dx = lx - px;
  float dy = ly - py;
  float cr = cosf(rot);
...
static inline void _pixel_to_ndc(float px, float py, float view_w, float view_h,
                                 float *nx, float *ny) {
...
static void _tessellate_polyline_fill(const EseDrawListObject *obj,
                                      EseVertex *vertices, size_t *vertex_count,
  ...
  bool is_closed =
      (point_count > 3 && points[0] == points[(point_count - 1) * 2] &&
  ...
  _pixel_to_ndc(centroid_x + screen_x, centroid_y + screen_y, view_w, view_h,
                &center_x, &center_y);

  // Triangulate the entire polygon with a simple fan around the centroid.
  // If the last point equals the first, treat it as a duplicate and ignore it
  // in the fan.
  ...
  for (size_t i = 0; i < limit; ++i) {
    ...
    float x1, y1, x2, y2;
    _pixel_to_ndc(points[idx1 * 2] + screen_x, points[idx1 * 2 + 1] + screen_y,
                  view_w, view_h, &x1, &y1);
    _pixel_to_ndc(points[idx2 * 2] + screen_x, points[idx2 * 2 + 1] + screen_y,
                  view_w, view_h, &x2, &y2);

    vertices[(*vertex_count)++] =
        (EseVertex){center_x, center_y, 0.0f, 0.0f, 0.0f};
...
static void _tessellate_polyline_stroke(const EseDrawListObject *obj,
                                        EseVertex *vertices,
                                        size_t *vertex_count, int view_w,
                                        int view_h) {
  const float *points;
  size_t point_count;
  float stroke_width;
  ...
  // For closed polygons, if last point is duplicate of first, process
  // (point_count - 1) lines Otherwise process (point_count - 1) lines
  size_t line_count;
  if (point_count > 2 && points[0] == points[(point_count - 1) * 2] &&
      points[1] == points[(point_count - 1) * 2 + 1]) {
    line_count =
        point_count -
        1; // Last point is duplicate of first, so process one fewer line
  } else {
  ...
  for (size_t i = 0; i < line_count; i++) {
    float x1, y1, x2, y2;
    _pixel_to_ndc(points[i * 2] + screen_x, points[i * 2 + 1] + screen_y,
                  view_w, view_h, &x1, &y1);
    _pixel_to_ndc(points[(i + 1) * 2] + screen_x,
                  points[(i + 1) * 2 + 1] + screen_y, view_w, view_h, &x2, &y2);
    ...
    vertices[(*vertex_count)++] =
        (EseVertex){x1_left, y1_left, 0.0f, 0.0f, 0.0f};
    vertices[(*vertex_count)++] =
        (EseVertex){x1_right, y1_right, 0.0f, 0.0f, 0.0f};
    vertices[(*vertex_count)++] =
        (EseVertex){x2_left, y2_left, 0.0f, 0.0f, 0.0f};

    vertices[(*vertex_count)++] =
        (EseVertex){x1_right, y1_right, 0.0f, 0.0f, 0.0f};
    vertices[(*vertex_count)++] =
        (EseVertex){x2_right, y2_right, 0.0f, 0.0f, 0.0f};
    vertices[(*vertex_count)++] =
        (EseVertex){x2_left, y2_left, 0.0f, 0.0f, 0.0f};
  }
}
...
static void
_render_batch_add_polyline_fill_vertices(EseRenderBatch *batch,
                                         const EseDrawListObject *obj,
                                         int view_w, int view_h) {
  log_assert("RENDER_LIST", batch,
             "_render_batch_add_polyline_fill_vertices called with NULL batch");
  log_assert("RENDER_LIST", obj,
             "_render_batch_add_polyline_fill_vertices called with NULL obj");
  ...
  if (batch->vertex_count + fill_vertices > batch->vertex_capacity) {
    ...
    while (batch->vertex_count + fill_vertices > new_capacity)
      new_capacity *= 2;
    EseVertex *new_buffer = memory_manager.realloc(
        batch->vertex_buffer, sizeof(EseVertex) * new_capacity,
        MMTAG_RENDERLIST);
    if (new_buffer) {
  ...
  _tessellate_polyline_fill(obj, &v[vertex_offset], &vertex_offset, view_w,
                            view_h);
...
static void
_render_batch_add_polyline_stroke_vertices(EseRenderBatch *batch,
                                           const EseDrawListObject *obj,
                                           int view_w, int view_h) {
  log_assert(
      "RENDER_LIST", batch,
      "_render_batch_add_polyline_stroke_vertices called with NULL batch");
  log_assert("RENDER_LIST", obj,
             "_render_batch_add_polyline_stroke_vertices called with NULL obj");
  ...
  size_t line_count =
      (point_count > 2 && points[0] == points[(point_count - 1) * 2] &&
       points[1] == points[(point_count - 1) * 2 + 1])
  ...
  if (batch->vertex_count + stroke_vertices > batch->vertex_capacity) {
    size_t new_capacity = batch->vertex_capacity * 2;
    while (batch->vertex_count + stroke_vertices > new_capacity)
      new_capacity *= 2;
    EseVertex *new_buffer = memory_manager.realloc(
        batch->vertex_buffer, sizeof(EseVertex) * new_capacity,
        MMTAG_RENDERLIST);
  ...
  _tessellate_polyline_stroke(obj, &v[vertex_offset], &vertex_offset, view_w,
                              view_h);
...
static void _render_batch_add_object_vertices(EseRenderBatch *batch,
                                              const EseDrawListObject *obj,
                                              int view_w, int view_h) {
  log_assert("RENDER_LIST", batch,
             "_render_batch_add_object_vertices called with NULL batch");
  log_assert("RENDER_LIST", obj,
             "_render_batch_add_object_vertices called with NULL obj");
  ...
  // x ranges from -1 to 1 (left to right), y ranges from -1 to 1 (bottom to
  // top)
  ...
  if (batch->vertex_count + 6 >
      batch->vertex_capacity) { // Each quad has 6 vertices
    // Grow vertex buffer
    size_t new_capacity = batch->vertex_capacity * 2;
    EseVertex *new_buffer = memory_manager.realloc(
        batch->vertex_buffer, sizeof(EseVertex) * new_capacity,
  ...
  if (draw_list_object_get_type(obj) == DL_TEXTURE) {
    ...
    if (fabsf(rot) < 1e-6f) {
      ...
      for (int i = 0; i < 4; ++i) {
        _rotate_point(oxp[i], oyp[i], px_pix, py_pix, rot, &r_oxp[i],
                      &r_oyp[i]);
      }
    }
    ...
    if (filled) {
      ...
      for (int i = 0; i < 4; ++i)
        _pixel_to_ndc(r_oxp[i], r_oyp[i], view_w, view_h, &ndc_px[i],
                      &ndc_py[i]);
      ...
      /* Hollow rectangle: create inner rect in pixels, rotate both outer and
       * inner, convert to NDC. */
      ...
      if (inner_w <= 0.0f || inner_h <= 0.0f) {
        ...
        ixp[0] = inner_x;
        iyp[0] = inner_y; /* TL */
        ixp[1] = inner_x + inner_w;
        iyp[1] = inner_y; /* TR */
        ixp[2] = inner_x + inner_w;
        iyp[2] = inner_y + inner_h; /* BR */
        ixp[3] = inner_x;
        iyp[3] = inner_y + inner_h; /* BL */
        ...
        if (fabsf(rot) < 1e-6f) {
          ...
          for (int i = 0; i < 4; ++i) {
            _rotate_point(ixp[i], iyp[i], px_pix, py_pix, rot, &r_ixp[i],
                          &r_iyp[i]);
          }
        }
        ...
        for (int i = 0; i < 4; ++i)
          _pixel_to_ndc(r_oxp[i], r_oyp[i], view_w, view_h, &odx[i], &ody[i]);
        for (int i = 0; i < 4; ++i)
          _pixel_to_ndc(r_ixp[i], r_iyp[i], view_w, view_h, &idx[i], &idy[i]);
    ...
    draw_list_object_get_polyline_color(obj, &fill_r, &fill_g, &fill_b,
                                        &fill_a);

    unsigned char stroke_r, stroke_g, stroke_b, stroke_a;
    draw_list_object_get_polyline_stroke_color(obj, &stroke_r, &stroke_g,
                                               &stroke_b, &stroke_a);
    ...
    if (fill_a > 0 && point_count >= 3) {
      // For closed polygons, if last point is duplicate of first, use
      // (point_count - 3) triangles Otherwise use (point_count - 2) triangles
      size_t triangle_count =
          (point_count > 3 && points[0] == points[(point_count - 1) * 2] &&
           points[1] == points[(point_count - 1) * 2 + 1])
              ? point_count - 3           // Last point is duplicate of first
    ...
    if (stroke_a > 0 && point_count >= 2) {
      // For closed polygons, if last point is duplicate of first, use
      // (point_count - 1) lines Otherwise use (point_count - 1) lines
      size_t line_count =
          (point_count > 2 && points[0] == points[(point_count - 1) * 2] &&
           points[1] == points[(point_count - 1) * 2 + 1])
              ? point_count - 1  // Last point is duplicate of first
      ...
      stroke_vertices =
          line_count * 6; // Quads for stroke (6 vertices per line segment)
    }
    ...
    if (batch->vertex_count + total_vertices > batch->vertex_capacity) {
      size_t new_capacity = batch->vertex_capacity * 2;
      while (batch->vertex_count + total_vertices > new_capacity)
        new_capacity *= 2;
      EseVertex *new_buffer = memory_manager.realloc(
          batch->vertex_buffer, sizeof(EseVertex) * new_capacity,
          MMTAG_RENDERLIST);
    ...
    if (stroke_vertices > 0) {
      _tessellate_polyline_stroke(obj, &v[vertex_offset], &vertex_offset,
                                  view_w, view_h);
    }
    ...
    draw_list_object_get_mesh(obj, &mesh_verts, &vert_count, &mesh_indices,
                              &idx_count, NULL);

    // Ensure we have enough capacity for all mesh vertices
    if (batch->vertex_count + vert_count > batch->vertex_capacity) {
      size_t new_capacity = batch->vertex_capacity * 2;
      while (batch->vertex_count + vert_count > new_capacity)
        new_capacity *= 2;
      EseVertex *new_buffer = memory_manager.realloc(
          batch->vertex_buffer, sizeof(EseVertex) * new_capacity,
          MMTAG_RENDERLIST);
    ...
    for (size_t i = 0; i < vert_count; ++i) {
      ...
      v[i] = (EseVertex){
          ...
          mesh_verts[i].u,
          mesh_verts[i].v // UV coordinates (already normalized)
      };
    }
...
EseRenderList *render_list_create(void) {
  EseRenderList *render_list =
      memory_manager.malloc(sizeof(EseRenderList), MMTAG_RENDERLIST);

  render_list->batches = memory_manager.malloc(sizeof(EseRenderBatch *) *
                                                   RENDER_LIST_INITIAL_CAPACITY,
                                               MMTAG_RENDERLIST);
...
void render_list_destroy(EseRenderList *render_list) {
  log_assert("RENDER_LIST", render_list,
             "render_list_destroy called with NULL render_list");
...
void render_list_clear(EseRenderList *render_list) {
  log_assert("RENDER_LIST", render_list,
             "render_list_clear called with NULL render_list");
...
void render_list_fill(EseRenderList *render_list, EseDrawList *draw_list) {
  log_assert("RENDER_LIST", render_list,
             "render_list_fill called with NULL render_list");
  log_assert("RENDER_LIST", draw_list,
             "render_list_fill called with NULL draw_list");
  ...
  for (size_t i = 0; i < draw_list_get_object_count(draw_list); ++i) {
    ...
    if (draw_list_object_get_type(obj) == DL_POLYLINE) {
      ...
      if (fill_vertices > 0) {
        ...
        _render_batch_add_polyline_fill_vertices(
            current_batch, obj, render_list->width, render_list->height);
      }
      ...
      if (stroke_vertices > 0) {
        ...
        _render_batch_add_polyline_stroke_vertices(
            current_batch, obj, render_list->width, render_list->height);
      }
    ...
    if (!current_batch) {
      ...
      draw_list_object_get_texture(obj, &current_texture_id, NULL, NULL, NULL,
                                   NULL);
      if (strcmp(current_texture_id, current_batch->shared_state.texture_id) !=
          0) {
        new_batch_needed = true;
      }
    ...
    // Check scissor rectangles - objects can only be batched if they have the
    // same scissor state
    if (!new_batch_needed) {
      bool obj_scissor_active;
      float obj_scissor_x, obj_scissor_y, obj_scissor_w, obj_scissor_h;

      draw_list_object_get_scissor(obj, &obj_scissor_active, &obj_scissor_x,
                                   &obj_scissor_y, &obj_scissor_w,
                                   &obj_scissor_h);
      ...
      if (obj_scissor_active != current_batch->scissor_active) {
        ...
        if (obj_scissor_x != current_batch->scissor_x ||
            obj_scissor_y != current_batch->scissor_y ||
            obj_scissor_w != current_batch->scissor_w ||
            obj_scissor_h != current_batch->scissor_h) {
          new_batch_needed = true;
        }
      }
    }
    ...
    if (new_batch_needed) {
      ...
      if (draw_list_object_get_type(obj) == DL_TEXTURE) {
        ...
        draw_list_object_get_texture(obj,
                                     &current_batch->shared_state.texture_id,
                                     NULL, NULL, NULL, NULL);
      } else if (draw_list_object_get_type(obj) == DL_RECT) {
        current_batch->type = RL_COLOR;
        draw_list_object_get_rect_color(
            obj, &current_batch->shared_state.color.r,
        ...
        draw_list_object_get_mesh(obj, NULL, NULL, NULL, NULL,
                                  &current_batch->shared_state.texture_id);
      }
    ...
    _render_batch_add_object_vertices(current_batch, obj, render_list->width,
                                      render_list->height);
  }
  // log_debug("RENDER_LIST", "Batch has %d draw calls", draw_calls);
}
...
size_t render_list_get_batch_count(const EseRenderList *render_list) {
  log_assert("RENDER_LIST", render_list,
             "render_list_get_batch_count called with NULL render_list");

  return render_list->batch_count;
}

const EseRenderBatch *render_list_get_batch(const EseRenderList *render_list,
                                            size_t batch_number) {
  log_assert("RENDER_LIST", render_list,
             "render_list_get_batch_count called with NULL render_list");
  log_assert("RENDER_LIST", render_list,
             "render_list_get_batch_count called with batch_number %zu out of "
             "bounds (max %zu)",
             batch_number, render_list->batch_count - 1);
