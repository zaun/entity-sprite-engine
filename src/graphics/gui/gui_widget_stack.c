...
static void _stack_draw(EseGui *gui, EseGuiWidget *widget,
                        EseDrawList *draw_list, size_t depth);
static void _stack_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
                                       int mouse_y);
static bool _stack_process_mouse_click(EseGuiWidget *widget, int mouse_x,
...
static void _stack_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
  ...
  log_assert("GUI", widget,
             "_stack_process_mouse_hover called with NULL widget");
...
static bool _stack_process_mouse_click(EseGuiWidget *widget, int mouse_x,
  ...
  log_assert("GUI", widget,
             "_stack_process_mouse_click called with NULL widget");
  (void)button;
...
static void _stack_layout(EseGuiWidget *widget) {
  ...
  const int child_w =
      widget->width - (ese_gui_style_get_padding_left(widget->style) +
                       ese_gui_style_get_padding_right(widget->style));
  const int child_h =
      widget->height - (ese_gui_style_get_padding_top(widget->style) +
...
static int _stack_lua_init(EseLuaEngine *engine) {
  ...
  log_assert("GUI", engine->runtime,
             "_stack_lua_init called with NULL lua runtime");
...
static int _stack_lua_open(lua_State *L) {
  ...
  if (n_args < 0 || n_args > 3) {
    return luaL_error(
        L,
        "GUI.open_stack([width[, height[, style]]]) takes up to 3 arguments");
  }
  ...
  if (gui->open_layout->root == NULL &&
    ...
    gui->open_layout->current_widget = _stack_create(
    ...
    return luaL_error(L,
                      "GUI.open_stack() called with no open container active");
  } else {
    EseGuiWidget *new_stack = _stack_create(
...
static int _stack_lua_close(lua_State *L) {
  ...
  if (current == NULL ||
      strncmp(current->type.id, "STACK", sizeof(current->type.id)) != 0) {
    return luaL_error(
        L, "GUI.close_stack() called but current container is not a Stack");
  }
