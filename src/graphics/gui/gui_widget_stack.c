...
static bool _stack_process_mouse_click(EseGuiWidget *widget, int mouse_x, int mouse_y, EseInputMouseButton button);
...
static void _stack_layout(EseGuiWidget *widget) {
	...
	const int child_w = widget->width - (ese_gui_style_get_padding_left(widget->style) + ese_gui_style_get_padding_right(widget->style));
	const int child_h = widget->height - (ese_gui_style_get_padding_top(widget->style) + ese_gui_style_get_padding_bottom(widget->style));
...
static int _stack_lua_open(lua_State *L) {
	...
	if (n_args < 0 || n_args > 3) {
		return luaL_error(L, "GUI.open_stack([width[, height[, style]]]) takes up to 3 arguments");
	}
	...
	if (gui->open_layout == NULL) {
	   ...
	   if (gui->open_layout->root == NULL && gui->open_layout->current_widget == NULL) {
		...
		gui->open_layout->current_widget = _stack_create(NULL, ese_gui_style_copy(opt_style ? opt_style : gui->default_style));
		...
		EseGuiWidget *new_stack = _stack_create(gui->open_layout->current_widget, ese_gui_style_copy(opt_style ? opt_style : gui->default_style));
