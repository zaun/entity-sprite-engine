...
// Note: memory_manager will never return a NULL pointer, so we don't need to
// check for NULL
...
void ese_gui_process(EseGui *gui, EseDrawList *draw_list) {
  ...
  for (size_t frame_idx = 0; frame_idx < gui->layouts_count; frame_idx++) {
    ...
    if (layout->root->width == 0 || layout->root->width == GUI_AUTO_SIZE)
      layout->root->width = layout->width;
    if (layout->root->height == 0 || layout->root->height == GUI_AUTO_SIZE)
      layout->root->height = layout->height;
    _ese_widget_layout(layout->root, layout->x, layout->y, layout->root->width,
                       layout->root->height);
  }
...
EseGuiStyle *ese_gui_get_default_style(EseGui *gui) {
  log_assert("GUI", gui != NULL,
             "ese_gui_get_default_style called with NULL gui");
  return gui->default_style;
}

void ese_gui_set_default_style(EseGui *gui, EseGuiStyle *style) {
  log_assert("GUI", gui != NULL,
             "ese_gui_set_default_style called with NULL gui");
  log_assert("GUI", style != NULL,
             "ese_gui_set_default_style called with NULL style");
...
void ese_gui_reset_default_style(EseGui *gui) {
  log_assert("GUI", gui != NULL,
             "ese_gui_reset_default_style called with NULL gui");
...
EseGuiStyleVariant ese_gui_get_top_variant(EseGui *gui) {
  log_assert("GUI", gui != NULL,
             "ese_gui_get_top_variant called with NULL gui");
  log_assert("GUI", gui->open_layout != NULL,
             "ese_gui_get_top_variant called with no open layout");
  log_assert("GUI", gui->open_layout->variant_stack_count > 0,
             "ese_gui_get_top_variant called with empty variant stack");

  return gui->open_layout
      ->variant_stack[gui->open_layout->variant_stack_count - 1];
}
...
void ese_gui_push_variant(EseGui *gui, EseGuiStyleVariant variant) {
  ...
  log_assert("GUI", gui->open_layout != NULL,
             "ese_gui_push_variant called with no open layout");
  log_assert("GUI", gui->open_layout->variant_stack_count < MAX_VARIANT_STACK,
             "ese_gui_push_variant called with full variant stack");

  gui->open_layout->variant_stack[gui->open_layout->variant_stack_count++] =
      variant;
}
...
void ese_gui_pop_variant(EseGui *gui) {
  ...
  log_assert("GUI", gui->open_layout != NULL,
             "ese_gui_pop_variant called with no open layout");
  log_assert("GUI", gui->open_layout->variant_stack_count > 1,
             "ese_gui_pop_variant called with only default variant on stack");
