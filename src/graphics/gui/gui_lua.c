...
void ese_gui_lua_init(EseLuaEngine *engine) {
  ...
  // Create global GUI table with session begin/end; widgets register their own
  // functions
...
static int _ese_gui_lua_begin(lua_State *L) {
  ...
  if (n_args == 5) {
    ...
    return luaL_error(
        L, "GUI.start() takes 5 arguments (draw_order, x, y, width, height)");
  }
  ...
  if (gui->layouts_count >= gui->layouts_capacity) {
    ...
    log_error("GUI",
              "ese_gui_begin called with no capacity to grow frame stack");
    return 0;
  }
  ...
  layout->variant_stack[layout->variant_stack_count++] =
      GUI_STYLE_VARIANT_DEFAULT;
...
static int _ese_gui_lua_get_default_style(lua_State *L) {
  log_assert("GUI_LUA", L,
             "ese_gui_lua_get_default_style called with NULL Lua state");
...
static int _ese_gui_lua_set_default_style(lua_State *L) {
  log_assert("GUI_LUA", L,
             "ese_gui_lua_set_default_style called with NULL Lua state");
  ...
  if (!style) {
    return luaL_error(
        L, "GUI.set_default_style() requires a valid GuiStyle object");
  }
...
static int _ese_gui_lua_reset_default_style(lua_State *L) {
  log_assert("GUI_LUA", L,
             "ese_gui_lua_reset_default_style called with NULL Lua state");
...
static int _ese_gui_lua_push_variant(lua_State *L) {
  log_assert("GUI_LUA", L,
             "ese_gui_lua_push_variant called with NULL Lua state");
  ...
  if (!lua_isnumber(L, 1)) {
    return luaL_error(
        L, "GUI.push_variant() requires a variant constant (number)");
  }
...
static int _ese_gui_lua_pop_variant(lua_State *L) {
  log_assert("GUI_LUA", L,
             "ese_gui_lua_pop_variant called with NULL Lua state");
  ...
  if (gui->open_layout->variant_stack_count <= 1) {
    return luaL_error(
        L, "GUI.pop_variant() called with only default variant on stack");
  }
