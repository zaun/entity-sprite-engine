...
void _ese_widget_process_mouse_hover(EseGuiWidget *widget, int mouse_x, int mouse_y) {
    log_assert("GUI", widget, "_ese_widget_process_mouse_hover called with NULL widget");
...
static bool _ese_widget_process_mouse_clicked_worker(
    EseGuiWidget *widget,
    int mouse_x, int mouse_y,
    EseInputMouseButton button
) {
    log_assert("GUI", widget, "_ese_widget_process_mouse_clicked_worker called with NULL widget");

    // Process children first so the deepest children can handle the click first
    for (size_t i = 0; i < widget->children_count; i++) {
        if (_ese_widget_process_mouse_clicked_worker(widget->children[i], mouse_x, mouse_y, button)) {
            return true;
        }
    }
...
void _ese_widget_process_mouse_clicked(EseGuiWidget *widget, int mouse_x, int mouse_y, EseInputMouseButton button) {
    log_assert("GUI", widget, "ese_widget_process_mouse_click called with NULL widget");
