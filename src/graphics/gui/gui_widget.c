...
static bool _ese_widget_process_mouse_clicked_worker(EseGuiWidget *widget, int mouse_x, int mouse_y,
    ...
    for (size_t i = 0; i < widget->children_count; i++) {
        if (_ese_widget_process_mouse_clicked_worker(widget->children[i], mouse_x, mouse_y,
                                                     button)) {
            return true;
        }
    }
