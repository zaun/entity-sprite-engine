...
static bool _button_process_mouse_click(EseGuiWidget *widget, int mouse_x, int mouse_y,
...
static GuiButtonData *_button_data_create(void) {
    GuiButtonData *data =
        (GuiButtonData *)memory_manager.calloc(1, sizeof(GuiButtonData), MMTAG_GUI);
    data->text = NULL;
    data->callback = NULL;
    data->userdata = NULL;
    return data;
...
static void _button_draw(EseGui *gui, EseGuiWidget *widget, EseDrawList *draw_list, size_t depth) {
    ...
    draw_list_object_set_rect_color(bg_obj, (unsigned char)(ese_color_get_r(bg) * 255),
                                    (unsigned char)(ese_color_get_g(bg) * 255),
                                    (unsigned char)(ese_color_get_b(bg) * 255),
                                    (unsigned char)(ese_color_get_a(bg) * 255),
                                    true // filled
    );
    draw_list_object_set_bounds(bg_obj, widget->x, widget->y, widget->width, widget->height);
    draw_list_object_set_z_index(bg_obj, z_index);

    // --------- DRAW BORDER ----------
    if (border_width > 0) {
        EseDrawListObject *border_obj = draw_list_request_object(draw_list);
        draw_list_object_set_rect_color(border_obj, (unsigned char)(ese_color_get_r(border) * 255),
    ...
    if (data->text != NULL && font_size > 0) {
        ...
        font_draw_text_scaled(engine, "console_font_10x20", data->text, text_x, text_y, z_index + 2,
                              (float)font_size, _button_font_texture_callback, draw_list);
    }
}
...
static void _button_font_texture_callback(float screen_x, float screen_y, float screen_w,
                                          float screen_h, uint64_t z_index, const char *texture_id,
                                          float texture_x1, float texture_y1, float texture_x2,
                                          float texture_y2, int width, int height,
                                          void *user_data) {
...
void ese_widget_button_set(EseGuiWidget *button, const char *text, void (*callback)(void *),
                           void *userdata) {
...
static int _button_lua_push(lua_State *L) {
    ...
    EseGuiWidget *button =
        _button_create(parent, ese_gui_style_copy(opt_style ? opt_style : gui->default_style));
