...
static void _button_draw(EseGui *gui, EseGuiWidget *widget,
                         EseDrawList *draw_list, size_t depth);
static void _button_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
                                        int mouse_y);
static bool _button_process_mouse_click(EseGuiWidget *widget, int mouse_x,
                                        int mouse_y,
...
static void _button_font_texture_callback(
...
static GuiButtonData *_button_data_create(void) {
  GuiButtonData *data = (GuiButtonData *)memory_manager.calloc(
      1, sizeof(GuiButtonData), MMTAG_GUI);
  data->text = NULL;
  data->callback = NULL;
  data->userdata = NULL;
  return data;
...
GuiWidgetVTable *ese_widget_button_get_vtable(void) { return &g_button_vtable; }
...
static void _button_draw(EseGui *gui, EseGuiWidget *widget,
  ...
  if (border_width > 0) {
    ...
    draw_list_object_set_rect_color(
        border_obj, (unsigned char)(ese_color_get_r(border) * 255),
        (unsigned char)(ese_color_get_g(border) * 255),
        (unsigned char)(ese_color_get_b(border) * 255),
        (unsigned char)(ese_color_get_a(border) * 255),
        false // border only
    );
    draw_list_object_set_bounds(border_obj, widget->x, widget->y, widget->width,
                                widget->height);
  ...
  if (data->text != NULL && font_size > 0) {
    float text_width =
        strlen(data->text) * (font_size * 0.6f); // Approximate character width
    ...
    EseEngine *engine = (EseEngine *)lua_engine_get_registry_key(
        gui->engine->runtime, ENGINE_KEY);
    // Use font_draw_text_scaled to draw the text
    font_draw_text_scaled(engine, "console_font_10x20", data->text, text_x,
                          text_y, z_index + 2, (float)font_size,
...
static void _button_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
  ...
  log_assert("GUI", widget,
             "_button_process_mouse_hover called with NULL widget");
  const bool inside =
...
static bool _button_process_mouse_click(EseGuiWidget *widget, int mouse_x,
                                        ...
                                        EseInputMouseButton button) {
  log_assert("GUI", widget,
             "_button_process_mouse_click called with NULL widget");
  (void)button;
...
static void _button_layout(EseGuiWidget *widget) { (void)widget; }
...
static int _button_lua_init(EseLuaEngine *engine) {
  ...
  log_assert("GUI", engine->runtime,
             "_button_lua_init called with NULL lua runtime");
...
void ese_widget_button_set(EseGuiWidget *button, const char *text,
...
static int _button_lua_push(lua_State *L) {
  ...
  if (n_args < 2 || n_args > 4) {
    return luaL_error(L, "GUI.push_button(text, callback[, userdata[, style]]) "
  ...
  EseGuiWidget *button = _button_create(
      parent, ese_gui_style_copy(opt_style ? opt_style : gui->default_style));
  ...
  LuaButtonCallbackData *cb = (LuaButtonCallbackData *)memory_manager.calloc(
      1, sizeof(LuaButtonCallbackData), MMTAG_GUI);
  cb->L = L;
  cb->ref = ref;
