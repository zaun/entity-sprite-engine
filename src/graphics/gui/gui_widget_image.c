...
static void _image_draw(EseGui *gui, EseGuiWidget *widget,
                        EseDrawList *draw_list, size_t depth);
static void _image_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
                                       int mouse_y);
static bool _image_process_mouse_click(EseGuiWidget *widget, int mouse_x,
...
static GuiImageData *_image_data_create(void) {
  GuiImageData *data =
      (GuiImageData *)memory_manager.calloc(1, sizeof(GuiImageData), MMTAG_GUI);
  data->sprite_id = NULL;
...
GuiWidgetVTable *ese_widget_image_get_vtable(void) { return &g_image_vtable; }
...
static void _image_draw(EseGui *gui, EseGuiWidget *widget,
  ...
  draw_list_object_set_texture(img_obj, data->sprite_id, 0.0f, 0.0f, 1.0f,
                               1.0f);
  draw_list_object_set_bounds(img_obj, widget->x, widget->y, widget->width,
                              widget->height);
...
static void _image_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
  ...
  log_assert("GUI", widget,
             "_image_process_mouse_hover called with NULL widget");
  const bool inside =
...
static bool _image_process_mouse_click(EseGuiWidget *widget, int mouse_x,
  ...
  (void)button;
  log_assert("GUI", widget,
             "_image_process_mouse_click called with NULL widget");
  const bool inside =
      (mouse_x >= widget->x && mouse_x < widget->x + widget->width &&
       mouse_y >= widget->y && mouse_y < widget->y + widget->height);
  return inside;
...
static void _image_layout(EseGuiWidget *widget) { (void)widget; }
...
void ese_widget_image_set(EseGuiWidget *image, const char *sprite_id,
                          EseGuiImageFit fit) {
...
static int _image_lua_init(EseLuaEngine *engine) {
  ...
  log_assert("GUI", engine->runtime,
             "_image_lua_init called with NULL lua runtime");
...
static int _image_lua_push(lua_State *L) {
  ...
  if (n_args < 1 || n_args > 3) {
    return luaL_error(
        L, "GUI.push_image(sprite_id[, fit[, style]]) takes 1 to 3 arguments");
  }
  ...
  EseGuiWidget *image = _image_create(
