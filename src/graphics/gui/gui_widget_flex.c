...
static bool _flex_process_mouse_click(EseGuiWidget *widget, int mouse_x, int mouse_y,
...
static void _flex_layout(EseGuiWidget *widget) {
    ...
    const int inner_w = widget->width - ese_gui_style_get_padding_left(widget->style) -
                        ese_gui_style_get_padding_right(widget->style);
    const int inner_h = widget->height - ese_gui_style_get_padding_top(widget->style) -
    ...
    log_verbose("GUI", "_flex_layout: widget=%p type=%s children=%zu inner=%dx%d spacing=%d\n",
    ...
    if (data->direction == FLEX_DIRECTION_ROW) {
        ...
        if (data->justify == FLEX_JUSTIFY_START) {
            ...
            start_x += widget->width - ese_gui_style_get_padding_right(widget->style) -
                       (fixed_width + total_spacing + auto_count * auto_width);
        } else if (data->justify == FLEX_JUSTIFY_CENTER) {
            start_x += ese_gui_style_get_padding_left(widget->style) +
                       (inner_w - (fixed_width + total_spacing + auto_count * auto_width)) / 2;
        }
        ...
        for (size_t i = 0; i < widget->children_count; i++) {
            ...
            log_verbose("GUI", "_flex_layout: child[%zu]=%p type=%s size=%dx%d\n", i, (void *)child,
                        child ? child->type.id : "<null>", child ? child->width : -9999,
                        child ? child->height : -9999);
            child->x = start_x;
            if (child->width == GUI_AUTO_SIZE) {
        ...
        if (data->justify == FLEX_JUSTIFY_START) {
            ...
            start_y += widget->height - ese_gui_style_get_padding_bottom(widget->style) -
                       (fixed_height + total_spacing + auto_count * auto_height);
        } else if (data->justify == FLEX_JUSTIFY_CENTER) {
            start_y += ese_gui_style_get_padding_top(widget->style) +
                       (inner_h - (fixed_height + total_spacing + auto_count * auto_height)) / 2;
        }
...
static int _flex_lua_open(lua_State *L) {
    ...
    if (gui->open_layout->root == NULL && gui->open_layout->current_widget == NULL) {
        ...
        gui->open_layout->current_widget =
            _flex_create(NULL, ese_gui_style_copy(opt_style ? opt_style : gui->default_style));
        ...
        gui->open_layout->current_widget =
            _flex_create(gui->open_layout->current_widget,
...
static int _flex_lua_close(lua_State *L) {
    ...
    if (current == NULL || strncmp(current->type.id, "FLEX", sizeof(current->type.id)) != 0) {
