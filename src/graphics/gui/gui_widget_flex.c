...
static void _flex_draw(EseGui *gui, EseGuiWidget *widget,
                       EseDrawList *draw_list, size_t depth);
static void _flex_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
                                      int mouse_y);
static bool _flex_process_mouse_click(EseGuiWidget *widget, int mouse_x,
                                      int mouse_y, EseInputMouseButton button);
...
static GuiFlexData *_flex_data_create(void) {
  GuiFlexData *data =
      (GuiFlexData *)memory_manager.calloc(1, sizeof(GuiFlexData), MMTAG_GUI);
...
GuiWidgetVTable *ese_widget_flex_get_vtable(void) { return &g_flex_vtable; }
  ...
  if (variant != GUI_STYLE_VARIANT_TRANSPARENT) {
    ...
    if (bg) {
      ...
      draw_list_object_set_rect_color(
          bg, ese_color_get_r(bg_color), ese_color_get_g(bg_color),
          ese_color_get_b(bg_color), ese_color_get_a(bg_color), true);
      draw_list_object_set_bounds(bg, widget->x, widget->y, widget->width,
                                  widget->height);
    }
    ...
    if (bd) {
      ...
      draw_list_object_set_rect_color(
          bd, ese_color_get_r(bd_color), ese_color_get_g(bd_color),
          ese_color_get_b(bd_color), ese_color_get_a(bd_color), false);
      draw_list_object_set_bounds(bd, widget->x, widget->y, widget->width,
                                  widget->height);
    }
  }

  for (size_t i = 0; i < widget->children_count; i++) {
    widget->children[i]->type.draw(gui, widget->children[i], draw_list,
                                   depth + 1);
  }
}
...
static void _flex_process_mouse_hover(EseGuiWidget *widget, int mouse_x,
  ...
  log_assert("GUI", widget,
             "_flex_process_mouse_hover called with NULL widget");
  ...
  for (size_t i = 0; i < widget->children_count; i++) {
    widget->children[i]->type.process_mouse_hover(widget->children[i], mouse_x,
                                                  mouse_y);
  }
}
...
static bool _flex_process_mouse_click(EseGuiWidget *widget, int mouse_x,
                                      int mouse_y, EseInputMouseButton button) {
  log_assert("GUI", widget,
             "_flex_process_mouse_click called with NULL widget");
  (void)button;
  ...
  for (size_t i = 0; i < widget->children_count; i++) {
    if (widget->children[i]->type.process_mouse_click(
            widget->children[i], mouse_x, mouse_y, button)) {
      return true;
    }
  }
...
static void _flex_layout(EseGuiWidget *widget) {
  ...
  const int inner_w = widget->width -
                      ese_gui_style_get_padding_left(widget->style) -
                      ese_gui_style_get_padding_right(widget->style);
  const int inner_h = widget->height -
                      ese_gui_style_get_padding_top(widget->style) -
                      ese_gui_style_get_padding_bottom(widget->style);
  const int child_count = (int)widget->children_count;
  const int total_spacing =
      (child_count > 0) ? data->spacing * (child_count - 1) : 0;
  log_verbose(
      "GUI",
      "_flex_layout: widget=%p type=%s children=%zu inner=%dx%d spacing=%d\n",
      (void *)widget, widget->type.id, widget->children_count, inner_w, inner_h,
      data->spacing);
  ...
  if (data->direction == FLEX_DIRECTION_ROW) {
    ...
    if (total_free_width < 0)
      total_free_width = 0;
    int auto_width =
        (auto_count > 0) ? (total_free_width - fixed_width) / auto_count : 0;
    if (auto_width < 0)
      auto_width = 0;
    ...
    if (data->justify == FLEX_JUSTIFY_START) {
      ...
      start_x += widget->width -
                 ese_gui_style_get_padding_right(widget->style) -
                 (fixed_width + total_spacing + auto_count * auto_width);
    } else if (data->justify == FLEX_JUSTIFY_CENTER) {
      start_x +=
          ese_gui_style_get_padding_left(widget->style) +
          (inner_w - (fixed_width + total_spacing + auto_count * auto_width)) /
              2;
    }
    ...
    for (size_t i = 0; i < widget->children_count; i++) {
      EseGuiWidget *child = widget->children[i];
      log_verbose("GUI", "_flex_layout: child[%zu]=%p type=%s size=%dx%d\n", i,
                  (void *)child, child ? child->type.id : "<null>",
                  child ? child->width : -9999, child ? child->height : -9999);
      child->x = start_x;
      if (child->width == GUI_AUTO_SIZE) {
    ...
    if (total_free_height < 0)
      total_free_height = 0;
    int auto_height =
        (auto_count > 0) ? (total_free_height - fixed_height) / auto_count : 0;
    if (auto_height < 0)
      auto_height = 0;
    ...
    if (data->justify == FLEX_JUSTIFY_START) {
      ...
      start_y += widget->height -
                 ese_gui_style_get_padding_bottom(widget->style) -
                 (fixed_height + total_spacing + auto_count * auto_height);
    } else if (data->justify == FLEX_JUSTIFY_CENTER) {
      start_y += ese_gui_style_get_padding_top(widget->style) +
                 (inner_h -
                  (fixed_height + total_spacing + auto_count * auto_height)) /
...
static int _flex_lua_init(EseLuaEngine *engine) {
  ...
  log_assert("GUI", engine->runtime,
             "_flex_lua_init called with NULL lua runtime");
  ...
  // Ensure STYLE constants exist: DIRECTION, JUSTIFY, ALIGN (mirrors old GUI
  // init) Get GUI.STYLE (create if missing)
...
static int _flex_lua_open(lua_State *L) {
  ...
  if (n_args <= 3 || n_args > 6) {
    return luaL_error(L, "GUI.open_flex(direction, justify, align_items, "
                         "[width[, height[, style]]]) takes 3 to 6 arguments");
  }
  ...
  if (gui->open_layout->root == NULL &&
      gui->open_layout->current_widget == NULL) {
    // first widget in layout
    gui->open_layout->current_widget = _flex_create(
        NULL, ese_gui_style_copy(opt_style ? opt_style : gui->default_style));
    ...
    return luaL_error(L,
                      "GUI.open_flex() called with no open container active");
  } else {
    gui->open_layout->current_widget = _flex_create(
...
static int _flex_lua_close(lua_State *L) {
  ...
  if (current == NULL ||
    ...
    return luaL_error(
        L, "GUI.close_flex() called but current container is not a Flex");
  }
