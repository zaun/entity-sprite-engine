...
void font_draw_text(EseEngine *engine, const char *font, const char *text,
                    ...
                    FontDrawTextureCallback texCallback,
  ...
  for (int i = 0; text[i]; i++) {
    ...
    if (c >= 32 && c <= 126) { // Printable ASCII
      ...
      if (letter) {
        ...
        texCallback((int)char_x, (int)start_y, w, h, draw_order, texture_id, x1,
                    y1, x2, y2, w, h, callback_user_data);
      }
    }
...
void font_draw_text_scaled(EseEngine *engine, const char *font,
                           const char *text, float start_x, float start_y,
                           uint64_t draw_order, float target_height,
                           FontDrawTextureCallback texCallback,
                           void *user_data) {
  ...
  log_assert("FONT", texCallback,
             "font_draw_text_scaled called with NULL callback");
  ...
  for (int i = 0; text[i]; i++) {
    ...
    if (c >= 32 && c <= 126) { // Printable ASCII
      ...
      if (letter) {
        ...
        texCallback((int)char_x, (int)start_y, scaled_w, scaled_h, draw_order,
