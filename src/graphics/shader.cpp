...
static bool compile_glsl_to_spirv(const char* source, int shaderStage, std::vector<uint32_t>& spirv, char* errBuf, size_t errBufSize) {
    glslang::TShader shader(static_cast<EShLanguage>(shaderStage));
    TBuiltInResource resources = GetDefaultResources();
    
    // Use proper SPIRV generation messages with debug info
    EShMessages messages = (EShMessages)(EShMsgSpvRules | EShMsgVulkanRules | EShMsgDebugInfo | EShMsgKeepUncalled);
    ...
    glslang::GlslangToSpv(*program.getIntermediate(static_cast<EShLanguage>(shaderStage)), spirv, &logger, &spvOptions);
...
static std::string spirv_to_metal(const std::vector<uint32_t>& spirv, int shaderStage) {
    ...
    if (!entry_points.empty()) {
        msl.rename_entry_point(entry_points[0].name, entry_point_name, entry_points[0].execution_model);
    }
    ...
    try {
        ...
        // This uses string analysis to detect what structs are referenced but not defined
        std::vector<std::string> missing_structs;
        std::string additional_code = "";
        
        // Check for missing input/output structs
        std::regex entry_func_regex(entry_point_name + R"(\([^)]*(\w+_in)\s+\w+\s+\[\[stage_in\]\])");
        std::smatch match;
        ...
        for (const std::string& struct_name : missing_structs) {
            ...
            if (struct_name.find("_in") != std::string::npos) {
                // Input struct - analyze function parameters or guess from vertex outputs
                if (shaderStage == EShLangFragment) {
                    ...
                    std::sregex_iterator out_iter(metal_code.begin(), metal_code.end(), vertex_out_regex);
                    for (; out_iter != end; ++out_iter) {
                        additional_code += "    " + (*out_iter)[1].str() + " " + (*out_iter)[2].str() + 
                                         " [[user(locn" + (*out_iter)[3].str() + ")]];\n";
                    }
                } else {
                    // Vertex input - analyze attribute declarations
                    std::regex attr_regex(R"((\w+)\s+(\w+)\s+\[\[attribute\((\d+)\)\]\])");
                    std::sregex_iterator attr_iter(metal_code.begin(), metal_code.end(), attr_regex);
                    for (; attr_iter != end; ++attr_iter) {
                        additional_code += "    " + (*attr_iter)[1].str() + " " + (*attr_iter)[2].str() + 
                                         " [[attribute(" + (*attr_iter)[3].str() + ")]];\n";
                    }
                }
                ...
                if (shaderStage == EShLangFragment) {
                    ...
                    std::sregex_iterator color_iter(metal_code.begin(), metal_code.end(), color_assign_regex);
                    ...
                    std::sregex_iterator out_iter(metal_code.begin(), metal_code.end(), out_assign_regex);
                    std::set<std::string> found_outputs;
                    int location = 0;
                    for (; out_iter != end; ++out_iter) {
                        ...
                        if (found_outputs.find(output_name) == found_outputs.end()) {
                            ...
                            if (output_name == "gl_Position") {
                                ...
                                additional_code += "    float2 " + output_name + " [[user(locn" + std::to_string(location++) + ")]];\n";
                            }
                        }
                    }
                }
                ...
                std::sregex_iterator member_iter(metal_code.begin(), metal_code.end(), member_access_regex);
                ...
                for (; member_iter != end; ++member_iter) {
                    ...
                    if (found_members.find(member_name) == found_members.end()) {
                        ...
                        if (member_name.find("Color") != std::string::npos || member_name.find("color") != std::string::npos) {
                            additional_code += "    float4 " + member_name + ";\n";
                        } else if (member_name.find("use") != std::string::npos || member_name.find("enable") != std::string::npos) {
...
// glsl_to_glsl:
// Convert GLSL source to GLSL output targeting desktop OpenGL (via SPIR-V intermediate).
// Returns null-terminated C string allocated on heap; free with free_shader_blob.
...
// glsl_to_metal:
// Convert GLSL source to Metal shading language source string (via SPIR-V intermediate).
// Returns null-terminated C string allocated on heap; free with free_shader_blob.
