...
static TBuiltInResource GetDefaultResources() {
  ...
  resources.maxDualSourceDrawBuffersEXT =
      1; // This should fix gl_MaxDualSourceDrawBuffersEXT
...
static bool compile_glsl_to_spirv(const char *source, int shaderStage,
                                  std::vector<uint32_t> &spirv, char *errBuf,
                                  size_t errBufSize) {
  ...
  EShMessages messages = (EShMessages)(EShMsgSpvRules | EShMsgVulkanRules |
                                       EShMsgDebugInfo | EShMsgKeepUncalled);
  ...
  glslang::GlslangToSpv(
      *program.getIntermediate(static_cast<EShLanguage>(shaderStage)), spirv,
      &logger, &spvOptions);

  if (!logger.getAllMessages().empty()) {
    log_debug("SHADER", "SPIRV generation messages: %s",
...
static std::string spirv_to_glsl(const std::vector<uint32_t> &spirv) {
  ...
  options.emit_uniform_buffer_as_plain_uniforms =
      true; // Convert UBOs to regular uniforms
...
static std::string spirv_to_metal(const std::vector<uint32_t> &spirv,
                                  int shaderStage) {
  ...
  options.msl_version =
      spirv_cross::CompilerMSL::Options::make_msl_version(2, 0);
  ...
  try {
    ...
    // This uses string analysis to detect what structs are referenced but not
    // defined
    ...
    for (const std::string &struct_name : missing_structs) {
      ...
      if (struct_name.find("_in") != std::string::npos) {
        // Input struct - analyze function parameters or guess from vertex
        // outputs
        if (shaderStage == EShLangFragment) {
          // Fragment input should match common vertex outputs
          std::regex vertex_out_regex(
          ...
          for (; out_iter != end; ++out_iter) {
            ...
            if (found_outputs.find(output_name) == found_outputs.end()) {
              ...
              if (output_name == "gl_Position") {
                additional_code +=
                    "    float4 " + output_name + " [[position]];\n";
              } else {
                additional_code += "    float2 " + output_name +
